...
const screenRecordingNode = document.getElementById(
...
async function refreshStatus() {
  ...
  const cuaMcp = status.mcpServers?.find(
    (server) => server.serverId === "cua-driver",
  );
  if (!cuaMcp) {
...
async function checkPermissions() {
  ...
  if (result.data.error) {
    console.warn(
      "[CUA Settings] Permission check returned diagnostics:",
      result.data,
    );
...
document.getElementById("guide")?.addEventListener("click", async () => {
  try {
    const result = await getPluginApi().invokeAction(
