...
description: Help developers build third-party tools that import, inspect, migrate, or analyze JiaorongAI data. Use when Codex needs to work with JiaorongAI provider configuration, model configuration, MCP/app settings, sessions, messages, legacy chat data, `agent.db`, `chat.db`, SQLCipher encrypted SQLite, Electron safeStorage wrapped passwords, Tauri importers, or native macOS/Windows/Linux data access.
---

# JiaorongAI Data Import

## Overview

Use this skill to design or implement importers for JiaorongAI local data. Treat JiaorongAI's SQLite
...
1. Identify the source: live JiaorongAI profile, copied profile, sync backup, exported `agent.db`, or
   legacy `chat.db`.
...
5. Read [references/import-recipes.md](references/import-recipes.md) when writing extractor code,
   mapping JiaorongAI data to another app, or creating a compatibility import.

## Safety Rules

- Get explicit user consent before reading local JiaorongAI data. Provider keys, OAuth tokens, MCP
...
- Never open the active `agent.db` read-write from a third-party tool. Copy `agent.db`,
  `agent.db-wal`, and `agent.db-shm`, or use SQLite backup APIs through JiaorongAI itself.
- If JiaorongAI is running, either ask the user to quit it or make a WAL-aware snapshot before import.
