...
function parseArgs(argv) {
  ...
  if (!args.pluginDir) {
    throw new Error(
      "Usage: node scripts/package-plugin.mjs [--validate] [--out <dir>] <pluginDir>",
    );
  }
  return args;
}
...
function validateManifest(pluginDir, manifest) {
  ...
  if (
    !Array.isArray(manifest.engines?.platforms) ||
    manifest.engines.platforms.length === 0
  ) {
  ...
  for (const contribution of manifest.settingsContributions ?? []) {
    assertFile(
      pluginDir,
      contribution.entry,
      `settings entry ${contribution.id}`,
    );
    assertFile(
      pluginDir,
      contribution.preloadTypes,
      `preload types ${contribution.id}`,
    );
  }
}
...
function artifactFileName(manifest, targetPlatform, targetArch) {
  ...
  const targetSuffix =
    targetPlatform && targetArch ? `-${targetPlatform}-${targetArch}` : "";
...
function createPackageManifest(manifest, args) {
  const version =
    args.version ||
    (args.releaseVersionFromRoot ? readRootPackageVersion() : manifest.version);
  ...
  if (next.source?.type === OFFICIAL_PLUGIN_SOURCE) {
    const assetName = artifactFileName(
      next,
      args.targetPlatform,
      args.targetArch,
    );
...
function validateCuaRuntime(pluginDir, manifest, args) {
  ...
  if (targetPlatform !== "darwin") {
    throw new Error(
      "CUA plugin packaging currently supports darwin runtime packages only",
    );
  }
  assertFile(
    pluginDir,
    `runtime/darwin/${args.targetArch}/JiaorongAI Computer Use.app/Contents/MacOS/cua-driver`,
    `CUA runtime binary ${targetPlatform}/${args.targetArch}`,
  );
  const expectedDetect = [
    `plugin:runtime/darwin/${args.targetArch}/JiaorongAI Computer Use.app/Contents/MacOS/cua-driver`,
    "/Applications/CuaDriver.app/Contents/MacOS/cua-driver",
  ];
  if (
    JSON.stringify(manifest.runtime?.detect ?? []) !==
    JSON.stringify(expectedDetect)
  ) {
    throw new Error(
      "CUA runtime detect paths must point to the bundled helper app first",
    );
  }

  const cuaServer = (manifest.mcpServers ?? []).find(
    (server) => server.id === "cua-driver",
  );
  if (!cuaServer) {
  ...
  if (cuaServer.command !== "${runtime.cua-driver.command}") {
    throw new Error(
      "CUA MCP server command must reference ${runtime.cua-driver.command}",
    );
  }
...
function packagePlugin(pluginDir, outDir, manifest, args) {
  ...
  files["plugin.json"] = new TextEncoder().encode(
  ...
  const outPath = path.join(
    outDir,
    artifactFileName(manifest, args.targetPlatform, args.targetArch),
  );
