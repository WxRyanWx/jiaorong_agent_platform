...
- Do not inspect authentication environment variables or JiaorongAI's local descriptor. Authorization
...
- Agent callers may consume a JiaorongAI-owned artifact with `--artifact <id>` and inspect metadata
...
- Generated media remains in JiaorongAI's artifact spool. Return the artifact metadata or ID so the
...
require the JiaorongAI UI or a human terminal.
