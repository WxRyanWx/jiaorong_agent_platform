...
const setupStore = async () => {
  vi.resetModules();
  vi.doUnmock("pinia");
  const { createPinia, setActivePinia } =
...
describe("useMcpStore toggleServer rollback", () => {
  ...
  it("restores local state and persisted config when runtime sync fails", async () => {
    ...
    setMcpServerEnabledMutate.mockRejectedValueOnce(
    ...
    expect(setMcpServerEnabledMutate).toHaveBeenNthCalledWith(1, [
      "demo",
      true,
    ]);
    expect(setMcpServerEnabledMutate).toHaveBeenNthCalledWith(2, [
      "demo",
      false,
    ]);
  ...
  it("hides enabled servers when MCP is globally disabled", async () => {
    ...
    expect(store.pluginServerList.map((server) => server.name)).toEqual([
      "cua-driver",
    ]);
    expect(store.enabledServers).toEqual([]);
    expect(store.enabledPluginServers.map((server) => server.name)).toEqual([
      "cua-driver",
    ]);
