...
describe('sessionStore.getFilteredGroups', () => {
  ...
  it('uses the last path segment for Windows project labels', async () => {
    ...
    store.sessions.value = [
      {
        ...
        agentId: 'deepchat',
        status: 'none',
        projectDir: 'C:\\Users\\JiaorongAI\\workspace',
        providerId: 'openai',
        modelId: 'gpt-4',
        isPinned: false,
        isDraft: false,
    ...
    expect(groups[0]?.id).toBe('C:\\Users\\JiaorongAI\\workspace')
...
describe('sessionStore streaming cleanup', () => {
  ...
  it('keeps restored model when re-selecting the current session', async () => {
    ...
    store.sessions.value = [createSession({ id: 'session-1' })]
    ...
    expect(store.activeSession.value?.modelId).toBe('gpt-4')

    await store.selectSession('session-1')

    expect(sessionClient.activate).toHaveBeenCalledWith('session-1')
    expect(clearStreamingState).not.toHaveBeenCalled()
    expect(store.activeSession.value?.providerId).toBe('openai')
    expect(store.activeSession.value?.modelId).toBe('gpt-4')
  })

  it('clears restored model when selecting a different session', async () => {
    const { store } = await setupStore()
    store.activeSessionId.value = 'session-1'
    store.sessions.value = [
      createSession({ id: 'session-1' }),
      createSession({ id: 'session-2', agentId: 'deepchat' })
    ]
    store.applyRestoredSession(
      createSession({
        id: 'session-1',
    ...
    expect(store.activeSession.value?.providerId).toBe('')
    expect(store.activeSession.value?.modelId).toBe('')
  })

  it('keeps restored model when the same session is activated again via IPC', async () => {
    const { store, emitSessionUpdate } = await setupStore()
    store.sessions.value = [createSession({ id: 'session-keep' })]
    store.activeSessionId.value = 'session-keep'
    store.applyRestoredSession(
      createSession({
        id: 'session-keep',
    ...
    expect(store.activeSession.value?.providerId).toBe('anthropic')
    expect(store.activeSession.value?.modelId).toBe('claude-3-7-sonnet')
  })

  it('does not clear a summary restored during activate race', async () => {
    const { store, sessionClient } = await setupStore()
    store.activeSessionId.value = 'session-a'
    ...
    // IPC/ChatPage restored the target session before selectSession resumes.
    store.applyRestoredSession(
      createSession({
        id: 'session-b',
        providerId: 'anthropic',
