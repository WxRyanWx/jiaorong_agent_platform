...
const setupStore = async (options: SetupStoreOptions = {}) => {
  ...
  const sessionClient = {
    ...
    setSessionModel: vi
      .fn()
      .mockImplementation(
        async (_sessionId: string, providerId: string, modelId: string) =>
  ...
  const resolveOnboardingStateAfterCompletion = (
    stepId: "first-chat" | "switch-model",
  ) => ({
    version: 1,
  ...
  const onboardingClient = {
      ...
      steps: [
        {
          ...
          required: true,
          status:
            onboardingCurrentStepId === "provider" ? "in_progress" : "pending",
        ...
        },
        {
          ...
          required: false,
          status:
            onboardingCurrentStepId === "skills" ? "in_progress" : "pending",
        ...
        },
        {
          ...
          required: false,
          status:
            onboardingCurrentStepId === "plugins" ? "in_progress" : "pending",
    ...
    setStepStatus: vi
      .fn()
      .mockImplementation(
        async ({ stepId }: { stepId: "first-chat" | "switch-model" }) =>
...
describe("sessionStore.getFilteredGroups", () => {
  ...
  it("hides pinned sessions from grouped list and exposes them in pinned list", async () => {
    ...
    const pinnedIds = store
      .getPinnedSessions(null)
  ...
  it("sorts fetched sessions alphabetically by title", async () => {
    ...
    expect(
      store.sessions.value.map((session: { title: string }) => session.title),
  ...
  it("sorts sessions inside project groups by most recent update", async () => {
    ...
    ).toEqual(["new-zulu", "middle-bravo", "old-alpha"]);
  });
...
describe("sessionStore group mode preferences", () => {
  ...
  it("serializes concurrent group mode writes and persists the last toggle", async () => {
    ...
    await store.fetchSessions();
    configClient.setSetting.mockImplementation(
...
describe("sessionStore.startNewConversation", () => {
  ...
  it("keeps the active session agent when all agents is selected during a chat", async () => {
    ...
    store.sessions.value = [
      createSession({ id: "session-active", agentId: "acp-a" }),
    ];
...
describe("sessionStore onboarding progress", () => {
  it("marks the first-chat step complete after creating the first session", async () => {
    const { store, onboardingClient, pageRouter, sessionClient } =
      await setupStore({
        onboardingCurrentStepId: "first-chat",
      });
...
describe("sessionStore streaming cleanup", () => {
  it("clears streaming state when switching active session", async () => {
    ...
    store.sessions.value = [
      createSession({ id: "session-b", agentId: "acp-a" }),
    ];
  ...
  it("routes to chat and syncs the selected agent on external session activation", async () => {
    const { store, pageRouter, emitSessionUpdate, agentStore } =
      await setupStore({
        selectedAgentId: "deepchat",
      });
    store.sessions.value = [
      createSession({ id: "session-external", agentId: "agent-b" }),
    ];
  ...
  it("updates the local session status immediately from the session status event", async () => {
    ...
    store.sessions.value = [
      createSession({ id: "session-status", status: "none" }),
    ];
