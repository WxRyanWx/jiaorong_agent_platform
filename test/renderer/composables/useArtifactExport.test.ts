...
describe("useArtifactExport", () => {
  it("exports mermaid as SVG via download", async () => {
    ...
    await api.exportSVG(
      mkArtifact("application/vnd.ant.mermaid", "graph TD; A-->B;", "m1"),
    );
  ...
  it("throws for invalid svg content", async () => {
    ...
    await expect(
      api.exportSVG(mkArtifact("image/svg+xml", "NOT_SVG", "bad")),
    ).rejects.toBeTruthy();
  });
  ...
  it("exports code and copies content", async () => {
    ...
    await api.copyContent(
      mkArtifact("application/vnd.ant.code", "hello world"),
    );
  ...
  it("copyAsImage delegates to captureAndCopy with proper config", async () => {
    ...
    const ok = await api.copyAsImage(mkArtifact("text/plain", "content"), {
      isDark: false,
      version: "1.0.0",
      texts: { brand: "JiaorongAI", tip: "tip" },
    });
    expect(ok).toBe(true);
