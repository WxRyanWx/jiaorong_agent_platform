...
const setup = async (options: SetupOptions = {}) => {
  ...
  const syncWeixinIlinkStatusFromSettings = () => {
    ...
    weixinIlinkState.status.accountCount =
      weixinIlinkState.settings.accounts.length;
    weixinIlinkState.status.accounts = weixinIlinkState.settings.accounts.map(
      (account) => ({
        ...account,
        state:
          weixinIlinkState.settings.remoteEnabled && account.enabled
            ? "running"
            : "disabled",
        connected: Boolean(
          weixinIlinkState.settings.remoteEnabled && account.enabled,
    ...
    weixinIlinkState.status.connectedAccountCount =
      weixinIlinkState.status.accounts.filter(
        (account) => account.connected,
      ).length;
    weixinIlinkState.status.bindingCount =
      weixinIlinkState.status.accounts.reduce(
  ...
  const remoteControlPresenter = {
        ...
        if (channel === "telegram") {
          return remoteState.bindings.map((binding) => ({
            ...
            threadId: binding.messageThreadId
              ? String(binding.messageThreadId)
              : null,
    ...
    removeChannelPrincipal: vi.fn(
      async (
        ...
        if (channel === "telegram") {
          ...
          remoteState.status.allowedUserCount =
            remoteState.pairingSnapshot.allowedUserIds.length;
          return;
        }

        if (channel === "feishu") {
          feishuState.pairingSnapshot.pairedUserOpenIds =
            feishuState.pairingSnapshot.pairedUserOpenIds.filter(
              (value) => value !== principalId,
            );
          feishuState.status.pairedUserCount =
            feishuState.pairingSnapshot.pairedUserOpenIds.length;
          return;
        }

        if (channel === "discord") {
          discordState.pairingSnapshot.pairedChannelIds =
            discordState.pairingSnapshot.pairedChannelIds.filter(
              (value) => value !== principalId,
            );
        ...
        qqbotState.pairingSnapshot.pairedUserIds =
          qqbotState.pairingSnapshot.pairedUserIds.filter(
            (value) => value !== principalId,
          );
        qqbotState.status.pairedUserCount =
          qqbotState.pairingSnapshot.pairedUserIds.length;
      },
    ),
    ...
    removeWeixinIlinkAccount: vi.fn(async (accountId: string) => {
      weixinIlinkState.settings.accounts =
        weixinIlinkState.settings.accounts.filter(
        ...
        return () =>
          h(
            "button",
            {
              ...attrs,
              "data-state":
                tabs.currentValue.value === props.value ? "active" : "inactive",
        ...
        return () =>
          h(
            "div",
            {
              ...
              style:
                tabs.currentValue.value === props.value
                  ? undefined
                  : { display: "none" },
            },
            slots.default?.(),
...
describe("RemoteSettings", () => {
  it("hides remote details when telegram remote is disabled", async () => {
    ...
    expect(
      wrapper.find('[data-testid="remote-control-details"]').exists(),
    ).toBe(false);
    expect(wrapper.text()).not.toContain(
      "settings.remote.remoteControl.streamMode",
    );
  });
  ...
  it("shows only the active tab content when switching channels", async () => {
    ...
    const feishuTrigger = wrapper
      ...
      .find(
        (component) =>
          component.attributes("data-testid") === "remote-tab-feishu",
      );
  ...
  it("toggles telegram remote control from the tab header", async () => {
    ...
    await wrapper
      .find('[data-testid="remote-channel-toggle-telegram"]')
      .setValue(true);
    await flushPromises();
    ...
    expect(
      wrapper.find('[data-testid="remote-bindings-button"]').exists(),
    ).toBe(true);
  });
  ...
  it("shows and removes authorized principals from the bindings dialog", async () => {
    ...
    await wrapper
      .find('[data-testid="remote-bindings-button"]')
      .trigger("click");
    await flushPromises();

    expect(wrapper.find('[data-testid="remote-principal-123"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="remote-binding-telegram:100:0"]').exists(),
    ).toBe(true);

    await wrapper
      .find('[data-testid="remote-principal-123"] button')
      .trigger("click");
    await flushPromises();

    expect(remoteControlPresenter.removeChannelPrincipal).toHaveBeenCalledWith(
      "telegram",
      "123",
    );
    expect(remoteState.pairingSnapshot.allowedUserIds).toEqual([]);
    expect(
      wrapper.find('[data-testid="remote-principals-empty"]').exists(),
    ).toBe(true);
  });
  ...
  it("shows feishu brand switch while keeping legacy paired-user raw inputs hidden", async () => {
    ...
    expect(
      wrapper
        .find('[data-testid="remote-feishu-paired-user-open-ids-input"]')
        .exists(),
    ).toBe(false);
    expect(wrapper.text()).toContain("settings.remote.feishu.brand");
    expect(wrapper.text()).toContain(
      "settings.remote.remoteControl.defaultWorkdir",
    );
    expect(
      wrapper.find('[data-testid="feishu-bindings-button"]').exists(),
    ).toBe(true);
  });
  ...
  it("shows a discord tab with bot token and pairing controls, without webhook fields", async () => {
    ...
    const discordTrigger = wrapper
      ...
      .find(
        (component) =>
          component.attributes("data-testid") === "remote-tab-discord",
      );
    ...
    expect(wrapper.text()).toContain(
      "settings.remote.discord.remoteControlDescription",
    );
    expect(wrapper.find('[data-testid="discord-pair-button"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="discord-bindings-button"]').exists(),
    ).toBe(true);
  });
  ...
  it("loads telegram settings without legacy hook fields", async () => {
    ...
    expect(
      wrapper.find('[data-testid="remote-default-agent-select"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="remote-allowed-user-ids-input"]').exists(),
    ).toBe(false);
  });
  ...
  it("uses remote control as the channel section title", async () => {
    ...
    expect(
      text.match(/settings\.remote\.sections\.remoteControl/g),
    ).toHaveLength(5);
  });
  ...
  it("does not create a separate lark tab when feishu brand switches to lark", async () => {
    ...
    expect(wrapper.find('[data-testid="remote-tab-feishu"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="remote-tab-lark"]').exists()).toBe(
      false,
    );
  });
  ...
  it("starts the wechat ilink qr login flow and shows the dialog", async () => {
    ...
    const weixinTrigger = wrapper
      ...
      .find(
        (component) =>
          component.attributes("data-testid") === "remote-tab-weixin-ilink",
      );
    ...
    await wrapper
      .find('[data-testid="weixin-ilink-connect-button"]')
      .trigger("click");
    await flushPromises();

    const connectButton = wrapper.find(
    ...
    expect(remoteControlPresenter.startWeixinIlinkLogin).toHaveBeenCalledTimes(
      1,
    );
    ...
    expect(wrapper.text()).toContain(
      "settings.remote.weixinIlink.loginWindowOpened",
    );
  });
  ...
  it("opens the pair dialog and closes it after pairing succeeds", async () => {
    ...
    expect(remoteControlPresenter.createChannelPairCode).toHaveBeenCalledWith(
      "telegram",
    );
    expect(wrapper.find('[data-testid="remote-pair-dialog"]').exists()).toBe(
      true,
    );
    ...
    expect(wrapper.find('[data-testid="remote-principal-456"]').exists()).toBe(
      true,
    );
  });
  ...
  it("lists only enabled agents in the default agent selector area", async () => {
    ...
    expect(wrapper.text()).not.toContain("JiaorongAI Alt");
    expect(wrapper.text()).toContain("ACP Agent (ACP)");
  });
  ...
  it("opens the bindings dialog and removes a binding from the list", async () => {
    ...
    expect(
      wrapper.find('[data-testid="remote-bindings-dialog"]').exists(),
    ).toBe(true);
    ...
    expect(wrapper.find('[data-testid="remote-bindings-empty"]').exists()).toBe(
      true,
    );
  });

  it("does not open bindings when saving feishu settings fails", async () => {
    const { wrapper, remoteControlPresenter, toast, tabsComponents } =
      await setup({
        feishuChannelSettingsOverride: {
          remoteEnabled: true,
        },
    ...
    await wrapper
      .find('[data-testid="feishu-bindings-button"]')
      .trigger("click");
    await flushPromises();
  ...
  it("renders the alias-equivalent agent label when binding holds a legacy ACP agent id", async () => {
    const { wrapper } = await setup({
      ...
      agents: [
        { id: "claude-acp", name: "Claude", type: "acp", enabled: true },
      ],
    });
