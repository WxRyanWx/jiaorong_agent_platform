...
const setup = async (
  ...
  const syncStore = reactive({
    ...
    saveCloudConfig: vi.fn().mockImplementation((config) => {
      syncStore.cloudConfig = {
        ...
        hasSecret:
          Boolean(config.secretAccessKey) || syncStore.cloudConfig.hasSecret,
      };
  ...
  vi.doMock("@api/legacy/presenters", () => ({
    useLegacyPresenter: (name: keyof typeof presenterMocks) =>
      presenterMocks[name],
  }));
  ...
  (
    window as typeof window & { api: { openExternal: typeof openExternal } }
  ).api = {
    openExternal,
  };
  vi.doMock("vue-i18n", () => ({
        ...
        (
          ({
            ...
            "settings.common.privacyModeDescription":
              "Stop automatic outbound requests owned by JiaorongAI:",
            "settings.common.privacyModeAutoUpdate": "App update checks",
            "settings.common.privacyModeProviderDb":
              "Provider and model metadata refresh",
            "settings.common.privacyModeAcpRegistry":
              "ACP Registry refresh and icon sync",
            "settings.common.privacyModeNpmRegistry":
              "MCP npm registry auto-detect",
            ...
            "settings.data.modelConfigUpdate.linkLabel":
  ...
  const DataSettings = (
    await import("../../../src/renderer/settings/components/DataSettings.vue")
  ).default;
...
const findRefreshButton = (wrapper: ReturnType<typeof mount>) =>
  findButtonByText(
    wrapper,
    "settings.data.modelConfigUpdate",
    "Refresh provider DB",
  );
...
const findDatabaseEncryptionButton = (
  wrapper: ReturnType<typeof mount>,
  text: string,
...
describe("DataSettings", () => {
  ...
  it("renders the consolidated sync and operations sections", async () => {
    ...
    expect(wrapper.text()).toContain(
      "settings.data.databaseEncryption.systemCredentialStore",
    );
  });
  ...
  it("defaults cloud sync setup to the R2 guide with R2 defaults", async () => {
    ...
    expect(wrapper.get('[data-testid="cloud-provider-r2"]').text()).toContain(
      "Cloudflare R2",
    );
    expect(wrapper.text()).toContain("settings.data.cloudSync.r2GuideTitle");
    expect(
      wrapper.get('[data-testid="cloud-r2-guide-endpoint"]').text(),
    ).toContain("settings.data.cloudSync.endpoint");
    expect(
      wrapper.get('[data-testid="cloud-r2-guide-access-key"]').text(),
    ).toContain("settings.data.cloudSync.accessKeyId");
    expect(
      wrapper.get('[data-testid="cloud-r2-guide-secret"]').text(),
    ).toContain("settings.data.cloudSync.secretAccessKey");
    expect(
      (wrapper.get("#cloud-r2-region").element as HTMLInputElement).value,
    ).toBe("auto");
    expect(
      (wrapper.get("#cloud-r2-prefix").element as HTMLInputElement).value,
    ).toBe("deepchat-backups");
  });
  ...
  it("keeps long sync failure text wrapped inside the error dialog", async () => {
    ...
    expect(
      wrapper.get('[data-testid="sync-error-dialog-footer"]').exists(),
    ).toBe(true);
    expect(
      wrapper.get('[data-testid="sync-error-dialog-confirm"]').exists(),
    ).toBe(true);
  });
  ...
  it("blocks Cloudflare API token values in the R2 secret field", async () => {
    ...
    await wrapper
      .get('[data-testid="cloud-secret-input"]')
      .setValue("cfat_example");

    expect(
      wrapper.get('[data-testid="cloud-secret-token-error"]').text(),
    ).toContain("Use the S3 Secret Access Key");
    expect(
      wrapper.get('[data-testid="cloud-save-test"]').attributes("disabled"),
    ).toBeDefined();
  ...
  it("switches cloud sync setup to custom S3-compatible fields", async () => {
    ...
    expect(
      wrapper.get('[data-testid="cloud-provider-custom"]').text(),
    ).toContain("Custom S3-compatible");
  ...
  it("falls back a blank custom S3 region to auto when saving cloud config", async () => {
    ...
    expect(
      wrapper.get('[data-testid="cloud-save-only"]').attributes("disabled"),
    ).toBeUndefined();
  ...
  it("renders a quiet danger zone entry and keeps reset choices in the dialog", async () => {
    ...
    expect(
      wrapper.find('[data-testid="danger-zone-reset-option-chat"]').exists(),
    ).toBe(true);
    expect(
      wrapper
        .find('[data-testid="danger-zone-reset-option-knowledge"]')
        .exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="danger-zone-reset-option-config"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="danger-zone-reset-option-all"]').exists(),
    ).toBe(true);
  });
  ...
  it("wires the privacy switch to its visible label and description", async () => {
    ...
    expect(privacySwitch.attributes("aria-labelledby")).toBe(
      "privacy-mode-label",
    );
    expect(privacySwitch.attributes("aria-describedby")).toBe(
      "privacy-mode-desc",
    );
  ...
  it("enables database encryption after matching password input", async () => {
    ...
    const inputs = wrapper.findAll(
      'input[type="password"]:not([data-testid="cloud-secret-input"])',
    );
  ...
  it("shows database encryption status as unknown when status loading fails", async () => {
    const { wrapper } = await setup({
      databaseSecurityGetStatus: vi
        .fn()
        .mockRejectedValue(new Error("status unavailable")),
    });

    expect(wrapper.text()).toContain(
      "settings.data.databaseEncryption.unknown",
    );
    expect(wrapper.text()).not.toContain(
      "settings.data.databaseEncryption.disabled",
    );
    expect(wrapper.text()).not.toContain(
      "settings.data.databaseEncryption.notRequired",
    );
    expect(
      wrapper
        .findAllComponents(buttonStub)
        .some((button) =>
          button
            .text()
            .includes("settings.data.databaseEncryption.setPasswordButton"),
        ),
    ).toBe(false);
  });
  ...
  it("shows an error toast when updating privacy mode fails", async () => {
    ...
    uiSettingsStore.setPrivacyModeEnabled = vi
      .fn()
  ...
  it("does not render a repair result summary before any repair run", async () => {
    ...
    expect(wrapper.text()).not.toContain(
      "settings.data.databaseRepair.lastResultLabel",
    );
    expect(wrapper.text()).not.toContain(
      "settings.data.databaseRepair.notCheckedYet",
    );
  });
  ...
  it("calls refreshProviderDb, shows loading state, then shows an updated toast", async () => {
    ...
    expect(loadingButton.text()).toContain(
      "settings.data.modelConfigUpdate.updating",
    );
    ...
    expect(
      presenterMocks.configPresenter.refreshProviderDb,
  ...
  it("runs schema repair and shows a healthy toast summary", async () => {
    ...
    expect(presenterMocks.sqlitePresenter.repairSchema).toHaveBeenCalledTimes(
      1,
    );
    expect(toast).toHaveBeenCalledWith({
  ...
  it("renders the PublicProviderConf link and opens it externally when clicked", async () => {
    ...
    const projectLink = wrapper.find(
      'a[href="https://github.com/ThinkInAIXYZ/PublicProviderConf"]',
    );
    ...
    expect(openExternal).toHaveBeenCalledWith(
      "https://github.com/ThinkInAIXYZ/PublicProviderConf",
    );
  });
  ...
  it("keeps reset data enabled when sync is disabled", async () => {
    ...
    expect(
      findResetEntryButton(wrapper).attributes("disabled"),
    ).toBeUndefined();
    expect(
      findResetConfirmButton(wrapper).attributes("disabled"),
    ).toBeUndefined();
  });
  ...
  it("disables reset actions during import and blocks the reset handler", async () => {
    ...
    expect(
      presenterMocks.devicePresenter.resetDataByType,
  ...
  it("defaults reset type to chat when opening the reset dialog", async () => {
    ...
    await wrapper
      .find('[data-testid="danger-zone-reset-option-all"]')
      .trigger("click");
    await findResetEntryButton(wrapper).trigger("click");
    await findResetConfirmButton(wrapper).trigger("click");
    await flushPromises();

    expect(presenterMocks.devicePresenter.resetDataByType).toHaveBeenCalledWith(
      "chat",
    );
  });
