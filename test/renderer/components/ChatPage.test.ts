...
const setup = async (options: SetupOptions = {}) => {
  ...
  vi.doMock('@jiaorong/knowledgeBase/picker/KnowledgeBasePickerButton.vue', () => ({
    default: defineComponent({
      name: 'KnowledgeBasePickerButton',
      template: '<button class="kb-picker-button-stub" />'
    })
  }))
  vi.doMock('@jiaorong/knowledgeBase/picker/KnowledgeBaseSelectionChips.vue', () => ({
    default: defineComponent({
      name: 'KnowledgeBaseSelectionChips',
      template: '<div class="kb-selection-chips-stub" />'
    })
  }))
  vi.doMock('@jiaorong/knowledgeBase/picker/prepareKnowledgeBaseSendFiles', () => ({
    prepareKnowledgeBaseSendFiles: vi.fn(
      async (_sessionId: string, _text: string, files: unknown[]) => ({
        ok: true,
        files
      })
    )
  }))
  vi.doMock('@jiaorong/knowledgeBase/picker/useKnowledgeBaseSelection', () => ({
    useKnowledgeBaseSelection: () => ({
      items: { value: [] },
      setItems: vi.fn(),
      removeByKey: vi.fn()
    }),
    clearKnowledgeBaseSelectionForSession: vi.fn()
  }))
  vi.doMock('vue-i18n', () => ({
    useI18n: () => ({
      t: (key: string) => key,
...
describe('ChatPage', () => {
  ...
  it('restores session messages immediately on mount', async () => {
    const { messageStore, pendingInputStore } = await setup()

    expect(messageStore.clear).toHaveBeenCalledTimes(1)
    expect(pendingInputStore.clear).toHaveBeenCalledTimes(1)
    expect(messageStore.loadMessages).toHaveBeenCalledWith('s1', 8)
    expect(pendingInputStore.loadPendingInputs).toHaveBeenCalledWith('s1')
  })

  it('requests older history on wheel-up when already pinned at the top', async () => {
    const { wrapper, messageStore } = await setup()
    messageStore.hasMoreHistory = true
    messageStore.loadOlderMessages = vi.fn().mockImplementation(async () => {
      messageStore.hasMoreHistory = false
      return 6
    })

    const chatPage = wrapper.get('[data-testid="chat-page"]')
    const el = chatPage.element as HTMLDivElement
    ...
    await chatPage.trigger('wheel', { deltaY: -40 })
    await flushPromises()

    expect(messageStore.loadOlderMessages).toHaveBeenCalled()
  })
