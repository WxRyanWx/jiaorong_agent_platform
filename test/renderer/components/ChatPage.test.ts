...
const setup = async (options: SetupOptions = {}) => {
  ...
  const messageStore = reactive({
    ...
    getAssistantMessageBlocks: vi.fn((message: { content: string }) =>
      JSON.parse(message.content),
    ),
    getUserMessageContent: vi.fn((message: { content: string }) =>
      JSON.parse(message.content),
    ),
    getMessageMetadata: vi.fn((message: { metadata: string }) =>
  ...
  const chatRespondToolInteraction = vi
    .fn()
  ...
  vi.doMock("@/components/chat/ChatTopBar.vue", () => ({
    default: defineComponent({
      ...
      template:
        '<div class="chat-top-bar-stub" :data-read-only="String(isReadOnly)" />',
    }),
  }));
  ...
  vi.doMock("@/components/chat/ChatInputBox.vue", () => ({
    default: defineComponent({
      ...
      template:
        '<div class="chat-input-box-stub"><slot name="toolbar" /></div>',
    }),
  }));
  ...
  vi.doMock("@/components/chat/AgentProgressFloat.vue", () => ({
    default: defineComponent({
      ...
      template:
        '<button class="agent-progress-float-stub" @click="$emit(\'toggle-collapse\')" />',
    }),
  }));
...
describe("ChatPage", () => {
  ...
  it("defers session restore until startup deferred tasks are released", async () => {
    const { messageStore, pendingInputStore, flushStartupDeferredTasks } =
      await setup({
        deferStartupTasks: true,
      });
  ...
  it("runs manual compaction instead of sending exact /compact in JiaorongAI sessions", async () => {
  ...
  it("does not queue or compact exact /compact while generating", async () => {
    const { wrapper, chatClient, sessionClient, pendingInputStore } =
      await setup({
        isStreaming: true,
        activeSessionPatch: {
          providerId: "openai",
  ...
  it("extracts ephemeral rate-limit streaming blocks instead of creating a virtual assistant message", async () => {
    ...
    expect(messageList.props("ephemeralRateLimitMessageId")).toBe(
      "__rate_limit__:s1:1",
    );
    ...
    expect(
      wrapper.find(".message-list-stub").attributes("data-has-rate-limit"),
    ).toBe("true");
  });
  ...
  it("keeps pending lane visible below the tool interaction overlay", async () => {
    ...
    expect(wrapper.find(".chat-tool-interaction-overlay-stub").exists()).toBe(
      true,
    );
  ...
  it("keeps the interaction overlay open after an inline skill draft view", async () => {
    ...
    chatRespondToolInteraction.mockResolvedValueOnce({
      accepted: true,
      handledInline: true,
    });
  ...
  it("inserts workspace references into the active chat input for the matching session", async () => {
    ...
    expect(chatInputInsertWorkspaceReference).toHaveBeenCalledWith(
      "C:/repo/README.md",
    );
  });

  it("routes tool interaction responses through ChatClient and refreshes messages", async () => {
    const { wrapper, chatClient, agentSessionPresenter, messageStore } =
      await setup({
        messages: [
          buildAssistantMessage([
            {
  ...
  it("renders pending lane above the input box when no tool interaction is active", async () => {
    ...
    expect(
      wrapper
        .findComponent({ name: "PendingInputLane" })
        .props("showResumeQueue"),
    ).toBe(true);
    expect(html.indexOf("pending-input-lane-stub")).toBeLessThan(
  ...
  it("renders subagent sessions as read-only display mode", async () => {
    ...
    expect(
      wrapper.find(".chat-top-bar-stub").attributes("data-read-only"),
    ).toBe("true");
    expect(
      wrapper.find(".message-list-stub").attributes("data-read-only"),
    ).toBe("true");
    expect(wrapper.find(".chat-input-box-stub").exists()).toBe(false);
    ...
    expect(wrapper.findComponent({ name: "ChatStatusBar" }).exists()).toBe(
      false,
    );
  });
  ...
  it("consumes pending spotlight message jumps after loading the target session", async () => {
    ...
    expect(wrapper.find('[data-message-id="m1"]').classes()).toContain(
      "message-highlight",
    );
