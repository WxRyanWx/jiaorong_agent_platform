...
function buildDashboard(
  overrides: Partial<UsageDashboardData> = {},
): UsageDashboardData {
  return {
...
async function setup(
  ...
  vi.resetModules();
  const getUsageDashboard =
    options.getUsageDashboard ?? vi.fn().mockResolvedValue(data);
  const retryRtkHealthCheck =
    options.retryRtkHealthCheck ?? vi.fn().mockResolvedValue(undefined);
    ...
    useI18n: () => ({
      ...
      t: (key: string, params?: Record<string, unknown>) => {
        ...
        if (key === "settings.dashboard.rtk.description") {
          return "Estimated tokens prevented from reaching the model context by RTK during JiaorongAI native command execution.";
        }
        if (key === "settings.dashboard.rtk.actions.retry")
          return "Retry check";
        ...
        if (key === "settings.dashboard.rtk.status.unhealthy")
          return "Unavailable";
        ...
        if (key === "settings.dashboard.rtk.descriptionChecking") {
          return "JiaorongAI is verifying whether RTK can run.";
        }
        ...
        if (key === "settings.dashboard.rtk.source.none")
          return "Not available";
        if (key === "settings.dashboard.rtk.summary.savedTokens")
          return "Saved tokens";
        if (key === "settings.dashboard.rtk.summary.commands")
          return "Tracked commands";
        if (key === "settings.dashboard.rtk.summary.avgSavingsPct")
          return "Average savings";
        if (key === "settings.dashboard.rtk.summary.outputTokens")
        ...
        if (key === "settings.dashboard.summary.nostalgiaDaysDetail") {
          return `You and JiaorongAI have spent ${params?.days ?? "0"} days together.`;
        }
...
describe("DashboardSettings", () => {
  ...
  it("renders the backfill banner while historical stats are initializing", async () => {
    ...
    expect(
      wrapper.find('[data-testid="dashboard-backfill-banner"]').exists(),
    ).toBe(true);
  });
  ...
  it("renders summary cards and breakdown rows when stats exist", async () => {
    ...
    const calendarHeatmap = wrapper.get(
      '[data-testid="dashboard-calendar-heatmap"]',
    );
    const calendarWeeks = wrapper.get(
    ...
    expect(wrapper.text()).toContain(
      "You and JiaorongAI have spent 17 days together.",
    );
    ...
    expect(wrapper.text()).not.toContain(
      "settings.dashboard.summary.cacheHitRate",
    );
    expect(summaryCards).toHaveLength(2);
    expect(header.classes()).toContain("flex-col");
    expect(header.classes()).toContain("sm:flex-row");
    expect(
      wrapper.find('[data-testid="summary-card-tokenUsage"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="summary-card-nostalgia"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="token-usage-trend-chart"]').exists(),
    ).toBe(true);
    expect(wrapper.findComponent({ name: "ChartTooltip" }).exists()).toBe(true);
    expect(wrapper.findComponent({ name: "ChartCrosshair" }).exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="token-usage-input-dot"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="token-usage-input-dot"]').attributes("style"),
    ).toContain("var(--primary-600)");
    expect(
      wrapper.find('[data-testid="token-usage-output-dot"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="token-usage-cached-dot"]').exists(),
    ).toBe(true);
    expect(wrapper.find('[data-testid="token-usage-cost-dot"]').exists()).toBe(
      true,
    );
    expect(wrapper.findAllComponents({ name: "VisArea" })).toHaveLength(4);
    expect(wrapper.find('[data-testid="token-usage-total-row"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="token-usage-cost-row"]').text(),
    ).toContain("Trend over the last 30 days");
    expect(
      wrapper.find('[data-testid="token-usage-list"]').text(),
    ).not.toContain("Uncached");
    expect(wrapper.find('[data-testid="cached-tokens-bar"]').exists()).toBe(
      false,
    );
    expect(
      wrapper.find('[data-testid="provider-breakdown-chart"]').exists(),
    ).toBe(true);
    expect(wrapper.find('[data-testid="model-breakdown-chart"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="provider-breakdown-scroll"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="model-breakdown-scroll"]').exists(),
    ).toBe(true);
    expect(wrapper.find('[title="1,200"]').exists()).toBe(true);
    expect(
      wrapper.findAll('[data-testid="calendar-cell"]').length,
    ).toBeGreaterThan(0);
    expect(calendarHeatmap.classes()).toContain("calendar-heatmap");
    expect(calendarWeeks.attributes("style")).toContain(
      "repeat(4, minmax(0, 1fr))",
    );
    expect(tokenUsageList.classes()).toContain("dashboard-token-usage-list");
    expect(
      wrapper.find('[data-testid="summary-card-nostalgia"]').html(),
    ...
    expect(
      wrapper.find('[data-testid="summary-card-nostalgia"]').html(),
    ).toContain("lg:grid-cols-[minmax(0,13rem)_minmax(0,1fr)]");
    expect(wrapper.find('[data-testid="nostalgia-details"]').html()).toContain(
      "space-y-2",
    );
    expect(
      wrapper.find('[data-testid="nostalgia-rotating-value"]').text(),
    ).toBe("17 days");
  ...
  it("renders RTK savings summary when RTK is healthy", async () => {
    ...
    expect(wrapper.find('[data-testid="rtk-card"]').html()).toContain(
      "dashboard-rtk-summary-grid",
    );
    expect(wrapper.find('[data-testid="rtk-status-badge"]').text()).toBe(
      "Bundled",
    );
    expect(wrapper.find('[data-testid="rtk-summary-saved"]').text()).toContain(
      "3.8k",
    );
    expect(
      wrapper.find('[data-testid="rtk-summary-commands"]').text(),
    ).toContain("12");
    expect(wrapper.find('[data-testid="rtk-summary-rate"]').text()).toContain(
      "76%",
    );
    expect(wrapper.find('[data-testid="rtk-status-copy"]').exists()).toBe(
      false,
    );
  });
  ...
  it("renders an empty trend summary with 0% ratios when total tokens are zero", async () => {
    ...
    expect(
      wrapper.find('[data-testid="summary-card-tokenUsage"]').text(),
    ).toContain("0");
    expect(
      wrapper.find('[data-testid="token-usage-trend-chart"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="total-tokens-input-ratio"]').text(),
    ).toBe("0%");
    expect(
      wrapper.find('[data-testid="total-tokens-output-ratio"]').text(),
    ).toBe("0%");
    expect(
      wrapper.find('[data-testid="cached-tokens-cached-ratio"]').text(),
    ).toBe("0%");
  });
  ...
  it("renders cached token ratio without uncached rows when input tokens are zero", async () => {
    ...
    expect(
      wrapper.find('[data-testid="cached-tokens-uncached-ratio"]').exists(),
    ).toBe(false);
  });
  ...
  it("renders N/A for days together when the first usage record is unavailable", async () => {
    ...
    expect(summaryCard.text()).toContain(
      "You have shared 3 sessions together.",
    );
    expect(
      wrapper.find('[data-testid="nostalgia-rotating-value"]').text(),
    ).toBe("3 sessions");
  ...
  it("renders N/A for the most active day when that summary is unavailable", async () => {
    ...
    expect(
      wrapper.find('[data-testid="nostalgia-detail-most-active-day"]').text(),
    ).toContain("N/A");
  });
