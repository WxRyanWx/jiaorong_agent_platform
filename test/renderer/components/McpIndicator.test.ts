...
const setup = async (options?: {
  ...
  const mcpStore = reactive({
    enabledServers: regularMcpEnabled
      ? [{ name: "demo-server", icons: "D", enabled: true }]
      : [],
      ...
      ? [
          {
            name: "cua-driver",
            icons: "plugin",
            descriptions: "CUA Driver",
            enabled: true,
          },
        ]
      : [],
    enabledServerCount: regularMcpEnabled ? 1 : 0,
    tools: regularMcpEnabled
      ? [buildTool("mcp_tool", "demo-server", "mcp")]
      : [],
    visibleTools: regularMcpEnabled
      ? [buildTool("mcp_tool", "demo-server", "mcp")]
      : [],
    pluginTools,
  });
  ...
  vi.doMock("@api/SessionClient", () => ({
    createSessionClient: vi.fn(() => ({
      getSessionDisabledAgentTools:
        agentSessionPresenter.getSessionDisabledAgentTools,
      updateSessionDisabledAgentTools:
        agentSessionPresenter.updateSessionDisabledAgentTools,
    })),
  }));
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, unknown>) => {
        ...
        const translations: Record<string, string> = {
          ...
          "chat.input.tools.groups.deepchatSettings": "JiaorongAI Settings",
  ...
  const McpIndicator = (
    await import("@/components/chat-input/McpIndicator.vue")
  ).default;
...
describe("McpIndicator", () => {
  it("renders icon-only trigger for deepchat and keeps built-in tools session scoped", async () => {
    ...
    expect(wrapper.text().indexOf("Tools")).toBeLessThan(
  ...
  it("supports enabling and disabling a whole tool group", async () => {
    ...
    expect(
      agentSessionPresenter.updateSessionDisabledAgentTools,
    ).toHaveBeenCalledWith("s1", ["exec", "read"]);
  });
  ...
  it("renders update_plan inside Agent Core and toggles it individually", async () => {
    ...
    expect(
      agentSessionPresenter.updateSessionDisabledAgentTools,
    ).toHaveBeenCalledWith("s1", ["update_plan"]);
  });
  ...
  it("renders subagent as a regular tool button inside Agent Core and emits updates", async () => {
    ...
    const subagentButton = wrapper
      .findAll("button")
      .find((node) => node.text() === "subagent");
