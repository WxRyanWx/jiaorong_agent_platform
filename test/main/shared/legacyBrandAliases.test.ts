...
describe('legacyBrandAliases', () => {
  ...
  it('maps legacy cli skill name to jiaorong-cli', () => {
    expect(resolveLegacySkillName('deepchat-cli')).toBe('jiaorong-cli')
    expect(resolveLegacySkillName('jiaorong-cli')).toBe('jiaorong-cli')
  })
  ...
  it('exposes reverse alias lookups for display localization', () => {
    ...
    expect(getLegacySkillAliasNames('jiaorong-cli')).toEqual(['deepchat-cli'])
  })
})
