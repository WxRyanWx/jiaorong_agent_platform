...
const {
  ...
  mockRunAiSdkGenerateText: vi
    .fn()
    .mockResolvedValue({ content: "mock completion" }),
}));
...
const presenterRuntimeMock = vi.hoisted(() => ({
  toolPresenter: {
    ...
    callTool: vi
      .fn()
      .mockResolvedValue({ content: "Mock tool response", rawData: {} }),
  },
  mcpPresenter: {
    getAllToolDefinitions: vi.fn().mockResolvedValue([]),
    callTool: vi
...
describe("LLMProviderPresenter Integration Tests", () => {
  ...
  beforeAll(() => {
    ...
    mockConfigPresenter =
      mockConfigPresenterInstance as unknown as ConfigPresenter;
  });
  ...
  afterEach(async () => {
    ...
    const activeStreams = (llmProviderPresenter as any).activeStreams as Map<
      string,
      any
    >;
  ...
  describe("Basic Provider Management", () => {
    ...
    it("defers provider bootstrap until a provider instance is requested", async () => {
      const fetchSpy = vi
        .spyOn(AiSdkProvider.prototype, "fetchModels")
        .mockResolvedValue([]);
    ...
    it("should resolve novita via apiType fallback without an id-specific provider mapping", () => {
      ...
      mockConfigPresenter.getProviders = vi
        .fn()
        .mockReturnValue([novitaProvider]);
      mockConfigPresenter.getProviderById = vi
        .fn()
        .mockReturnValue(novitaProvider);
      ...
      const providerInstance =
        llmProviderPresenter.getProviderInstance("novita");
  ...
  describe("Non-stream Completion", () => {
    ...
    it("should summarize titles", async () => {
      const messages = [
        {
          role: "user" as const,
          content: "Hello, I want to learn about artificial intelligence",
        },
        {
          role: "assistant" as const,
          content:
            "I can help you learn about AI. What specific aspects interest you?",
        },
      ];
    ...
    it("should swallow ACP warmup shutdown errors", async () => {
      ...
      const mockAcpProvider = {
        warmupProcess: vi
          ...
          .mockRejectedValue(
            new Error(
              "[ACP] Process manager is shutting down, refusing to spawn",
            ),
          ),
      };
      vi.spyOn(
        llmProviderPresenter as any,
        "getAcpProviderInstance",
      ).mockReturnValue(mockAcpProvider as any);
