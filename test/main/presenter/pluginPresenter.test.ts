...
const createPluginPresenter = async (
  ...
  const options =
    typeof optionsOrAppPath === "string"
      ? { appPath: optionsOrAppPath }
      : optionsOrAppPath;
...
const createBundledFixture = async (
  ...
  const packagePath = path.join(
    packageRoot,
    "deepchat-plugin-fixture-0.2.3-darwin-x64.dcplugin",
  );
...
const createDirectoryFixture = async (
  ...
  const root = await mkdtemp(
    path.join(os.tmpdir(), "deepchat-plugin-dir-test-"),
  );
  tempRoots.push(root);
  const appPath = options.appPath ?? path.join(root, "app");
  const userDataPath = path.join(root, "userData");
  ...
  delete (staleInstalledManifest as { settingsContributions?: unknown })
    .settingsContributions;
  ...
  await writeFile(
    path.join(pluginRoot, "mcp", "serve.mjs"),
    'console.log("serve")\n',
  );
  await writeFile(
  ...
  await writeFile(
    path.join(installedRoot, "mcp", "legacy.mjs"),
    'console.log("legacy")\n',
  );
  vi.mocked(app.getPath).mockImplementation((name: string) => {
    if (name === "userData") {
...
describe("PluginPresenter", () => {
  ...
  it("hides the CUA official plugin on unsupported platforms", async () => {
    ...
    const manifest = JSON.parse(
      await readFile("plugins/cua/plugin.json", "utf8"),
    );

    expect(manifest.engines.platforms).toEqual(["darwin"]);
    expect(
      (await winPresenter.listPlugins()).map((plugin) => plugin.id),
    ).not.toContain("com.deepchat.plugins.cua");
    expect(
      (await linuxPresenter.listPlugins()).map((plugin) => plugin.id),
    ).not.toContain("com.deepchat.plugins.cua");
  });
  ...
  it("restores plugin settings from the installed manifest when stored resources are missing", async () => {
    ...
    expect(
      vi.mocked(BrowserWindow).mock.results[0]?.value.loadFile,
  ...
  it("opens settings for a disabled packaged plugin that declares a settings contribution", async () => {
    ...
    const plugin = (await presenter.listPlugins()).find(
      (item) => item.id === fixture.pluginId,
    );
  ...
  it("refreshes stale same-version installs before startup activation and preserves config", async () => {
    ...
    await writeFile(
      path.join(fixture.installedRoot, "config.json"),
    ...
    expect(installedManifest.mcpServers[0].args).toEqual([
      "${plugin.root}/mcp/serve.mjs",
    ]);
    expect(
      fs.existsSync(path.join(fixture.installedRoot, "mcp", "serve.mjs")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(fixture.installedRoot, "mcp", "legacy.mjs")),
    ).toBe(false);
    ...
    expect(presenter.__mocks.mcpPresenter.startServer).toHaveBeenCalledWith(
      "fixture-tools",
    );
  });
  ...
  it("syncs dev directory installs even when only the plugin files changed", async () => {
    ...
    const currentManifest = await readFile(
      path.join(fixture.pluginRoot, "plugin.json"),
      "utf8",
    );
    const config = {
      appId: "cli_fixture_app_id",
    ...
    const serveScript = await readFile(
      path.join(fixture.installedRoot, "mcp", "serve.mjs"),
      "utf8",
    );
    const configAfterRefresh = JSON.parse(
  ...
  it("removes persisted plugin state when discovery rejects an installed official plugin", async () => {
    ...
    const presenter = await createPluginPresenter("darwin", fixture.appPath);
    const { getPluginToolPolicy } =
      await import("@/presenter/pluginPresenter/toolPolicyStore");
  ...
  it("loads official packages only from resources roots in packaged mode", async () => {
    const cwdRoot = await mkdtemp(
      path.join(os.tmpdir(), "deepchat-plugin-cwd-"),
    );
  ...
  it("loads the electron-vite plugin settings preload output", async () => {
    ...
    expect(presenterSource).not.toContain(
      "../preload/plugin-settings-preload.mjs",
    );
  });
  ...
  it("resolves CUA helper paths, MCP env, and runtime auto-start hooks", async () => {
    ...
    expect(presenterSource).toContain(
      "this.mcpPresenter.startServer(serverName)",
    );
    expect(presenterSource).not.toContain(
  ...
  it("declares the CUA MCP server with plugin helper context", async () => {
    ...
    const mcpConfig = JSON.parse(
      await readFile("plugins/cua/mcp/cua-driver.json", "utf8"),
    );
    const server = manifest.mcpServers.find(
  ...
  it("keeps new CUA cursor style controls permission-gated", async () => {
    ...
    const policy = JSON.parse(
      await readFile("plugins/cua/policies/tool-policy.json", "utf8"),
    );
  ...
  it("tracks CUA vendor source as a JiaorongAI-owned fork", async () => {
  ...
  it("keeps CUA updates managed by JiaorongAI instead of upstream release checks", async () => {
    ...
    expect(commandSource).toContain(
      "JiaorongAI packages this cua-driver fork with the app.",
    );
    expect(commandSource).toContain(
      "Update JiaorongAI to receive newer Computer Use helper builds.",
    );
  ...
  it("keeps the CUA skill instructions MCP-only", async () => {
    ...
    const contents = await Promise.all(
      files.map((file) =>
        readFile(`plugins/cua/skills/cua-driver/${file}`, "utf8"),
      ),
    );
  ...
  it("uses MCP-mode cache guidance in the CUA driver vendor source", async () => {
    ...
    for (const source of [clickTool, rightClickTool]) {
      ...
      expect(source).toContain(
        "Call get_window_state with the same pid and window_id",
      );
    }
  });
  ...
  it("uses conservative Feishu MCP defaults in the plugin manifest", async () => {
    const manifest = JSON.parse(
      await readFile("plugins/feishu/plugin.json", "utf8"),
    );
  ...
  it("declares a Feishu plugin skill for MCP tool routing", async () => {
    ...
    expect(skill).toContain(
      "Use the live tool names and descriptions in the current session",
    );
  ...
  it("wires CUA plugin packaging docs and release gates for both mac architectures", async () => {
    ...
    expect(packageJson.scripts["plugin:cua:package:mac:arm64"]).toContain(
      "--target-arch arm64",
    );
    expect(packageJson.scripts["plugin:cua:package:mac:x64"]).toContain(
      "--target-arch x64",
    );
    expect(packageJson.scripts["plugin:cua:build:mac:x64"]).toContain(
      "--arch x64",
    );
    expect(packageJson.scripts["plugin:cua:bundle:mac:arm64"]).toContain(
      "--target-arch arm64",
    );
    expect(packageJson.scripts["plugin:cua:bundle:mac:x64"]).toContain(
      "--target-arch x64",
    );
    expect(packageJson.scripts["build:mac:arm64"]).toContain(
      "plugin:cua:bundle:mac:arm64",
    );
    ...
    expect(buildWorkflow).toContain(
      "Contents/Resources/app.asar.unpacked/plugins",
    );
