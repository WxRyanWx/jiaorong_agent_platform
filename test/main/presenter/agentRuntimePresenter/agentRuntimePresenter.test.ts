...
function createMockSqlitePresenter() {
  ...
  const pendingInputsTable = {
    ...
    listClaimed: vi.fn(() =>
      pendingRows.filter((row) => row.state === "claimed"),
    ),
  ...
  return {
    ...
    deepchatSessionsTable: {
      ...
      updateSummaryState: vi.fn((_id: string, nextState: any) => {
        ...
        summaryState.summary_cursor_order_seq =
          nextState.summaryCursorOrderSeq ?? 1;
        summaryState.summary_updated_at = nextState.summaryUpdatedAt ?? null;
      }),
      updateSummaryStateIfMatches: vi.fn(
        (_id: string, nextState: any, expectedState: any) => {
          if (
            summaryState.summary_text !== (expectedState.summaryText ?? null) ||
            summaryState.summary_cursor_order_seq !==
              (expectedState.summaryCursorOrderSeq ?? 1) ||
            summaryState.summary_updated_at !==
    ...
    deepchatTapeEntriesTable: (deepchatTapeEntriesTable = {
      ...
      getLatestAnchor: vi.fn(
        (sessionId: string) =>
          tapeEntries
            .filter(
              (entry) =>
                entry.session_id === sessionId && entry.kind === "anchor",
            )
      ...
      countBySession: vi.fn(
        (sessionId: string) =>
          tapeEntries.filter((entry) => entry.session_id === sessionId).length,
      ),
      ...
      countEntriesAfter: vi.fn(
        (sessionId: string, entryId: number) =>
          tapeEntries.filter(
            (entry) =>
              entry.session_id === sessionId && entry.entry_id > entryId,
          ).length,
      ),
...
function createMockLlmProviderPresenter() {
  ...
  return {
    ...
    generateCompletionStandalone: vi
      .fn()
      .mockResolvedValue("English screenshot summary"),
...
function createMockConfigPresenter() {
  ...
  return {
    ...
    getThinkingBudgetRange: vi
      .fn()
      .mockReturnValue({ min: 0, max: 8192, default: 512 }),
...
function makeTextWithEstimatedTokens(minTokens: number): string {
  ...
  while (
    estimateMessagesTokens([{ role: "user", content: "x".repeat(high) }]) <
    minTokens
  ) {
    low = high + 1;
    high *= 2;
  ...
  while (low < high) {
    ...
    if (
      estimateMessagesTokens([{ role: "user", content: "x".repeat(mid) }]) <
      minTokens
    ) {
      low = mid + 1;
    } else {
...
describe("AgentRuntimePresenter", () => {
  ...
  beforeEach(() => {
    ...
    (processStream as ReturnType<typeof vi.fn>).mockResolvedValue({
      status: "completed",
    });
  ...
  function installSessionRows(initialRows: any[]) {
    ...
    sqlitePresenter.deepchatMessagesTable.getBySession.mockImplementation(
      (sessionId: string) => rows.filter((row) => row.session_id === sessionId),
    );
    ...
    sqlitePresenter.deepchatMessagesTable.getMaxOrderSeq.mockImplementation(
      (sessionId: string) =>
        rows.reduce(
          (maxOrderSeq, row) =>
            row.session_id === sessionId
              ? Math.max(maxOrderSeq, row.order_seq)
              : maxOrderSeq,
          0,
        ),
    );
  ...
  describe("constructor (crash recovery)", () => {
    it("calls pending status query on init", () => {
      expect(
        sqlitePresenter.deepchatMessagesTable.getByStatus,
    ...
    it("only recovers claimed pending inputs for sessions that still exist", () => {
      ...
      sqlitePresenter.deepchatSessionsTable.get.mockImplementation(
        (sessionId: string) => (sessionId === "s1" ? { id: "s1" } : null),
      );
      ...
      expect(
        sqlitePresenter.deepchatPendingInputsTable.update,
      ).toHaveBeenCalledTimes(1);
      expect(
        sqlitePresenter.deepchatPendingInputsTable.update,
      ).toHaveBeenCalledWith("pending-existing", {
        state: "pending",
        claimed_at: null,
      });
      expect(loggerInfoMock).toHaveBeenCalledWith(
  ...
  describe("processMessage", () => {
    it("creates user and assistant messages with correct order_seq", async () => {
      ...
      // User message insert
      const userInsert =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls[0][0];
      ...
      const assistantInsert =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls[1][0];
    ...
    it("rejects blank text-only messages before creating records", async () => {
      ...
      await expect(agent.processMessage("s1", "   ")).rejects.toThrow(
    ...
    it("queues steer during pre-stream setup and drains it as the next visible turn", async () => {
      ...
      const userInserts =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls
          .map(([row]) => row)
    ...
    it("queues active stream steer without aborting the current stream", async () => {
      ...
      expect(
        (processStream as ReturnType<typeof vi.fn>).mock.calls[0][0],
      ).toEqual(
        expect.objectContaining({
      ...
      expect(
        (
          processStream as ReturnType<typeof vi.fn>
        ).mock.calls[0][0].shouldYieldForPendingInput(),
      ).toBe(true);
      ...
      ).not.toHaveBeenCalledWith("mock-msg-id", expect.any(String), "error");
      const userInserts =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls
          .map(([row]) => row)
          .filter((row) => row.role === "user");
    ...
    it("rehydrates agentId from persisted new session rows before dispatching hooks", async () => {
      ...
      await agent.processMessage("s1", "Reopened session", {
        projectDir: "/tmp/project",
      });
    ...
    it("fails loudly when auto-grant is requested without a session permission port", async () => {
      ...
      await agentWithoutPermissionPort.initSession("s1", {
        providerId: "openai",
        modelId: "gpt-4",
      });
      await agentWithoutPermissionPort.processMessage("s1", "Hello");

      const params = (processStream as ReturnType<typeof vi.fn>).mock
        .calls[0]?.[0];
      await expect(
    ...
    it("includes conversation history in LLM call", async () => {
      ...
      const callArgs = (processStream as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
    ...
    it("compacts old turns into summary before building prompt", async () => {
      ...
      expect(
        sqlitePresenter.deepchatSessionsTable.updateSummaryState,
    ...
    it("uses session generation settings for context and model config", async () => {
      ...
      expect(
        callArgs.messages[0].content.trim().startsWith("Custom system prompt"),
      ).toBe(true);
    ...
    it("emits and clears an ephemeral rate-limit message while waiting for the provider gate", async () => {
      llmProvider.executeWithRateLimit.mockImplementation(
        async (
          _providerId: string,
          options?: { onQueued?: (snapshot: any) => void },
        ) => {
          options?.onQueued?.({
            providerId: "openai",
            qpsLimit: 1,
      ...
      const typedStreamUpdates = (
        publishDeepchatEvent as ReturnType<typeof vi.fn>
      ).mock.calls
    ...
    it("does not call provider.coreStream when a queued request is canceled", async () => {
      ...
      const providerCoreStream =
        llmProvider.getProviderInstance.mock.results[0]?.value.coreStream;
    ...
    it("reuses cached system prompt within the same day", async () => {
      ...
      const firstCallArgs = (processStream as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      const secondCallArgs = (processStream as ReturnType<typeof vi.fn>).mock
        .calls[1][0];
      expect(firstCallArgs.messages[0].content).toBe(
        secondCallArgs.messages[0].content,
      );
    });
    ...
    it("invalidates cached prompt after system prompt update", async () => {
      ...
      await agent.updateGenerationSettings("s1", {
        systemPrompt: "Updated user prompt",
      });
    ...
    it("uses persisted project directory when runtime state was restored from DB", async () => {
      ...
      const callArgs = (processStream as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      expect(callArgs.messages[0].content).toContain(
        "WORKDIR:/tmp/restored-workspace",
      );
    ...
    it("invalidates cached prompt when pinned skills change", async () => {
      ...
      skillPresenter.loadSkillContent.mockResolvedValue({
    ...
    it("derives runtime capabilities from the current enabled agent tools", async () => {
      const runtimeBuilder = buildRuntimeCapabilitiesPrompt as ReturnType<
        typeof vi.fn
      >;
      toolPresenter.getAllToolDefinitions.mockResolvedValueOnce([
        {
    ...
    it("transitions status: idle → generating → idle", async () => {
      ...
      const statusCalls = (
        eventBus.sendToRenderer as ReturnType<typeof vi.fn>
      ).mock.calls.filter((c: any[]) => c[0] === "session:status-changed");
      expect(statusCalls).toHaveLength(2);
      expect(statusCalls[0][2]).toEqual({
        sessionId: "s1",
        status: "generating",
      });
    ...
    it("finalizes the assistant placeholder when streaming setup fails", async () => {
      ...
      const [messageId, contentJson, status] =
        sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock
          .calls[0];
    ...
    it("skips JiaorongAI runtime prompt layers and local tools for ACP-backed subagent sessions", async () => {
      sqlitePresenter.newSessionsTable.get.mockReturnValue({
        id: "s-acp-subagent",
    ...
    it("passes abort signals into next-turn compaction execution", async () => {
      ...
      ).mockResolvedValue(compactionIntent);
    ...
    it("injects request trace context when trace debug is enabled", async () => {
      ...
      expect(
        sqlitePresenter.deepchatMessageTracesTable.insert,
      ).toHaveBeenCalledTimes(1);
      const inserted =
        sqlitePresenter.deepchatMessageTracesTable.insert.mock.calls[0][0];
      const headers = JSON.parse(inserted.headersJson) as Record<
        string,
        string
      >;
  ...
  describe("generation settings", () => {
    ...
    it("keeps image generation settings for OpenAI-compatible providers", async () => {
      configPresenter.getModelConfig.mockImplementation(
    ...
    it("inherits interleaved thinking defaults and allows explicit session disable", async () => {
      ...
      const interleavedConfig = (
        agent as any
      ).resolveInterleavedReasoningConfig("openai", "gpt-4", disabled);
      ...
      const deepseekDisabledConfig = (
        agent as any
      ).resolveInterleavedReasoningConfig("openai", "deepseek-v4", disabled);
      ...
      const deepseekInterleavedConfig = (
        agent as any
      ).resolveInterleavedReasoningConfig("deepseek", "deepseek-v4", defaults);
      expect(deepseekInterleavedConfig.preserveReasoningContent).toBe(true);
      expect(deepseekInterleavedConfig.preserveEmptyReasoningContent).toBe(
        true,
      );

      const nonDeepseekInterleavedConfig = (
        agent as any
      ).resolveInterleavedReasoningConfig("openai", "gpt-4", defaults);
      expect(nonDeepseekInterleavedConfig.preserveReasoningContent).toBe(true);
      expect(nonDeepseekInterleavedConfig.preserveEmptyReasoningContent).toBe(
        false,
      );
    ...
    it("drops new-api anthropic adaptive reasoning overrides when backend reasoning is disabled", async () => {
      configPresenter.getCapabilityProviderId.mockImplementation(
        (providerId: string, modelId: string) =>
          providerId === "new-api" && modelId === "claude-opus-4-7"
            ? "anthropic"
            : providerId,
      );
      configPresenter.getModelConfig.mockImplementation(
    ...
    it("keeps system prompt and resets other settings to the new model defaults", async () => {
      ...
      expect(
        sqlitePresenter.deepchatSessionsTable.updateSessionModel,
      ).toHaveBeenCalledWith("s1", "anthropic", "claude-3-5-sonnet");
    ...
    it("rejects model switching while the session is generating", async () => {
      await agent.initSession("s1", { providerId: "openai", modelId: "gpt-4" });
      ((agent as any).runtimeState as Map<string, DeepChatSessionState>).set(
        "s1",
        {
          status: "generating",
          providerId: "openai",
      ...
      await expect(
        agent.setSessionModel("s1", "anthropic", "claude-3-5-sonnet"),
      ).rejects.toThrow("Cannot switch model while session is generating.");
      expect(
        sqlitePresenter.deepchatSessionsTable.updateSessionModel,
      ).not.toHaveBeenCalled();
    });
  ...
  describe("destroySession", () => {
    it("cleans up messages, session record, and runtime state", async () => {
      ...
      expect(
        sqlitePresenter.deepchatMessagesTable.deleteBySession,
      ).toHaveBeenCalledWith("s1");
      expect(sqlitePresenter.deepchatSessionsTable.delete).toHaveBeenCalledWith(
        "s1",
      );
  ...
  describe("cancelGeneration", () => {
    ...
    it("cancels generation only when the event id matches the active assistant message", async () => {
      ...
      await expect(
        agent.cancelGenerationByEventId("s1", "msg-other"),
      ).resolves.toBe(false);
      await expect(
        agent.cancelGenerationByEventId("s1", "msg-active"),
      ).resolves.toBe(true);
  ...
  describe("queuePendingInput", () => {
    it("claims immediately runnable turns instead of exposing a queued item first", async () => {
      ...
      expect(queueSpy).toHaveBeenCalledWith("s1", "Hello", {
        state: "claimed",
      });
    ...
    it("keeps queue-origin inputs pending while waiting for a tool follow-up", async () => {
      ...
      const processSpy = vi.spyOn(agent, "processMessage").mockResolvedValue();
      vi.spyOn(agent as any, "isAwaitingToolQuestionFollowUp").mockReturnValue(
        true,
      );
      vi.spyOn(
        agent as any,
      ...
      const result = await agent.queuePendingInput("s1", "Queued later", {
        source: "queue",
      });

      expect(queueSpy).toHaveBeenCalledWith("s1", "Queued later", {
        state: "pending",
      });
    ...
    it("pauses automatic queue draining when a queued turn is stopped", async () => {
      await agent.initSession("s1", { providerId: "openai", modelId: "gpt-4" });
      (agent as any).pendingInputCoordinator.queuePendingInput(
        "s1",
        "Queued retry",
      );
      ...
      const drainPromise = (agent as any).drainPendingQueueIfPossible(
        "s1",
        "resume",
      );
      await streamStarted;
  ...
  describe("summary invalidation", () => {
    it("resets summary when deleting history before cursor", async () => {
      ...
      expect(
        sqlitePresenter.deepchatSessionsTable.resetSummaryState,
      ).toHaveBeenCalledWith("s1");
  ...
  describe("session compaction state", () => {
    ...
    it("bypasses JiaorongAI context preflight for oversized ACP provider calls", async () => {
      await agent.initSession("s1", {
        providerId: "acp",
      ...
      const requestMessages = [
        { role: "user" as const, content: oversizedPrompt },
      ];
      ...
      expect(
        JSON.stringify(
          (eventBus.sendToRenderer as ReturnType<typeof vi.fn>).mock.calls,
        ),
    ...
    it("does not start JiaorongAI context-pressure compaction for ACP turns", async () => {
      const historyRows = createSentTurnRecords(3);
      installSessionRows(historyRows);
      const prepareSpy = vi.spyOn(
        (
          agent as unknown as {
            compactionService: { prepareForNextUserTurn: () => unknown };
          }
        ).compactionService,
        "prepareForNextUserTurn",
      );
    ...
    it("preflights provider calls with a safety margin and compacts before low-output pressure calls", async () => {
      ...
      expect(totalRequestTokens).toBeLessThanOrEqual(
    ...
    it("trims provider request history without deleting stored messages when compaction is disabled", async () => {
      ...
      expect(providerMessages).not.toContainEqual({
        role: "user",
        content: oldHistoryText,
      });
      expect(requestMessages).not.toContainEqual({
        role: "user",
        content: oldHistoryText,
      });
      expect(
        sqlitePresenter.deepchatMessagesTable.delete,
      ).not.toHaveBeenCalled();
      expect(totalRequestTokens).toBeLessThanOrEqual(
        getUsableContextLength(8192),
      );
    ...
    it("emits compacting before compacted on successful compaction", async () => {
      ...
      const insertRows =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls.map(
          ([row]: any[]) => row,
        );
      ...
      const compactionInsert =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls.find(
          ([row]: any[]) =>
            typeof row?.metadata === "string" &&
            row.metadata.includes('"messageType":"compaction"'),
        )?.[0];
      ...
      const finalizedCompaction =
        sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls.find(
          ([, , , metadata]: any[]) =>
            ...
            metadata.includes('"messageType":"compaction"'),
        );
    ...
    it("marks the session generating before manual compaction preparation awaits", async () => {
      ...
      const getGenerationSettings = (
        agent as any
      ).getEffectiveSessionGenerationSettings.bind(agent);
      const generationSettingsSpy = vi
        .spyOn(agent as any, "getEffectiveSessionGenerationSettings")
        .mockImplementation(async (sessionId: string) => {
          expect((agent as any).runtimeState.get(sessionId).status).toBe(
            "generating",
          );
    ...
    it("returns persisted compacted state for reopened sessions", async () => {
      ...
      const compactionState =
        await reopenedAgent.getSessionCompactionState("s1");
  ...
  describe("retry context overflow recovery", () => {
    ...
    it("fails retry with budget guidance when the current input cannot fit", async () => {
      ...
      const providerCoreStream =
        llmProvider.getProviderInstance.mock.results[0].value.coreStream;
      const errorUpdate =
        sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls
  ...
  describe("respondToolInteraction", () => {
    const makeAssistantRow = (overrides?: {
      ...
      sqlitePresenter.deepchatMessagesTable.get.mockImplementation(
        (id: string) => (id === row.id ? row : undefined),
      );
    ...
    it("handles question_option and resumes assistant message", async () => {
      ...
      ).toHaveBeenCalledWith("m1", expect.any(String));
    ...
    it("inserts resume compaction indicators before the assistant message being resumed", async () => {
      ...
      expect(result).toEqual({ resumed: true });
      expect(
        sqlitePresenter.deepchatMessagesTable.incrementOrderSeqFrom,
      ).toHaveBeenCalledWith("s1", 3);
      const compactionInsert =
        sqlitePresenter.deepchatMessagesTable.insert.mock.calls.find(
          ([row]: any[]) =>
            typeof row?.metadata === "string" &&
    ...
    it("treats an aborted resume signal as cancellation even for non-abort errors", async () => {
      ...
      vi.spyOn(agent as any, "runStreamForMessage").mockImplementation(
        async () => {
      ...
      const resumed = await (agent as any).resumeAssistantMessage(
        "s1",
        "m1",
        [],
      );
      ...
      const [messageId, contentJson, status] =
        sqlitePresenter.deepchatMessagesTable.updateContentAndStatus.mock.calls.at(
          -1,
        );
    ...
    it("views a skill draft inline and keeps the confirmation pending", async () => {
      ...
      expect(skillPresenter.viewDraftSkill).toHaveBeenCalledWith(
        "s1",
        "draft-1",
      );
      expect(processStream).not.toHaveBeenCalled();
      expect(
        sqlitePresenter.deepchatMessagesTable.updateStatus,
      ).toHaveBeenCalledWith("m1", "pending");
      ...
      expect(updatedBlocks[1].extra.skillDraftPreview).toContain(
        "# Draft body",
      );
      expect(
        updatedBlocks[1].extra.questionOptions.map(
    ...
    it("installs a skill draft and resumes assistant message", async () => {
      ...
      expect(result).toEqual({ resumed: true });
      expect(skillPresenter.installDraftSkill).toHaveBeenCalledWith(
        "s1",
        "draft-1",
      );
      expect(processStream).toHaveBeenCalledTimes(1);
      ...
      expect(updatedBlocks[1].extra.answerText).toBe(
        "chat.skillDraft.actions.install",
      );
    ...
    it("discards a skill draft and resumes assistant message", async () => {
      ...
      expect(result).toEqual({ resumed: true });
      expect(skillPresenter.discardDraftSkill).toHaveBeenCalledWith(
        "s1",
        "draft-1",
      );
      const updatedBlocks = JSON.parse(
    ...
    it("normalizes deferred screenshot tool results before resume", async () => {
      tempHome = await fs.mkdtemp(
        path.join(os.tmpdir(), "deepchat-deferred-offload-"),
      );
      ...
      expect(updatedBlocks[0].tool_call.response).toBe(
        "English screenshot summary",
      );
      expect(updatedBlocks[0].tool_call.response).not.toContain(
    ...
    it("cleans deferred offload files when resume budget downgrades the tool result", async () => {
      tempHome = await fs.mkdtemp(
        path.join(os.tmpdir(), "deepchat-deferred-cleanup-"),
      );
      getPathSpy = vi.spyOn(app, "getPath").mockReturnValue(tempHome);
      ...
      const hasContextBudgetSpy = vi.spyOn(
        (agent as any).toolOutputGuard,
        "hasContextBudget",
      );
      ...
      try {
        ...
        expect(result).toEqual({ resumed: true });
        const updateCalls =
          sqlitePresenter.deepchatMessagesTable.updateContent.mock.calls;
        const updatedBlocks = JSON.parse(
    ...
    it("handles ACP permission grant through live provider resolver without deferred tool resume", async () => {
      ...
      makeAssistantRow({
        blocks: [
          ...
          },
          {
            ...
            timestamp: 2,
            content:
              "components.messageBlockPermissionRequest.description.command",
            tool_call: {
              id: "tc1",
              name: "Terminal",
              params: '{"command":"dir"}',
      ...
      expect(result).toEqual({ resumed: false });
      expect(llmProvider.resolveAgentPermission).toHaveBeenCalledWith(
        "acp-req-1",
        true,
      );
      ...
      expect(updatedBlocks[1].status).toBe("granted");
      expect(updatedBlocks[1].extra.needsUserAction).toBe(false);
      expect((agent as any).activeProviderPermissions.has("acp-req-1")).toBe(
        false,
      );
    });
  ...
  describe("permission mode", () => {
    it("setPermissionMode updates runtime and db", async () => {
      ...
      ).toHaveBeenCalledWith("s1", "default");
    });
  ...
  describe("disabled tools", () => {
    ...
    it("prefers the current session model for screenshot analysis during deferred execution", async () => {
      ...
      const result = await (agent as any).executeDeferredToolCall("s1", "m1", {
        ...
        name: "cdp_send",
        params:
          '{"method":"Page.captureScreenshot","params":{"format":"jpeg"}}',
      });
    ...
    it("registers a cancellable controller for deferred subagent tool calls", async () => {
      ...
      toolPresenter.callTool.mockImplementationOnce(
      ...
      const executionPromise = (agent as any).executeDeferredToolCall(
        "s1",
        "m1",
        {
          id: "tc-subagent",
    ...
    it("re-reads the latest message content before persisting subagent progress", async () => {
      ...
      sqlitePresenter.deepchatMessagesTable.get
        .mockImplementationOnce((id: string) =>
          id === "m1" ? staleRow : undefined,
        )
        .mockImplementationOnce((id: string) =>
          id === "m1" ? latestRow : undefined,
        );
    ...
    it("falls back to the current session agent vision model when the current model has no vision", async () => {
      ...
      expect(configPresenter.resolveDeepChatAgentConfig).toHaveBeenCalledWith(
        "persisted-agent",
      );
    ...
    it("returns a cancellation message when screenshot normalization is aborted", async () => {
      ...
      expect(normalized).toBe(
        "Screenshot captured, but automatic English analysis was canceled.",
      );
    });
