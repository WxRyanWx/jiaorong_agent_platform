...
describe("RemoteCommandRouter", () => {
  ...
  it("reports runtime state for /status", async () => {
    ...
    expect(result.replies[0]).toContain(
      "Current session: Remote chat [session-1]",
    );
  ...
  it("routes plain text to a pending permission response before opening a new turn", async () => {
    ...
    expect(runner.respondToPendingInteraction).toHaveBeenCalledWith(
      "telegram:100:0",
      {
        kind: "permission",
        granted: true,
  ...
  it("re-sends the current pending interaction with buttons", async () => {
    ...
    expect(bindingStore.createPendingInteractionState).toHaveBeenCalledWith(
      "telegram:100:0",
      {
        type: "question",
        messageId: "assistant-2",
      ...
      expect.objectContaining({
        ...
        replyMarkup: {
          inline_keyboard: expect.arrayContaining([
            [
              expect.objectContaining({
                text: expect.stringContaining("JiaorongAI"),
              }),
            ],
          ]),
        },
  ...
  it("switches the channel default agent from a callback query", async () => {
    ...
    expect(setChannelDefaultAgent).toHaveBeenCalledWith(
      "telegram:100:0",
      "claude-code",
    );
    expect(bindingStore.clearAgentMenuState).toHaveBeenCalledWith(
      "agent-token",
    );
  ...
  it("surfaces agent switch errors via callback alert", async () => {
    ...
    const runner = createRunner({
      ...
      setChannelDefaultAgent: vi
        ...
        .mockRejectedValue(
          new Error(
            "Cannot switch to ACP agent: this channel has no default workdir set.",
          ),
        ),
    });
    const router = new RemoteCommandRouter({
