...
const createHarness = async (options?: {
  ...
  const client = {
    ...
    startMessageStream: vi
      .fn()
      .mockImplementation(
        async (params: { onMessage: (event: unknown) => Promise<void> }) => {
    ...
    sendText: vi
      .fn()
      .mockImplementation(async () => `om_bot_${nextMessageId++}`),
...
describe("FeishuRuntime", () => {
  it("marks attachments as failed when message resource download fails", async () => {
    ...
    harness.client.downloadMessageResource.mockRejectedValue(
  ...
  it("streams answer text beside a persistent trace log", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        ...
        expect(
          harness.bindingStore.rememberRemoteDeliveryState,
        ...
        expect(harness.client.updateMarkdown).toHaveBeenCalledWith(
          "om_bot_2",
          "Final answer",
        );
        expect(
          harness.bindingStore.clearRemoteDeliveryState,
        ).toHaveBeenCalledWith("feishu:oc_1:root");
      });
  ...
  it("preserves null messageId holes so later updates do not target the wrong chunk", async () => {
    ...
    try {
      ...
      const initialText =
        firstChunk +
        "B".repeat(FEISHU_OUTBOUND_TEXT_LIMIT) +
        "C".repeat(FEISHU_OUTBOUND_TEXT_LIMIT);
      const updatedText =
        firstChunk +
        changedMiddleChunk +
        "C".repeat(FEISHU_OUTBOUND_TEXT_LIMIT);
      ...
      harness.client.sendMarkdown.mockImplementation(async () =>
        sendResults.length > 0
          ? (sendResults.shift() as string | null)
          : "om_bot_4",
      );
      harness.router.handleMessage.mockResolvedValue({
        replies: [],
      ...
      expect(
        harness.bindingStore.rememberRemoteDeliveryState.mock.calls[0],
      ).toEqual([
        "feishu:oc_1:root",
        {
          sourceMessageId: "msg-1",
  ...
  it("appends later process and answer segments in JiaorongAI order instead of rewriting the first answer", async () => {
    vi.useFakeTimers();
  ...
  it("serializes messages per endpoint while allowing different endpoints in parallel", async () => {
    ...
    harness.router.handleMessage.mockImplementation(
  ...
  it("falls back to text when sending a Feishu card fails", async () => {
    const harness = await createHarness();
    harness.client.sendCard.mockRejectedValueOnce(
