...
describe("RemoteControlPresenter", () => {
  ...
  it("auto-disables remote control after a fatal poller failure", async () => {
    ...
    pollerInstances[0].deps.onFatalError?.(
  ...
  it("removes authorized principals through the generic presenter contract", async () => {
    ...
    await expect(
      presenter.getChannelPairingSnapshot("feishu"),
    ).resolves.toEqual({
      pairCode: null,
      pairCodeExpiresAt: null,
  ...
  it("stores a wechat ilink account after qr login completes", async () => {
    ...
    const startLoginSpy = vi
      .spyOn(WeixinIlinkClient, "startLogin")
    ...
    const waitLoginSpy = vi
      .spyOn(WeixinIlinkClient, "waitForLogin")
      .mockResolvedValueOnce({
        connected: true,
    ...
    expect(
      vi.mocked(BrowserWindow).mock.results[0]?.value.loadURL,
    ).toHaveBeenCalledWith("https://liteapp.weixin.qq.com/mock-login");
