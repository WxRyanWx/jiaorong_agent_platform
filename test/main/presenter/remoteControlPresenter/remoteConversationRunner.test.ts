...
const createConfigPresenter = (overrides: Record<string, unknown> = {}) => ({
  getAgentType: vi.fn(async (agentId: string) =>
    agentId === "acp-agent" ? "acp" : "deepchat",
  ),
...
describe("RemoteConversationRunner", () => {
  ...
  it("creates new sessions with the current default deepchat agent", async () => {
    ...
    const session = await runner.createNewSession(
      "telegram:100:0",
      "Remote Session",
    );

    expect(session.agentId).toBe("deepchat-alt");
    expect(bindingStore.setBinding).toHaveBeenCalledWith(
      "telegram:100:0",
      session.id,
    );
  });
  ...
  it("downloads inbound remote files into the session workspace before sending", async () => {
    const workspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "deepchat-remote-runner-"),
    );
    const preparedFile = {
      name: "note.txt",
      path: path.join(
        workspace,
        ".deepchat/remote-assets/telegram/hash/message/note.txt",
      ),
      mimeType: "text/plain",
      content: "hello file",
    };
    ...
    expect(preparedPath).toContain(
      path.join(".deepchat", "remote-assets", "telegram"),
    );
  ...
  it("stores same-named remote attachments at unique local paths", async () => {
    ...
    expect(sentInput.files.map((file) => file.name)).toEqual([
      "duplicate.txt",
      "duplicate.txt",
    ]);
    expect(sentInput.files.map((file) => file.path)).toEqual([
      firstPath,
      secondPath,
    ]);
  ...
  it("skips remote attachments that have no downloadable data", async () => {
    ...
    try {
      ...
      expect(path.basename(preparedPath)).toBe("note-1.txt");
      await expect(fs.readFile(preparedPath, "utf8")).resolves.toBe(
        "hello file",
      );
      expect(agentSessionPresenter.sendMessage).toHaveBeenCalledWith(
        "session-bound",
  ...
  it("downloads and decrypts inbound encrypted remote media before sending", async () => {
    ...
    const preparedFile = {
      ...
      path: path.join(
        workspace,
        ".deepchat/remote-assets/weixin-ilink/hash/message/image-1.png",
      ),
      mimeType: "image/png",
      content: "",
    };
  ...
  it("persists generated remote images from cached and base64 sources", async () => {
    const workspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "deepchat-remote-images-"),
    );
    const userData = await fs.mkdtemp(
      path.join(os.tmpdir(), "deepchat-user-data-"),
    );
    ...
    const execution = await runner.sendText(
      "weixin-ilink:account:user",
      "draw a sunset",
    );
    ...
    await expect(
      fs.readFile(snapshot.generatedImages![0].path),
    ).resolves.toEqual(cachedImage);
    await expect(
      fs.readFile(snapshot.generatedImages![1].path),
    ).resolves.toEqual(dataUrlImage);
    await expect(
      fs.readFile(snapshot.generatedImages![2].path),
    ).resolves.toEqual(rawBase64Image);
    await expect(
      fs.readFile(snapshot.generatedImages![3].path),
    ).resolves.toEqual(toolOutputImage);
    await expect(
      fs.readFile(snapshot.generatedImages![4].path),
    ).resolves.toEqual(screenshotImage);
  ...
  it("lists recent sessions for the currently bound agent before falling back to default agent", async () => {
    ...
    expect(bindingStore.rememberSessionSnapshot).toHaveBeenCalledWith(
      "telegram:100:0",
      ["session-b", "session-a"],
    );
  });
  ...
  it("delegates remote model switching to the bound session", async () => {
    ...
    const updated = await runner.setSessionModel(
      "telegram:100:0",
      "anthropic",
      "claude-3-5-sonnet",
    );
  ...
  it("does not fall back to the previous active assistant event while waiting for a new reply", async () => {
    ...
    expect(bindingStore.rememberActiveEvent).not.toHaveBeenCalledWith(
      "telegram:100:0",
      "msg-old",
    );
  ...
  it("extracts the latest pending interaction from assistant action blocks", async () => {
    ...
    await expect(
      runner.getPendingInteraction("telegram:100:0"),
    ).resolves.toEqual({
      type: "permission",
  ...
  it("keeps stream text empty while the assistant is still reasoning", async () => {
    ...
    const snapshot = await (runner as any).getConversationSnapshot(
      "telegram:100:0",
      "session-1",
      {
        afterOrderSeq: 0,
  ...
  it("creates a follow-up execution after responding to a pending interaction", async () => {
    ...
    const response = await runner.respondToPendingInteraction(
      "telegram:100:0",
      {
        kind: "permission",
        granted: true,
  ...
  it("creates ACP sessions with provider, model, and the global default workdir", async () => {
    ...
    const runner = new RemoteConversationRunner(
      ...
      {
        getTelegramDefaultWorkdir: vi
          .fn()
          .mockReturnValue("/workspaces/remote"),
        setBinding: vi.fn(),
      } as any,
    );
  ...
  it("requires a channel default workdir for ACP workdir resolution", async () => {
    ...
    await expect(
      runner.getDefaultWorkdir("telegram:100:0"),
    ).resolves.toBeNull();
  });
  ...
  it("prefers the discord channel default workdir for ACP sessions", async () => {
    const runner = new RemoteConversationRunner(
      ...
      {
        getDiscordDefaultWorkdir: vi
          .fn()
          .mockReturnValue("/workspaces/discord"),
      } as any,
    );

    await expect(runner.getDefaultWorkdir("discord:dm:123")).resolves.toBe(
      "/workspaces/discord",
    );
  });
  ...
  it("switches the channel default agent and starts a new session", async () => {
    ...
    const createDetachedSession = vi
      .fn()
      .mockResolvedValue(
        createSession({ id: "session-new", agentId: "codex" }),
      );
    const configPresenter = createConfigPresenter({
      listAgents: vi.fn().mockResolvedValue([
    ...
    const result = await runner.setChannelDefaultAgent(
      "telegram:100:0",
      "codex",
    );

    expect(setChannelDefaultAgentId).toHaveBeenCalledWith(
      "telegram:100:0",
      "codex",
    );
  ...
  it("rejects an unknown agent id", async () => {
    ...
    await expect(
      runner.setChannelDefaultAgent("telegram:100:0", "missing"),
    ).rejects.toThrow('Agent "missing" is not available');
  });
  ...
  it("rejects an ACP agent switch when the channel has no default workdir", async () => {
    ...
    await expect(
      runner.setChannelDefaultAgent("telegram:100:0", "codex"),
    ).rejects.toThrow(/no default workdir/);
