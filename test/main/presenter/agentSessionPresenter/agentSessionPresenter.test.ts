  ...
  const db = {
    prepare: vi.fn((sql: string) => ({
      all: vi.fn((...args: unknown[]) => {
        ...
        throw new Error(
          `Unexpected SQL in test: ${sql} with args ${JSON.stringify(args)}`,
        );
      }),
    })),
  };
...
describe("AgentSessionPresenter", () => {
  ...
  describe("createSession", () => {
    ...
    it("derives title from first 50 chars of message", async () => {
      ...
      const result = await presenter.createSession(
        { agentId: "deepchat", message: longMessage },
        1,
      );
    ...
    it("uses the JiaorongAI agent default directory when createSession does not provide one", async () => {
    ...
    it("falls back to the global default directory when the JiaorongAI agent has none", async () => {
      configPresenter.resolveDeepChatAgentConfig.mockResolvedValue({});
      configPresenter.getDefaultProjectPath.mockReturnValue(
    ...
    it("applies active skills before first message processing", async () => {
      ...
      expect(skillPresenter.setActiveSkills).toHaveBeenCalledWith(
        "mock-session-id",
        ["skill-a", "skill-b"],
      );
    });
    ...
    it("generates title asynchronously without blocking createSession", async () => {
      ...
      sqlitePresenter.newSessionsTable.update.mockImplementation(
      ...
      await presenter.createSession(
        { agentId: "deepchat", message: "Please summarize" },
        1,
      );
      await new Promise((r) => setTimeout(r, 20));

      expect(llmProviderPresenter.summaryTitles).toHaveBeenCalled();
      expect(sqlitePresenter.newSessionsTable.update).toHaveBeenCalledWith(
        "mock-session-id",
        {
    ...
    it("aborts ACP session creation when workdir sync fails", async () => {
      ...
      llmProviderPresenter.setAcpWorkdir.mockRejectedValueOnce(
      ...
      expect(deepChatAgent.destroySession).toHaveBeenCalledWith(
        "mock-session-id",
      );
      expect(sqlitePresenter.newSessionsTable.delete).toHaveBeenCalledWith(
        "mock-session-id",
      );
      expect(deepChatAgent.processMessage).not.toHaveBeenCalled();
    });
  ...
  describe("setSessionProjectDir", () => {
    it("syncs workspace changes into the active agent runtime", async () => {
      ...
      expect(deepChatAgent.setSessionProjectDir).toHaveBeenCalledWith(
        "s1",
        "/tmp/workspace",
      );
      expect(
        sqlitePresenter.newEnvironmentsTable.syncPath,
      ).toHaveBeenCalledWith("/tmp/workspace");
    });
  });
  ...
  describe("createSubagentSession", () => {
    it("routes ACP target subagents to the native ACP provider without inheriting parent tooling state", async () => {
      ...
      configPresenter.getAgentType.mockImplementation(
      ...
      sqlitePresenter.newSessionsTable.delete.mockImplementation(
        (id: string) => {
    ...
    it("retries subagent initialization exactly once when ACP setup fails before the child starts", async () => {
      const nanoidMock = nanoid as unknown as ReturnType<typeof vi.fn>;
      nanoidMock
        .mockReturnValueOnce("child-session-1")
        .mockReturnValueOnce("child-session-2");
      ...
      expect(llmProviderPresenter.clearAcpSession).toHaveBeenCalledWith(
        "child-session-1",
      );
    ...
    it("refreshes the session list only after a child is fully materialized", async () => {
      ...
      (eventBus.sendToRenderer as ReturnType<typeof vi.fn>).mockImplementation(
        (event: string) => {
  ...
  describe("getSessionList", () => {
    ...
    it("skips sessions whose state loading fails", async () => {
      ...
      deepChatAgent.getSessionListState.mockImplementation(
  ...
  describe("getSessionCompactionState", () => {
    it("delegates to the agent implementation", async () => {
      ...
      expect(deepChatAgent.getSessionCompactionState).toHaveBeenCalledWith(
        "s1",
      );
  ...
  describe("message traces", () => {
    ...
    it("returns empty list for blank message id", async () => {
      ...
      expect(traces).toEqual([]);
      expect(
        sqlitePresenter.deepchatMessageTracesTable.listByMessageId,
      ).not.toHaveBeenCalled();
    });

    it("returns trace count by message id", async () => {
      sqlitePresenter.deepchatMessageTracesTable.countByMessageId.mockReturnValue(
        3,
      );
      await expect(presenter.getMessageTraceCount("m1")).resolves.toBe(3);
      expect(
        sqlitePresenter.deepchatMessageTracesTable.countByMessageId,
  ...
  describe("generation settings", () => {
    ...
    it("delegates updateSessionGenerationSettings to agent", async () => {
      ...
      expect(deepChatAgent.updateGenerationSettings).toHaveBeenCalledWith(
        "s1",
        {
          temperature: 1.4,
          reasoningEffort: "high",
    ...
    it("throws when generation settings methods target unknown session", async () => {
      ...
      await expect(
        presenter.getSessionGenerationSettings("unknown"),
      ).rejects.toThrow("Session not found: unknown");
      await expect(
        presenter.updateSessionGenerationSettings("unknown", {
          temperature: 1,
        }),
      ).rejects.toThrow("Session not found: unknown");
    });
  ...
  describe("disabled agent tools", () => {
    ...
    it("updates disabled agent tools and invalidates the deepchat prompt cache", async () => {
      ...
      const disabledTools = await presenter.updateSessionDisabledAgentTools(
        "s1",
        ["grep", "ls", "cdp_send", "exec", "exec"],
      );

      expect(disabledTools).toEqual(["cdp_send", "exec", "grep"]);
      expect(
        sqlitePresenter.newSessionsTable.updateDisabledAgentTools,
      ).toHaveBeenCalledWith("s1", ["cdp_send", "exec", "grep"]);
      expect(
        deepChatAgent.invalidateSessionSystemPromptCache,
    ...
    it("cleans legacy persisted grep without blocking new grep updates", async () => {
      ...
      configPresenter.getDeepChatAgentConfig.mockImplementation(
        async (agentId: string) => {
      ...
      expect(
        sqlitePresenter.newSessionsTable.updateDisabledAgentTools,
      ).toHaveBeenCalledWith("session-1", ["cdp_send", "exec"]);
      ...
      expect(
        sqlitePresenter.configTables.setAgentSetting,
  ...
  describe("setSessionSubagentEnabled", () => {
    it("throws when the updated session state cannot be rebuilt", async () => {
      ...
      deepChatAgent.getSessionState.mockRejectedValueOnce(
        new Error("state unavailable"),
      );

      await expect(
        presenter.setSessionSubagentEnabled("s1", true),
      ).rejects.toThrow("Failed to build session state for sessionId: s1");
  ...
  describe("setSessionModel", () => {
    it("updates deepchat session model and emits LIST_UPDATED", async () => {
      ...
      const updated = await presenter.setSessionModel(
        "s1",
        "anthropic",
        "claude-3-5-sonnet",
      );
    ...
    it("rejects ACP session model switching", async () => {
      ...
      await expect(
        presenter.setSessionModel("s-acp", "openai", "gpt-4"),
      ).rejects.toThrow("ACP session model is locked.");
  ...
  describe("agent session transfer", () => {
    it("reports movable sessions and empty drafts before deleting an agent", async () => {
      ...
      sqlitePresenter.newSessionsTable.list.mockImplementation(
        (filters: any) => (filters?.parentSessionId ? [] : rows),
      );
      configPresenter.getAgentType.mockResolvedValue("deepchat");
      deepChatAgent.getMessageIds.mockImplementation(
        async (sessionId: string) => (sessionId === "s-ready" ? ["m1"] : []),
      );
    ...
    it("rejects blank agent ids for destructive agent-session deletion", async () => {
      await expect(presenter.deleteAgentSessions("   ")).rejects.toThrow(
    ...
    it("moves a completed conversation to the target agent context", async () => {
      ...
      const updated = await presenter.moveSessionToAgent(
        "s1",
        "deepchat-coder",
      );
      ...
      expect(
        sqlitePresenter.newSessionsTable.updateAgentId,
      ).toHaveBeenCalledWith("s1", "deepchat-coder");
      expect(
        sqlitePresenter.newSessionsTable.updateDisabledAgentTools,
      ).toHaveBeenCalledWith("s1", ["agent_filesystem_read_file"]);
    ...
    it("moves an ACP conversation to a JiaorongAI agent and clears the ACP binding", async () => {
      const row = {
        id: "s-acp",
        agent_id: "acp-coder",
        title: "ACP session",
      ...
      expect(
        llmProviderPresenter.clearAcpSession.mock.invocationCallOrder[0],
      ).toBeGreaterThan(
      ...
      expect(
        ...
        sqlitePresenter.newSessionsTable.updateAgentId.mock
    ...
    it("keeps the ACP binding when target ownership update fails", async () => {
      ...
      await expect(
        presenter.moveSessionToAgent("s-acp", "deepchat-coder"),
    ...
    it("rejects moving a JiaorongAI conversation to an ACP target", async () => {
      const row = {
        id: "s-deepchat",
        agent_id: "deepchat-writer",
        title: "JiaorongAI session",
        project_dir: "/repo",
        is_pinned: 0,
        is_draft: 0,
        session_kind: "regular",
      ...
      await expect(
        presenter.moveSessionToAgent("s-deepchat", "acp-coder"),
    ...
    it("rejects ACP targets for batch agent transfers before mutating sessions", async () => {
      ...
      await expect(
        presenter.moveAgentSessions("deepchat-writer", "acp-coder"),
      ).rejects.toThrow("Conversation history cannot be moved to ACP agents.");
      expect(
        sqlitePresenter.newSessionsTable.updateAgentId,
      ).not.toHaveBeenCalled();
    ...
    it("rejects JiaorongAI targets whose default provider is ACP", async () => {
      const row = {
        id: "s-deepchat",
        agent_id: "deepchat-writer",
        title: "JiaorongAI session",
      ...
      configPresenter.getAgentType.mockImplementation(
        async (agentId: string) => {
          if (
            agentId === "deepchat-writer" ||
            agentId === "deepchat-acp-default"
          ) {
            return "deepchat";
          }
          return null;
    ...
    it("rejects moving an ACP conversation to another ACP target", async () => {
      ...
      await expect(
        presenter.moveSessionToAgent("s-acp", "acp-reviewer"),
      ).rejects.toThrow("Conversation history cannot be moved to ACP agents.");
      expect(deepChatAgent.setSessionAgentContext).not.toHaveBeenCalled();
      expect(
        sqlitePresenter.newSessionsTable.updateAgentId,
  ...
  describe("getAgents", () => {
    it("returns registered agents", async () => {
      ...
      expect(agents[0].name).toBe("JiaorongAI");
    });
    ...
    it("includes ACP agents from config", async () => {
      ...
      expect(
        agents.some(
          (agent: any) => agent.id === "acp-coder" && agent.type === "acp",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("getAcpSessionCommands", () => {
    ...
    it("fetches commands for ACP-backed sessions", async () => {
      ...
      expect(llmProviderPresenter.getAcpSessionCommands).toHaveBeenCalledWith(
        "s-acp",
      );
  ...
  describe("ACP session config options", () => {
    it("returns null for non-ACP sessions", async () => {
      ...
      expect(
        llmProviderPresenter.getAcpSessionConfigOptions,
      ).not.toHaveBeenCalled();
    });
    ...
    it("writes ACP session config options through llmProviderPresenter", async () => {
      ...
      const result = await presenter.setAcpSessionConfigOption(
        "s-acp",
        "model",
        "gpt-5-mini",
      );
      ...
      expect(
        llmProviderPresenter.setAcpSessionConfigOption,
      ).toHaveBeenCalledWith("s-acp", "model", "gpt-5-mini");
