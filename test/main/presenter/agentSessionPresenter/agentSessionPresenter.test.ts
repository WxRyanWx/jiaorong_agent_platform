...
function createMockConfigPresenter() {
  return {
    ...
    listAgents: vi
      .fn()
      .mockResolvedValue([{ id: 'deepchat', name: 'JiaorongAI', type: 'deepchat', enabled: true }]),
...
describe('AgentSessionPresenter', () => {
  ...
  describe('agent session transfer', () => {
    ...
    it('uses the first enabled chat model when the target agent has no default preset', async () => {
      sqlitePresenter.newSessionsTable.list.mockReturnValue([])
      configPresenter.getDefaultModel.mockReturnValue(undefined)
      configPresenter.resolveDeepChatAgentConfig.mockResolvedValue({
      ...
      configPresenter.getAllEnabledModels = vi.fn().mockResolvedValue([
        {
          providerId: 'openai',
      ...
      configPresenter.getAgentType.mockImplementation(async (agentId: string) => {
        if (agentId === 'deepchat-custom' || agentId === 'deepchat') {
          return 'deepchat'
        }
        return null
      })

      await expect(presenter.moveAgentSessions('deepchat-custom', 'deepchat')).resolves.toEqual({
        movedSessionIds: [],
        deletedSessionIds: []
      })

      expect(configPresenter.getAllEnabledModels).toHaveBeenCalled()
    })
