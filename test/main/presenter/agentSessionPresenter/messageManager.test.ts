...
describe("NewMessageManager", () => {
  ...
  describe("getMessages", () => {
    ...
    it("throws if session not found", async () => {
      (mocks.sessionManager.get as ReturnType<typeof vi.fn>).mockReturnValue(
        null,
      );
      await expect(manager.getMessages("missing")).rejects.toThrow(
  ...
  describe("getMessageIds", () => {
    ...
    it("throws if session not found", async () => {
      ...
      await expect(manager.getMessageIds("missing")).rejects.toThrow(
        "Session not found: missing",
      );
    });
  });
