  ...
  it("attempts one schema repair and retries initialization for repairable schema errors", async () => {
    ...
    const SQLitePresenter = vi
      ...
      .mockImplementationOnce(() => {
        throw new Error(
          "table deepchat_sessions has no column named reasoning_visibility",
        );
      })
    ...
    vi.doMock("electron", () => ({
      app: {
        getPath: vi
          .fn()
          .mockReturnValue("C:/Users/test/AppData/Roaming/JiaorongAI"),
      },
    }));
    ...
    expect(repairSQLiteDatabaseFile).toHaveBeenCalledWith(
      "C:/tmp/deepchat-agent.db",
      undefined,
    );
