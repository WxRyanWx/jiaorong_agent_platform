...
const buildFromTemplateMock = vi.hoisted(() =>
  vi.fn((template) => ({ template })),
);
...
describe("ShortcutPresenter", () => {
  ...
  it("registers sidebar and workspace menu accelerators and sends renderer events to the focused window", async () => {
    ...
    const toggleSidebarItem = findMenuItemByAccelerator(
      template,
      "CommandOrControl+B",
    );
    const toggleWorkspaceItem = findMenuItemByAccelerator(
      template,
      "CommandOrControl+J",
    );
    ...
    expect(
      presenterMock.windowPresenter.sendToWebContents,
    ).toHaveBeenNthCalledWith(1, 42, SHORTCUT_EVENTS.TOGGLE_SIDEBAR);
    expect(
      presenterMock.windowPresenter.sendToWebContents,
    ).toHaveBeenNthCalledWith(2, 42, SHORTCUT_EVENTS.TOGGLE_WORKSPACE);
  });
  ...
  it("does not register app-scoped shortcuts through globalShortcut", async () => {
    ...
    expect(
      registerMock.mock.calls.some(
        ([accelerator]) => accelerator === "CommandOrControl+B",
      ),
    ).toBe(false);

    registerMock.mock.calls[0][1]();
    expect(eventBusMock.sendToMain).toHaveBeenCalledWith(
