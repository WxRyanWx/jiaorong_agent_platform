...
describe('ShortcutPresenter', () => {
  ...
  it('does not register app-scoped shortcuts through globalShortcut', async () => {
    const { ShortcutPresenter } = await import('@/presenter/shortcutPresenter')
    const shortcutPresenter = new ShortcutPresenter(createConfigPresenter())
    const screenshotShortcut = 'CommandOrControl+Alt+A'

    shortcutPresenter.registerShortcuts()

    expect(registerMock).toHaveBeenCalledTimes(2)
    expect(registerMock).toHaveBeenCalledWith('CommandOrControl+O', expect.any(Function))
    expect(registerMock).toHaveBeenCalledWith(screenshotShortcut, expect.any(Function))
    expect(
  ...
  it('registers the configured screenshot shortcut', async () => {
    const { ShortcutPresenter } = await import('@/presenter/shortcutPresenter')
    const shortcutPresenter = new ShortcutPresenter(
      createConfigPresenter({ Screenshot: 'CommandOrControl+Shift+S' })
    )

    shortcutPresenter.registerShortcuts()

    expect(registerMock).toHaveBeenCalledWith('CommandOrControl+Shift+S', expect.any(Function))
  })
