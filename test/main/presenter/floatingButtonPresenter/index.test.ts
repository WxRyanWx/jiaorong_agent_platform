...
vi.mock('electron', () => ({
  ...
  screen: {
    ...
    getCursorScreenPoint: vi.fn(() => ({
      x: electronState.cursorPoint.x,
      y: electronState.cursorPoint.y
    }))
  },
  Menu: {
...
vi.mock('../../../../src/main/presenter/floatingButtonPresenter/FloatingButtonWindow', () => ({
  FloatingButtonWindow: class MockFloatingButtonWindow {
    ...
    public setPosition = vi.fn((x: number, y: number) => {
      floatingWindowState.bounds = {
        ...floatingWindowState.bounds,
        x: Math.round(x),
        y: Math.round(y)
      }
...
describe('FloatingButtonPresenter drag layout sync', () => {
  ...
  const setCursorPoint = (x: number, y: number) => {
    electronState.cursorPoint.x = x
    electronState.cursorPoint.y = y
  }
