...
describe("toolNameMapping", () => {
  it("maps Claude Code tool names to canonical names", () => {
    expect(normalizeSkillToolName("Read")).toEqual({
      canonical: "read",
      mapped: true,
    });
    expect(normalizeSkillToolName("MultiEdit")).toEqual({
      canonical: "edit",
      mapped: true,
    });
    ...
    expect(normalizeSkillToolName("Bash")).toEqual({
      canonical: "exec",
      mapped: true,
    });
  });

  it("maps legacy JiaorongAI names to canonical names", () => {
    expect(normalizeSkillToolName("read_file")).toEqual({
      canonical: "read",
      mapped: true,
    });
    expect(normalizeSkillToolName("write_file")).toEqual({
      canonical: "write",
      mapped: true,
    });
  ...
  it("keeps unknown names and emits warnings", () => {
    ...
    expect(
      result.warnings.some((msg) => msg.includes("read_file -> read")),
    ).toBe(true);
    expect(
      result.warnings.some((msg) => msg.includes("Unknown allowedTools entry")),
    ).toBe(true);
  });
});
