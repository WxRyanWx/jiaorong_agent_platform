...
describe("Agent tape tools", () => {
  it("exposes tape tools for JiaorongAI sessions", async () => {
    ...
    const handoffDef = defs.find(
      (def) => def.function.name === TAPE_TOOL_NAMES.handoff,
    );
  ...
  it("does not expose tape tools outside JiaorongAI sessions", async () => {
    const manager = buildManager(
      buildRuntimePort({
        resolveConversationSessionInfo: vi.fn().mockResolvedValue({
          agentType: "acp",
    ...
    expect(defs.some((def) => def.function.name === TAPE_TOOL_NAMES.info)).toBe(
      false,
    );
  });
  ...
  it("routes tape tool calls through the runtime port", async () => {
    ...
    const info = (await manager.callTool(
      TAPE_TOOL_NAMES.info,
      {},
      "conv-1",
    )) as {
    ...
    const anchors = (await manager.callTool(
      TAPE_TOOL_NAMES.anchors,
      { limit: 5 },
      "conv-1",
    )) as {
    ...
    expect(JSON.parse(info.content)).toMatchObject({
      entries: 3,
      migrationState: "ready",
    });
    ...
    expect(runtimePort.listTapeAnchors).toHaveBeenCalledWith("conv-1", {
      limit: 5,
    });
    expect(runtimePort.handoffTape).toHaveBeenCalledWith("conv-1", "manual", {
      summary: "done",
    });
  });
