...
describe("SubagentOrchestratorTool", () => {
  it("includes the parent session workdir in the child handoff", async () => {
    let listener: ((update: DeepChatInternalSessionUpdate) => void) | null =
      null;
    let handoffMessage = "";
    ...
    const resolveConversationWorkdir = vi
      .fn()
      .mockResolvedValue(resolvedWorkdir);
    ...
    const tool = new SubagentOrchestratorTool({
      ...
      resolveConversationSessionInfo: vi
        ...
        .mockImplementation(async (conversationId: string) =>
          conversationId === parentSession.sessionId
            ? parentSession
            : childSession,
        ),
    ...
    expect(resolveConversationWorkdir).toHaveBeenCalledWith(
      parentSession.sessionId,
    );
  ...
  it("starts background runs and supports list, info, and kill operations", async () => {
    ...
    const tool = new SubagentOrchestratorTool({
      resolveConversationWorkdir: vi
        .fn()
        .mockResolvedValue(parentSession.projectDir),
    ...
    const progress = JSON.parse(
      (started.rawData?.toolResult as any).subagentProgress,
    );
    ...
    for (
      let index = 0;
      index < 10 && createSubagentSession.mock.calls.length === 0;
      index += 1
    ) {
    ...
    const info = await tool.call(
      { operation: "info", runId },
      parentSession.sessionId,
    );
    expect(info.content).toContain("Keep running");

    const killed = await tool.call(
      { operation: "kill", runId },
      parentSession.sessionId,
    );
  ...
  it("retries failed subagent tape finalization on terminal wait", async () => {
    ...
    const finalProgress = JSON.parse(
      (waited.rawData?.toolResult as any).subagentFinal,
    );
  ...
  it("exposes persistent subagent tape finalization failures and keeps retrying", async () => {
    ...
    const mergeSubagentTape = vi
      .fn()
      .mockRejectedValue(new Error("merge still failed"));
    const warnSpy = vi
      .spyOn(console, "warn")
      .mockImplementation(() => undefined);
    ...
    const waitedProgress = JSON.parse(
      (waited.rawData?.toolResult as any).subagentFinal,
    );

    expect(mergeSubagentTape).toHaveBeenCalledTimes(2);
    expect(waited.rawData?.isError).toBe(true);
    expect(waited.content).toContain(
      "Tape Finalization: failed: merge still failed",
    );
  ...
  it("cancels a newly created child before handoff when the parent signal aborts", async () => {
    ...
    await expect(runPromise).rejects.toThrow(
      "subagent_orchestrator cancelled.",
    );
