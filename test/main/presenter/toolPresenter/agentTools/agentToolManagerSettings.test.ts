...
describe("AgentToolManager JiaorongAI settings tool gating", () => {
  ...
  it("rejects skill_manage create requests without content before calling the presenter", async () => {
    ...
    await expect(
      manager.callTool("skill_manage", { action: "create" }, "conv-1"),
  ...
  it("resolves workdir from new session first", async () => {
    ...
    const workdir = await (manager as any).getWorkdirForConversation(
      "new-session-1",
    );
  ...
  it("builds a stable slotId enum for subagent_orchestrator from the session config", async () => {
    ...
    const subagentDef = defs.find(
      (def) => def.function.name === "subagent_orchestrator",
    );
    const slotIdSchema = (subagentDef?.function.parameters as any)?.properties
      ?.tasks?.items?.properties?.slotId;
    const promptSchema = (subagentDef?.function.parameters as any)?.properties
      ?.tasks?.items?.properties?.prompt;

    expect(slotIdSchema?.enum).toEqual(["reviewer", "writer"]);
    expect(slotIdSchema?.description).toContain(
      "reviewer: ACP Reviewer | target=acp-reviewer",
    );
    expect(slotIdSchema?.description).toContain(
      "writer: Writer Clone | target=current agent",
    );
