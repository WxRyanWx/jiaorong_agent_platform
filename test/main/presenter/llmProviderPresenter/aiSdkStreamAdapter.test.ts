...
async function collectEvents(
  parts: any[],
  options: Parameters<typeof adaptAiSdkStream>[1],
) {
...
describe("AI SDK stream adapter", () => {
  it("maps native tool streaming events to JiaorongAI core events", async () => {
    const events = await collectEvents(
      [
        ...
        },
        {
          ...
          text: "thinking",
          providerMetadata: {
            vertex: { thoughtSignature: "reason-signature" },
          },
        },
        {
    ...
    expect(events).toEqual([
      ...
      },
      {
        type: "tool_call_chunk",
        tool_call_id: "call-1",
        tool_call_arguments_chunk: 'Beijing"}',
      },
      {
        type: "tool_call_end",
        tool_call_id: "call-1",
