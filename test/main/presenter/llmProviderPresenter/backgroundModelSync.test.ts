...
vi.mock(
  "../../../../src/main/presenter/configPresenter/modelCapabilities",
  () => ({
    modelCapabilities: {
...
describe("LLMProviderPresenter background model sync", () => {
  ...
  it("refreshes provider DB before rebuilding DB-backed provider models", async () => {
    ...
    expect(refreshSpy).toHaveBeenCalledTimes(1);
    expect(
      configPresenter.refreshProviderDb.mock.invocationCallOrder[0],
    ).toBeLessThan(refreshSpy.mock.invocationCallOrder[0]);
  });
