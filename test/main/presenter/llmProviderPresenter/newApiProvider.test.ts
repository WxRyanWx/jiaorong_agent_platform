...
describe("NewApiProvider capability routing", () => {
  beforeEach(() => {
    ...
    mockRunAiSdkCoreStream.mockReturnValue({
      async *[Symbol.asyncIterator]() {
        yield {
          type: "image_data",
          image_data: { data: "generated-image", mimeType: "image/png" },
        };
      },
    });
  });
  ...
  it("maps openai-response delegates to openai capability semantics", () => {
    ...
    const runtimeProvider = (provider as any).getRuntimeProvider(
      routeDecision,
    ) as LLM_PROVIDER;
  ...
  it("maps gemini delegates to gemini capability semantics", () => {
    ...
    const routeDecision = (provider as any).resolveRouteDecision(
      "gemini-model",
    );
    const runtimeProvider = (provider as any).getRuntimeProvider(
      routeDecision,
  ...
  it("maps anthropic delegates to anthropic capability semantics", () => {
    ...
    expect(runtimeContext.context.provider.capabilityProviderId).toBe(
      "anthropic",
    );
    expect(runtimeContext.context.supportsOfficialAnthropicReasoning).toBe(
      true,
    );
  });
  ...
  it("overlays provider DB capabilities while preserving new-api endpoint routing", async () => {
    ...
    expect(supportsReasoningSpy).toHaveBeenCalledWith(
      "anthropic",
      "claude-opus-4-8",
    );
  ...
  it("keeps transport-compatible anthropic api providers off the official anthropic reasoning route", () => {
    ...
    expect(
      runtimeContext.context.provider.capabilityProviderId,
    ).toBeUndefined();
    expect(
      runtimeContext.context.supportsOfficialAnthropicReasoning,
    ).toBeUndefined();
  });
