...
describe("AI SDK provider factory", () => {
  ...
  it("normalizes anthropic-style base urls to a v1 prefix", () => {
    ...
    expect(
      normalizeAnthropicBaseUrl("https://api.minimaxi.com/anthropic"),
  ...
  it("avoids duplicating the messages suffix", () => {
    ...
    expect(
      normalizeAnthropicBaseUrl("https://zenmux.ai/api/anthropic/v1/messages"),
    ).toBe("https://zenmux.ai/api/anthropic/v1");
    expect(
      normalizeAnthropicBaseUrl("https://proxy.example.com/messages"),
  ...
  it("normalizes vertex express-mode base urls to the publishers/google prefix", () => {
    ...
    expect(
      normalizeVertexBaseUrl(
        "https://zenmux.ai/api/vertex-ai/v1",
        "api-key",
        "v1",
      ),
    ).toBe("https://zenmux.ai/api/vertex-ai/v1/publishers/google");
  ...
  it("normalizes gemini base urls to a v1beta prefix", () => {
    ...
    expect(normalizeGeminiBaseUrl("https://api.newapi.ai")).toBe(
      "https://api.newapi.ai/v1beta",
    );
    expect(normalizeGeminiBaseUrl("https://api.newapi.ai/v1")).toBe(
      "https://api.newapi.ai/v1beta",
    );
  ...
  it("normalizes Ollama base urls for SDK and OpenAI-compatible endpoints", () => {
    expect(normalizeOllamaSdkHost("http://localhost:11434")).toBe(
      "http://localhost:11434",
    );
    expect(normalizeOllamaSdkHost("http://localhost:11434/api")).toBe(
      "http://localhost:11434",
    );
    expect(normalizeOllamaSdkHost("http://localhost:11434/v1")).toBe(
      "http://localhost:11434",
    );
    ...
    expect(normalizeOllamaOpenAIBaseUrl("http://localhost:11434")).toBe(
  ...
  it("normalizes azure resource base urls to the openai prefix with v1 semantics", () => {
    expect(
      normalizeAzureBaseUrl("https://example.openai.azure.com", undefined),
    ).toEqual({
