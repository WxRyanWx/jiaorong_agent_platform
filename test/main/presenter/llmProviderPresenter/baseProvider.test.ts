...
describe("BaseLLMProvider tool XML conversion", () => {
  ...
  it("normalizes discriminated union tool schemas before building XML", () => {
    ...
    expect(xml).toContain(
      '<tool name="skill_manage" description="Manage draft skills">',
    );
    expect(xml).toContain(
      '<parameter name="action" required="true" type="string"></parameter>',
    );
    expect(xml).toContain(
  ...
  it("keeps tools without properties renderable", () => {
    ...
    expect(xml).toContain(
      '<tool name="noop" description="No arguments tool"></tool>',
    );
  });
