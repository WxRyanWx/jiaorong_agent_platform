...
vi.mock("electron", () => ({
  app: {
    ...
    getPath: vi.fn((name: string) =>
      name === "userData" ? "/mock/userData" : "/mock/home",
    ),
  },
  utilityProcess: {
...
describe("BackgroundExecSessionManager", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(fs, "statSync").mockImplementation((candidate) =>
      String(candidate).includes("workspace")
        ? mockStats("directory")
        : mockStats("file"),
    );
  ...
  it("waits for completion and returns a completion snapshot before cleanup", async () => {
    ...
    const result = await manager.waitForCompletionOrYield(
      "conv-1",
      "bg_123",
      10,
    );
  ...
  it("returns running when the session outlives the yield window", async () => {
    ...
    const resultPromise = manager.waitForCompletionOrYield(
      "conv-1",
      "bg_123",
      10,
    );
  ...
  it("clears the yield timer when the session closes before the yield window elapses", async () => {
    ...
    await expect(
      manager.waitForCompletionOrYield("conv-1", "bg_123", 1000),
    ).resolves.toEqual({
      kind: "completed",
      result: {
        status: "done",
...
describe("backgroundExecSessionManager utility proxy", () => {
  ...
  it("returns crashed completion results without starting a fresh utility host", async () => {
    ...
    await expect(
      backgroundExecSessionManager.waitForCompletionOrYield(
        "conv-1",
        "bg_crashed",
        10,
      ),
