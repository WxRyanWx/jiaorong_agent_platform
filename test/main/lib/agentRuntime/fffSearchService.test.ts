...
describe("FffSearchService", () => {
  it("loads the packaged FFF module from app.asar.unpacked when available", async () => {
    const resourcesPath = await fs.mkdtemp(
      path.join(os.tmpdir(), "fff-packaged-resources-"),
    );
...
`,
    ...
    vi.mocked(existsSync).mockImplementation(
      (candidate) => String(candidate) === entryPath,
    );
  ...
  it("maps file search results into JiaorongAI JSON shape", async () => {
    const mock = createMockModule();
    const service = new FffSearchService({
      moduleLoader: vi
        .fn()
        .mockResolvedValue({ FileFinder: mock.FileFinder } as any),
    });
  ...
  it("maps grep results with context into JiaorongAI JSON shape", async () => {
    const mock = createMockModule();
    const service = new FffSearchService({
      moduleLoader: vi
        .fn()
  ...
  it("hydrates grep snippets from disk when FFF line content is truncated", async () => {
    const workspaceRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "fff-grep-snippet-"),
    );
