      ...
      if (sql.includes("FROM providers")) {
        return {
          all: (...appTypes: string[]) => {
            ...
            return rows.filter(
              (row) => allowed.size === 0 || allowed.has(String(row.app_type)),
            );
          },
        };
      }
...
const writeDarwinCcSwitchAppPaths = (
  homeDir: string,
  overrideDir: string,
  content?: string,
) => {
...
const writeCherryStudioConfig = (homeDir: string, appDataPath: unknown) => {
  writeFile(
    path.join(homeDir, ".cherrystudio/config/config.json"),
    JSON.stringify({ appDataPath }),
  );
};
...
const createHome = () =>
  mkdtempSync(path.join(tmpdir(), "deepchat-provider-import-"));
...
describe("ProviderImportService", () => {
  ...
  it("scans Linux using the same home-relative paths as macOS", async () => {
    ...
    expect(
      result.sources.find((source) => source.id === "openclaw"),
    ).toMatchObject({
      status: "found",
  ...
  it("scans Windows APPDATA and user profile paths", async () => {
    ...
    writeFile(
      path.join(appDataDir, "alma/chat_threads.db"),
    ...
    expect(
      result.sources.find((source) => source.id === "cherry-studio"),
    ).toMatchObject({
      status: "not_found",
      configPath: "%APPDATA%\\CherryStudio\\Local Storage\\leveldb",
    });
    expect(
      result.sources.find((source) => source.id === "hermes"),
    ).toMatchObject({
      status: "found",
  ...
  it("uses Windows HOME fallback for legacy CC Switch database paths", async () => {
    ...
    expect(
      result.sources.find((source) => source.id === "cc-switch"),
    ).toMatchObject({
      status: "found",
  ...
  it("falls back to the default CC Switch database when Desktop override db is missing", async () => {
    homeDir = createHome();
    writeDarwinCcSwitchAppPaths(
      homeDir,
      path.join(homeDir, "missing-cc-switch"),
    );
    createCcSwitchDb(path.join(homeDir, ".cc-switch/cc-switch.db"), [
      {
        id: "deepseek",
  ...
  it("reports an error on the CC Switch Desktop override database instead of falling back", async () => {
    ...
    writeDarwinCcSwitchAppPaths(homeDir, overrideDir);
    writeFile(
      path.join(overrideDir, "cc-switch.db"),
      "unregistered mock database",
    );
    createCcSwitchDb(path.join(homeDir, ".cc-switch/cc-switch.db"), [
      {
        id: "deepseek",
  ...
  it("keeps Codex-only CC Switch Desktop override providers hidden", async () => {
    ...
    expect(result.providers).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ sourceId: "cc-switch" }),
      ]),
    );
  });
  ...
  it("does not select providers by default when JiaorongAI already has a config", async () => {
    homeDir = createHome();
    writeFile(
      path.join(homeDir, ".hermes/config.yaml"),
      [
  ...
  it("hides missing-key providers and maps unknown key-url providers to custom", async () => {
    ...
    expect(result.providers).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ sourceProviderId: "deepseek" }),
      ]),
    );
  });
  ...
  it("hides custom base-url-only providers even if they could be overridden to ollama", async () => {
    ...
    expect(scan.providers).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ sourceProviderId: "local-ollama" }),
      ]),
    );
  });
  ...
  it("scans CC Switch non-Codex providers and hides empty or Codex rows", async () => {
    ...
    expect(
      scan.sources.find((source) => source.id === "cc-switch"),
    ).toMatchObject({
      status: "found",
      configPath: "~/.cc-switch/cc-switch.db",
      providerCount: 6,
  ...
  it("uses Cherry Studio custom data directory from app config", async () => {
    ...
    const customCherryPath = path.join(
      customCherryDataDir,
    ...
    expect(result.providers).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ sourceProviderId: "default-openai" }),
      ]),
    );
  });
  ...
  it("falls back to the default Cherry Studio LevelDB when configured data directory is missing", async () => {
    ...
    writeCherryStudioConfig(
      homeDir,
      path.join(homeDir, "Downloads/missing-cherry"),
    );
    await createCherryStudioLevelDb(defaultCherryPath, [
      {
        id: "ppio",
  ...
  it("applies selected providers in source order and lets later sources overwrite earlier ones", async () => {
    ...
    const scan = await service.scan();
    const hermesProvider = scan.providers.find(
      (provider) => provider.sourceId === "hermes",
    )!;
    const openclawProvider = scan.providers.find(
      (provider) => provider.sourceId === "openclaw",
    )!;
