...
describe('afterPack', () => {
  ...
  it('adds the Linux launcher wrapper for AppImage builds', async () => {
    const afterPack = await loadAfterPack()
    const linuxAppName = 'jiaorongsuperintelligentagent'
    const launcherPath = path.join(tmpDir, linuxAppName)
    ...
    await expect(stat(path.join(tmpDir, `${linuxAppName}.bin`))).resolves.toBeTruthy()
