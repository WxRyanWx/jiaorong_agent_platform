...
describe("sign-cua-helper", () => {
  ...
  it("imports the release certificate and signs the helper before plugin packaging", async () => {
    ...
    const calls = childProcessMocks.execFileAsync.mock.calls as Array<
      [string, string[]]
    >;
    expect(
      calls.some(
    ...
    expect(
      calls.some(
        ([command, args]) =>
          command === "/usr/bin/codesign" && args.includes("--timestamp"),
      ),
    ).toBe(true);
    expect(
      calls.some(
        ([command, args]) =>
          command === "/usr/bin/security" && args[0] === "delete-keychain",
      ),
    ).toBe(true);
  });
});
