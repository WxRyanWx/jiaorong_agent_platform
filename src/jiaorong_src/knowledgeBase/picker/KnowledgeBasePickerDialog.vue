...
import {
  knowledgeBaseSelectionKey,
  type KnowledgeBaseSelectionItem,
} from "./types";
...
const allDirectoryChecked = computed(() => {
  ...
  return directoryList.value.every((row) =>
    isDraftSelected(directoryRowKey(row)),
  );
});
...
async function loadDirectory(
  ...
  const page =
    pageOverride ?? (append ? directoryPage.value + 1 : directoryPage.value);
  ...
  try {
    ...
    errorMessage.value =
      error instanceof Error ? error.message : "加载目录失败";
...
function setKnowledgeBaseChecked(
    ...
    >
      ...
      <div class="kb-picker-body">
        <template v-if="viewMode === 'list'">
          ...
          <div v-if="errorMessage" class="kb-picker-error">
            {{ errorMessage }}
          </div>
          <div
          ...
          >
            <Icon
              ...
              class="kb-picker-loading-spinner"
            />
            <span>加载中…</span>
          </div>
          <div v-else-if="kbList.length === 0" class="kb-picker-empty">
            暂无知识库
          </div>
        ...
        <template v-else>
          ...
          <div
            v-else-if="
              !directoryLoading && directoryList.length === 0 && !errorMessage
            "
          ...
          <div
            v-else-if="directoryList.length > 0"
            class="kb-picker-table-wrap"
          >
            <table class="kb-picker-table">
              <thead>
                <tr>
                  ...
                  <th class="kb-picker-th kb-picker-th-name">
                    文件夹 / 文件名称
                  </th>
              ...
              <tbody>
                <tr
                  ...
                  class="kb-picker-row"
                  :class="{
                    'is-selected': isDraftSelected(directoryRowKey(row)),
                  }"
                >
                  <td class="kb-picker-td kb-picker-td-check" @click.stop>
                    <Checkbox
                      class="kb-picker-checkbox"
                      :class="{
                        'is-checked': isDraftSelected(directoryRowKey(row)),
                      }"
                  ...
                  <td class="kb-picker-td">
                    ...
                    >
                      ...
                      />
                      <span
                        class="kb-picker-name-text"
                        :class="{ 'is-folder': row.isDirectory }"
                      >
                        {{ row.fileName }}
                      </span>
                    </button>
                  ...
                  <td class="kb-picker-td kb-picker-td-status">
                    <span
                      v-if="!row.isDirectory && row.status"
                      class="kb-picker-status"
                    >
                      <span
                        class="kb-picker-status-dot"
                        :class="statusClass(row.status)"
                      />
                      <span
                        class="kb-picker-status-text"
                        :class="statusClass(row.status)"
                      >
                        {{ row.status }}
                      </span>
          ...
          <div
            v-if="directoryList.length < directoryTotal"
            class="kb-picker-load-more"
          >
            <Button
              variant="outline"
              size="sm"
      ...
      <div class="kb-picker-footer">
        ...
        <div class="kb-picker-actions">
          <Button
            variant="outline"
            class="kb-picker-btn-cancel"
            @click="cancelSelection"
          >
            取消
          </Button>
          <Button class="kb-picker-btn-confirm" @click="confirmSelection"
            >确认</Button
          >
        </div>
      </div>
