...
import { JiaorongError } from 'jiaorong-app-sdk'
import { formatJiaorongError, isUserCanceledError, localizeErrorText } from 'jiaorong-app-sdk'
 ...
 * @param options.appId 应用 id，HTTP / 宿主 connect 都要用
 * @param options.agentId 智能体 id，列会话、建会话时用
 * @param options.sessionId 当前会话；空表示新对话
 * @param options.httpBase 有值则走 HTTP runtime，否则走宿主桥
 * @param options.client 外部注入的客户端；传 null 表示暂不 boot
