...
export function getFileTypeIcon(fileName: string, mimeType?: string): string {
  ...
  if (mime.startsWith('image/') || IMAGE_EXT.test(fileName)) { // 图片
    return 'vscode-icons:file-type-image'
  }
  if (mime.startsWith('text/plain') || mime.includes('json') || TEXT_EXT.test(fileName)) { // 文本 / JSON / 脚本
    return 'vscode-icons:file-type-text'
  }
  if (mime.startsWith('text/csv') || mime.includes('spreadsheet') || EXCEL_EXT.test(fileName)) { // 表格
    return 'vscode-icons:file-type-excel'
  }
  if (mime.startsWith('text/markdown') || MD_EXT.test(fileName)) { // Markdown
    return 'vscode-icons:file-type-markdown'
  }
  if (mime.startsWith('application/x-yaml') || YAML_EXT.test(fileName)) { // YAML
    return 'vscode-icons:file-type-yaml'
  }
  if (mime.startsWith('application/xml') || XML_EXT.test(fileName)) { // XML
    return 'vscode-icons:file-type-xml'
  }
  if (mime.startsWith('application/pdf') || PDF_EXT.test(fileName)) { // PDF
  ...
  if (mime.includes('presentationml') || PPT_EXT.test(fileName)) { // PPT
    return 'vscode-icons:file-type-powerpoint'
  }
  if (mime.startsWith('text/html') || HTML_EXT.test(fileName)) { // HTML
    return 'vscode-icons:file-type-html'
  }
  if (mime.startsWith('text/css') || CSS_EXT.test(fileName)) { // CSS
    return 'vscode-icons:file-type-css'
  }
  if (mime.startsWith('audio/') || AUDIO_EXT.test(fileName)) { // 音频
