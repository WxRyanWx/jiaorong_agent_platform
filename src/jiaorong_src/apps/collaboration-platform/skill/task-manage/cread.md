...
### 第二步：从用户消息中提取信息
分析用户当前消息，尝试提取与 `./assets/task-schema.json` 中字段对应的值。将提取结果暂存到`objectCatch`。
提取规则：
...
**情况 A — 所有字段已齐全：**
1. 将完整的数据摘要展示给用户，并且在对话中以JSON格式展示`objectCatch`。
2. 最终必须输出两个选项按钮给到用户使用`deepchat_question`，1是确认创建 + `objectCatch`的json数据，2是其它
