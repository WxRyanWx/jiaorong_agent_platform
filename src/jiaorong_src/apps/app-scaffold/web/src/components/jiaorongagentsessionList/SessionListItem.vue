<!-- 侧栏单条会话：标题高亮、生成中光泽、置顶 / 删除、完成或错误状态图标。 -->
<script setup lang="ts">
import { computed } from 'vue'
import { Icon } from '@iconify/vue'
import type { SessionWithState } from '../jiaorongagentchat/model/host'

/**
 * 行尾状态图标。无完成 / 错误时为 null，不渲染。
 */
...
/** 置顶按钮文案 */
const pinActionLabel = computed(() => (props.session.isPinned ? '取消置顶' : '置顶'))
/** 生成中：本行正在生成或会话状态为 generating */
const isWorking = computed(() => props.generating || props.session.status === 'generating')
/** 置顶态：已置顶常驻钉，未置顶仅悬停浮出 */
const pinState = computed<'docked' | 'overlay'>(() =>
  props.session.isPinned ? 'docked' : 'overlay'
)
...
const titleSegments = computed(() => {
  ...
  const lowerTitle = source.toLowerCase()
  ...
  while (matchIndex !== -1) {
    if (matchIndex > searchIndex) {
      segments.push({ text: source.slice(searchIndex, matchIndex), match: false })
    }
  ...
  if (searchIndex < source.length) {
    segments.push({ text: source.slice(searchIndex), match: false })
  }
