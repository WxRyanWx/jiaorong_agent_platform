...
async function writePatchedFolderZip(
  ...
  await workspaceClient.registerWorkspace(root, 'workspace')
  try {
    ...
    const walk = async (dir: string, relParts: string[]): Promise<void> => {
      ...
      const nodes = await workspaceClient.readDirectory(dir)
    ...
    try {
      await workspaceClient.unregisterWorkspace(root, 'workspace')
    } catch {
