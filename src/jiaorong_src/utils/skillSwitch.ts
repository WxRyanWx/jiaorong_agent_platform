...
/** hydrate 期间若用户已写入，则不要用 config 覆盖 */
let localWriteSeq = 0

/** 测试用：重置 hydrate 闸门 */
export function _resetSkillSwitchHydrateForTests(): void {
  didHydrateFromConfig = false
  hydratePromise = null
  localWriteSeq = 0
}
...
/**
 * 与主进程 config 对齐后再给 UI 过滤：
 * - config 有数据 → 写入 local（UI 与主进程过滤一致）
 * - config 空而 local 有 → 回写 config（修复曾写入失败）
 * - hydrate 期间若已有本地写入，则不覆盖
 */
export function ensureSkillSwitchHydrated(): Promise<void> {
  if (didHydrateFromConfig) return Promise.resolve()
  ...
  hydratePromise = (async () => {
    ...
    try {
      const fromConfig = normalizeSkillSwitchMap(
        await createConfigClient().getSetting(JIAORONG_SKILL_SWITCH_SETTING_KEY)
      )
      ...
      emitSwitchEvent('', SkillSwitchStatus.On)
    } catch (error) {
...
function readLocalMap(): SkillSwitchMap {
  void ensureSkillSwitchHydrated()
...
async function persistToConfig(map: SkillSwitchMap): Promise<void> {
  try {
    await createConfigClient().setSetting(JIAORONG_SKILL_SWITCH_SETTING_KEY, map)
  } catch (error) {
