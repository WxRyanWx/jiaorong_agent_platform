...
export function resolveToolDisplay(tool: ToolDisplaySource) {
  ...
  const staticName = resolveStaticToolDisplayName(tool.name)
  // Remote HTTP MCP often echoes the function name as `title`. That is not a UI label.
  const runtimeLabel =
    displayName && displayName.toLowerCase() !== tool.name.trim().toLowerCase()
      ? displayName
      : undefined
  return {
...
export function getDisplayLabel(
  ...
  return (
    resolveStaticToolDisplayName(normalized) ||
    resolveStaticToolDisplayName(resolvedToolName) ||
    identifier
  )
}
