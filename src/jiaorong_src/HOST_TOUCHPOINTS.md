...
| H156 | `src/main/app/composition.ts` | `listSlashSources` 给 `jr.catalog.slash`（技能 + MCP）。`JiaorongAgentChat` 的 `/` 菜单改走组件 `slashItems`，不请求该方法 | app chat kit | 中 | SDK `catalog.slash` 仍需登录；对话组件不再自动拉 |
...
| H160 | `electron.vite.config.ts` preload `jiaorongApp`；`electron-builder.yml` extraResources `node/node_modules`；`scan.ts` 拷贝源 `node/node_modules` | 打开应用 spawn Node + 注入 `globalThis.jiaorong`；preload `window.jiaorong.userinfo`；侧栏 `listVisible` 用内置目录 `app.json` 版本对安装目录，不一致才同步拷（含 `node/node_modules`）；不要用 catalog 版本和安装目录交叉比对 | app scaffold | 中 | Node 走 IPC；userinfo 来自主进程 auth session；依赖随包带上，用户机不跑 pnpm；catalog 与 `app.json` version 必须同一字符串 |
...
| H166 | `appHost/devCenter/main/devApps.ts` `appHost/devCenter/main/devCenter.ts` `appHost/devCenter/renderer/DevCenterPage.vue` `WindowSideBar.vue` `main/scan.ts` | 侧栏「开发者中心」入口（所有人可见，不做权限控制）+ `/dev-center` 页；创建=选目录+app.json 必填校验（id/name/version/entry），名单存渲染 localStorage 并 sync 主进程；scan 并入本地 runtime 供打开；发布=最终版 app.json + zip 包表单校验后占位提交；示例应用走配置 devApp 对象，下载选目录落 zip；入口在侧栏搜索正上方 | dev center | 中 | 入口与全部 handler 均不校验开发者身份（应用中心权限不变）；名单为空时 scan 与现状一致；发布接口待接入 |
| H167 | `src/renderer/src/apps/chat-main/ChatMainApp.vue` `preload/index.ts` `preload/index.d.ts` `appHost/main/register.ts` `appHost/channels.ts` `appHost/devCenter/main/devAppWindow.ts` `appHost/devCenter/renderer/DevAppWindowRail.vue` `DevAppWindowRail.less` | 开发者中心页改在主窗口渲染（侧栏入口 router push，同应用中心）；「打开」走 `jiaorong-dev-app:open-window`，主进程每应用一个 BrowserWindow 加载 `#/apps/:appId?standalone=1`；独立窗口壳 = macOS 拖拽条 + `DevAppWindowRail`（纯 UI：顶部不可点「所有 agent」+ 选中态的已打开应用图标，尺寸 / 选中态对齐主窗口图标栏）+ `JiaorongAppFrameHost`；关窗停 Node（先查其它窗口是否仍有该应用 webview，有则保留进程），同应用重复打开只聚焦 | dev center window | 中 | 独立窗口 `webSecurity:false` 与主窗口一致（图标 file://）；handler 只校验 appId，不校验开发者身份；旧开发者中心独立窗口与返回按钮已删 |
| H168 | `appHost/renderer/openAppHandoff.ts` `appHost/appCenter/renderer/useJiaorongAppCenter.ts` `AppCenterPage.vue` `appHost/devCenter/renderer/useJiaorongDevCenter.ts` `DevCenterPage.vue` `src/renderer/src/i18n/*/routes.json` | 应用中心 / 开发者中心「打开」加 loading，预热结果经 `renderer/openAppHandoff.ts` 交给常驻宿主（免二次 IPC）；打开 / 安装 / 卸载 / 移除期间 `isBusy` 锁住全部操作按钮；开发者中心卡片目录按钮（占满行内剩余宽度、中段省略号；本地应用不再显示冗余「本地开发」提供方）复用宿主既有 `project.openDirectory`（`shell.openPath`），不新增 IPC；i18n 新增 `devCenterOpenDirFailed` | app center / dev center | 低 | 打开目录失败统一 toast；未改既有 i18n 键与 preload 面 |
