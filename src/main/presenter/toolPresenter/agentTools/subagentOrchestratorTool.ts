...
export class SubagentOrchestratorTool {
  ...
  private async finalizeTaskTape(params: {
    ...
    try {
      if (task.status === "completed") {
        await this.runtimePort.mergeSubagentTape?.(
          parentSessionId,
          task.sessionId,
          meta,
        );
      } else {
        await this.runtimePort.discardSubagentTape?.(
          parentSessionId,
          task.sessionId,
          meta,
        );
      ...
      console.warn(
        "[SubagentOrchestratorTool] Failed to finalize subagent tape fork:",
        {
          parentSessionId,
  ...
  private async retryPendingTapeFinalization(
    run: MutableRunState,
  ): Promise<void> {
  ...
  private async handleRunOperation(
    ...
    if (args.operation === "kill") {
      ...
      for (const task of run.tasks) {
        ...
        task.resultSummary =
          task.resultSummary || "Cancelled by parent session.";
        task.updatedAt = Date.now();
        task.completion.resolve();

        if (task.sessionId) {
          await this.runtimePort
            .cancelConversation(task.sessionId)
            .catch(() => undefined);
        }
      }
      this.updateRunStatus(run);
  ...
  private buildSlotIdParameter(slots: DeepChatSubagentSlot[]) {
    ...
    const slotLines = normalizedSlots.map((slot) => {
      ...
      const summaryParts = [
        `${slot.id}: ${slot.displayName || slot.id}`,
        `target=${target}`,
      ];
      if (slot.description) {
  ...
  async getToolDefinition(
    ...
    const slotIdParameter = this.buildSlotIdParameter(
      session.availableSubagentSlots,
    );
    ...
    return {
        ...
        parameters: {
          ...
          properties: {
              ...
              items: {
                ...
                properties: {
                  id: {
                    type: "string",
                    description:
                      "Optional stable task identifier for this orchestrator run.",
                  },
            ...
            timeoutMs: {
              type: "number",
              description:
                "Maximum wait time for operation=wait. Defaults to 60000.",
            },
          },
        },
      },
      server: {
        ...
        icons: "🧩",
        description: "JiaorongAI subagent orchestration",
      },
    };
  }
  ...
  async call(
    ...
    if (
      ...
      throw new Error(
        "subagent_orchestrator is only available in JiaorongAI regular sessions with subagents enabled.",
      );
    }
    ...
    const inheritedWorkspace =
      (
        await this.runtimePort.resolveConversationWorkdir(parent.sessionId)
      )?.trim() ||
      parent.projectDir?.trim() ||
      null;

    const slotMap = new Map(
      parent.availableSubagentSlots.map((slot) => [slot.id, slot]),
    );
    const now = Date.now();
    const tasks = taskSpecs.map((task, index): MutableTaskState => {
      ...
      if (!slot) {
        throw new Error(
          `Subagent slot not found or not enabled: ${task.slotId}`,
        );
      }
      ...
      const targetAgentId =
        slot.targetType === "self"
          ? parent.agentId
          : (slot.targetAgentId?.trim() ?? null);
      if (!targetAgentId) {
        throw new Error(
          `Subagent slot is missing a target agent: ${task.slotId}`,
        );
      }
    ...
    const updateTaskStatusFromRuntime = (task: MutableTaskState) => {
      ...
      if (task.runtimeStatus === "error") {
        ...
        task.resultSummary =
          task.resultSummary ||
          summarizeResult(task.responseMarkdown) ||
    ...
    const unsubscribe = this.runtimePort.subscribeDeepChatSessionUpdates(
      (update) => {
        ...
        if (update.kind === "blocks") {
          task.previewMarkdown = truncate(
            update.previewMarkdown?.trim() || "",
            600,
          );
          task.responseMarkdown = truncate(
    ...
    const runTask = async (task: MutableTaskState): Promise<void> => {
      ...
      try {
        ...
        if (!child) {
          throw new Error(
            `Failed to create subagent session for slot ${task.slotId}.`,
          );
        }
        ...
        if (
          ...
          maybeResolveTask(task);
          await this.runtimePort
            .cancelConversation(child.sessionId)
            .catch(() => undefined);
          await this.finalizeTaskTape({
            parentSessionId: parent.sessionId,
            runId,
        ...
        await this.runtimePort.sendConversationMessage(
          child.sessionId,
          handoff,
        );
        task.started = true;
        ...
        task.resultSummary =
          error instanceof Error
            ? error.message
            : "Subagent session failed unexpectedly.";
        maybeResolveTask(task);
        await this.finalizeTaskTape({
          parentSessionId: parent.sessionId,
          runId,
