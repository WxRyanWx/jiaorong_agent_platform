...
import type { ToolCallImagePreview, ToolDisplayMetadata } from '@shared/types/core/mcp'
import { toToolDisplayMetadata } from '@shared/lib/toolDisplayMetadata'
...
export class AgentToolManager {
  ...
  async getAllToolDefinitions(context: {
    ...
    if (isAgentMode && this.isSkillsEnabled() && context.conversationId) {
      try {
        ...
        logger.warn('[AgentToolManager] Failed to load JiaorongAI settings tools', { error })
      }
    }
  ...
  /**
   * UI-only catalog built from the same tool definition sources.
   * Skips session/skill gates so labels stay available for display lookup.
   */
  getToolDisplayCatalog(): ToolDisplayMetadata[] {
    const defs: MCPToolDefinition[] = []
    ...
    return Array.from(byName.values()).map(toToolDisplayMetadata)
  }
