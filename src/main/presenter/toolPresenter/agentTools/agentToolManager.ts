...
export class AgentToolManager {
  private static readonly YO_BROWSER_TOOL_NAME_SET = new Set<string>(
  ...
  private readonly fileSystemSchemas = {
    read: z.object({
      ...
      offset: z
        ...
        .min(0)
        .optional()
        .describe("Starting character offset (0-based)"),
      limit: z
        .number()
        .int()
        .positive()
        .optional()
        .describe(
          "Maximum characters to read. Large files are auto-truncated if not specified",
        ),
      base_directory: z
        .string()
        .optional()
    ...
    process: z.object({
      ...
      offset: z
        ...
        .min(0)
        .optional()
        .describe("Starting offset for log action"),
      limit: z
        .number()
        .int()
        .min(1)
      ...
      data: z
        .string()
        .optional()
        .describe("Data to write to stdin (write action only)"),
      eof: z
        .boolean()
        .optional()
        .describe("Send EOF after writing data (write action only)"),
    }),
  };
  ...
  private readonly skillSchemas = {
    ...
    skill_run: z.object({
      ...
      script: z
        ...
        .min(1)
        .describe(
          "Script path under the skill root, usually scripts/<name>.<ext>",
        ),
      args: z
        .array(z.string())
        .optional()
      ...
      background: z
        ...
        .default(false)
        .describe(
          "Run the script in the background and manage it with process tool",
        ),
      timeoutMs: z
        .number()
        .min(100)
    ...
    skill_manage: z.discriminatedUnion("action", [
      z.object({
        ...
        content: z
          .string()
          .describe(
            "Complete SKILL.md document including frontmatter and body",
          ),
      }),
      z.object({
        action: z
        ...
        draftId: z
          .string()
          .describe("Opaque draft ID returned by skill_manage create"),
        content: z
          .string()
          .describe(
            "Complete SKILL.md document including frontmatter and body",
      ...
      z.object({
        ...
        filePath: z
          .string()
          .describe(
            "Relative file path under references/, templates/, scripts/, or assets/",
          ),
  ...
  public syncContext(context: {
    ...
    const effectiveWorkspacePath = isAgentMode
      ? context.agentWorkspacePath?.trim() ||
        this.getDefaultAgentWorkspacePath()
      : null;
  ...
  async getAllToolDefinitions(context: {
    ...
    if (
      ...
      try {
        const subagentToolDefinition =
          await this.subagentOrchestratorTool.getToolDefinition(
            context.conversationId,
          );
    ...
    if (isAgentMode && this.isSkillsEnabled()) {
      ...
      if (
        context.conversationId &&
        (await this.hasRunnableSkillScripts(context.conversationId))
      ) {
    ...
    if (isAgentMode && this.isSkillsEnabled() && context.conversationId) {
      try {
        const activeSkills = await this.getSkillPresenter().getActiveSkills(
          context.conversationId,
        );
        if (activeSkills.includes(CHAT_SETTINGS_SKILL_NAME)) {
          ...
          const settingsDefs = buildChatSettingsToolDefinitions(
    ...
    if (isAgentMode) {
      try {
        ...
        logger.warn("[AgentToolManager] Failed to load YoBrowser tools", {
          error,
        });
      }
    }
  ...
  async callTool(
    ...
    if (toolName === SUBAGENT_ORCHESTRATOR_TOOL_NAME) {
      ...
      return await this.subagentOrchestratorTool.call(
        args,
        conversationId,
        options,
      );
    ...
    if (this.isFileSystemTool(toolName)) {
      if (!this.fileSystemHandler) {
        throw new Error(
          `FileSystem handler not initialized for tool: ${toolName}`,
        );
      }
      return await this.callFileSystemTool(
        toolName,
        args,
        conversationId,
        options,
      ...
      try {
        ...
        return {
          ...
          rawData: {
            content,
            isError: true,
            toolResult: createAgentToolErrorResult(
              toolName,
  ...
  private async getWorkdirForConversation(
    conversationId: string,
  ): Promise<string | null> {
    try {
      ...
      if (!this.isConversationNotFoundError(error)) {
        logger.warn(
          "[AgentToolManager] Failed to resolve conversation workdir:",
          {
            conversationId,
            error,
          },
  ...
  private async callProcessTool(
    ...
    if (!validationResult.success) {
      throw new Error(
        `Invalid arguments for process: ${validationResult.error.message}`,
      );
    }

    const { action, sessionId, offset, limit, data, eof } =
      validationResult.data;

    switch (action) {
      case "list": {
        const sessions =
          await backgroundExecSessionManager.list(conversationId);
        return {
  ...
  private async callFileSystemTool(
    ...
    const schema =
      this.fileSystemSchemas[toolName as keyof typeof this.fileSystemSchemas];
    if (!schema) {
    ...
    if (!validationResult.success) {
      throw new Error(
        `Invalid arguments for ${toolName}: ${validationResult.error.message}`,
      );
    }
    ...
    if (conversationId) {
      try {
        ...
        logger.warn(
          "[AgentToolManager] Failed to get workdir for conversation:",
          {
            conversationId,
            error,
          },
    ...
    try {
      switch (toolName) {
        ...
        case "write":
          ...
          return {
            content: await fileSystemHandler.writeFile(
              parsedArgs,
              baseDirectory,
            ),
          };
              ...
              content: await fileSystemHandler.editText(
                {
                  ...
                  edits: [
                    { oldText: editArgs.oldText, newText: editArgs.newText },
                  ],
                  dryRun: false,
                },
                baseDirectory,
  ...
  private async buildAllowedDirectories(
    ...
    if (conversationId && includeSkillRoots) {
      const activeSkillRoots =
        await this.resolveActiveSkillRoots(conversationId);
  ...
  private async resolveActiveSkillRoots(
    ...
    let metadataList: Awaited<
      ReturnType<typeof skillPresenter.getMetadataList>
    >;
    ...
    for (const skillName of activeSkillNames) {
      ...
      if (!skillRoot) {
        logger.warn(
          "[AgentToolManager] Active skill root missing during file allowlist build",
          {
            conversationId,
            skillName: normalizedSkillName,
          },
      ...
      try {
        ...
        if (
          ...
          logger.warn(
            "[AgentToolManager] Active skill root is not a directory",
            {
              conversationId,
              skillName: normalizedSkillName,
        ...
        logger.warn(
          "[AgentToolManager] Failed to normalize active skill root",
          {
            conversationId,
            skillName: normalizedSkillName,
            skillRoot,
  ...
  private async resolveValidatedReadPath(
    ...
    const resolvedPath = fileSystemHandler.resolvePath(
      requestedPath,
      baseDirectory,
    );
    ...
    if (
      !allowExternalFileAccess &&
      !fileSystemHandler.isPathAllowedAbsolute(resolvedPath)
    ) {
      throw new Error(
    ...
    try {
      ...
      if (
        !allowExternalFileAccess &&
        !fileSystemHandler.isPathAllowedAbsolute(realPath)
      ) {
        throw new Error(
  ...
  private buildImageMetadataBlock(
    filePath: string,
    mimeType: string,
    fileSize: number,
  ): string {
    ...
    try {
      ...
      logger.warn("[AgentToolManager] Failed to read image dimensions", {
        filePath,
        error,
      });
    }
  ...
  private async readImageWithVisionFallback(
    ...
    throwIfAbortRequested(signal);
    const metadata = this.buildImageMetadataBlock(
      filePath,
      mimeType,
      fileBuffer.length,
    );
    ...
    if (this.runtimePort.cacheImage) {
      try {
        ...
        logger.warn("[AgentToolManager] Failed to cache image preview", {
          filePath,
          error,
        });
      }
    ...
    let visionTarget: Awaited<
      ReturnType<typeof this.resolveVisionTargetForConversation>
    >;

    try {
      visionTarget = await this.resolveVisionTargetForConversation(
        conversationId,
        signal,
      );
    } catch (error) {
      logger.warn(
        "[AgentToolManager] Failed to resolve vision target for image read:",
        {
          conversationId,
          filePath,
          error,
    ...
    try {
      ...
      if (signal) {
        await llmProviderPresenter.executeWithRateLimit(
          visionTarget.providerId,
          { signal },
        );
      } else {
  ...
  private async resolveVisionTargetForConversation(
    conversationId?: string,
    signal?: AbortSignal,
  ) {
    if (!conversationId) {
    ...
    try {
      const sessionInfo =
        await this.runtimePort.resolveConversationSessionInfo(conversationId);
  ...
  private async resolveContainmentTarget(
    resolvedPath: string,
  ): Promise<string> {
    try {
      return await fs.promises.realpath(resolvedPath);
  ...
  private collectReadTargets(
    ...
    switch (toolName) {
      ...
      case "find": {
        const pathArg = args.path;
        return typeof pathArg === "string" && pathArg.trim().length > 0
          ? [pathArg]
          : [];
      }
      case GLOB_TOOL_NAME: {
        ...
        return this.collectPathScopeReadTargets(
          (options as Record<string, unknown>).pathScope,
        );
      }
      case GREP_TOOL_NAME:
  ...
  private async hasRunnableSkillScripts(
    ...
    try {
      ...
      for (const skillName of activeSkills) {
        const scripts =
          await this.getSkillPresenter().listSkillScripts(skillName);
      ...
      logger.warn(
        "[AgentToolManager] Failed to inspect runnable skill scripts",
        {
          conversationId,
          error,
        },
    ...
    if (this.isFileSystemTool(toolName)) {
      ...
      if (conversationId) {
        try {
          ...
          logger.warn(
            "[AgentToolManager] Failed to get workdir for permission check:",
            {
              conversationId,
              error,
            },
      ...
      const explicitBaseDirectory =
        typeof args.base_directory === "string" &&
        args.base_directory.trim().length > 0
          ? args.base_directory
          : undefined;
      const baseDirectory =
        explicitBaseDirectory ?? dynamicWorkdir ?? undefined;
      ...
      if (toolName === "exec") {
        ...
        if (!allowExternalFileAccess && requestedCwd) {
          ...
          const resolvedCwd = fileSystemHandler.resolvePath(
            requestedCwd,
            defaultCwd,
          );
        ...
        if (this.bashHandler.checkCommandPermission) {
          ...
          if (result.needsPermission) {
            return {
              ...
              description:
                result.description ||
                `Command "${command}" requires permission`,
              command,
  ...
  private async callSkillTool(
    ...
    if (toolName === "skill_view") {
      ...
      if (!validationResult.success) {
        throw new Error(
          `Invalid arguments for skill_view: ${validationResult.error.message}`,
        );
      }
      ...
      return {
        ...
        rawData: {
          ...
          toolResult: {
            ...
            activationSource,
            ...(activationApplied
              ? { activatedSkill: validationResult.data.name }
              : {}),
          },
        },
      };
    ...
    if (toolName === "skill_manage") {
      ...
      if (!validationResult.success) {
        throw new Error(
          `Invalid arguments for skill_manage: ${validationResult.error.message}`,
        );
      }
      const result = await skillTools.handleSkillManage(
        conversationId,
        validationResult.data,
      );
      return {
  ...
  private buildSkillManageToolResult(
  ...
  private async callSkillExecutionTool(
    ...
    if (!validationResult.success) {
      throw new Error(
        `Invalid arguments for skill_run: ${validationResult.error.message}`,
      );
    }

    const result = await this.getSkillExecutionService().execute(
      validationResult.data,
      {
        conversationId,
      },
  ...
  private async callChatSettingsTool(
    ...
    if (toolName === CHAT_SETTINGS_TOOL_NAMES.open) {
      const shouldCheckPermission =
        await this.isChatSettingsSkillActive(conversationId);
      if (shouldCheckPermission && conversationId) {
        const approved = this.runtimePort.consumeSettingsApproval(
          conversationId,
          toolName,
        );
        if (!approved) {
