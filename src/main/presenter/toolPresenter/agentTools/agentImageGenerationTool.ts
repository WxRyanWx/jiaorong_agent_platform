...
const imageGenerateSchema = z
  .object({
    ...
    size: z
      ...
      .optional()
      .describe(
        "Optional output size, such as 1024x1024, 1536x1024, or 1024x1536.",
      ),
    quality: z
      .enum(IMAGE_GENERATION_QUALITY_VALUES)
      .optional()
      .describe(
        "Optional quality hint when the selected image model supports it.",
      ),
    outputFormat: z
      .enum(IMAGE_GENERATION_OUTPUT_FORMAT_VALUES)
      .optional()
      .describe(
        "Optional output format hint when the selected image model supports it.",
      ),
    background: z
      .enum(OPENAI_IMAGE_GENERATION_BACKGROUND_VALUES)
      .optional()
      .describe(
        "Optional background hint when the selected image model supports it.",
      ),
    moderation: z
      .enum(IMAGE_GENERATION_MODERATION_VALUES)
      .optional()
      .describe(
        "Optional moderation hint when the selected image model supports it.",
      ),
  })
  .strict();
...
export class AgentImageGenerationTool {
  ...
  getToolDefinition(): MCPToolDefinition {
    return {
      ...
      function: {
        ...
        description:
          "Generate a new image from a text prompt using the JiaorongAI Agent configured image generation model. Use this when the user asks to create, draw, render, or generate an image. The generated image is returned as a JiaorongAI image preview, not as text.",
  ...
  async call(
    ...
    if (!parsed.success) {
      throw new Error(
        `Invalid arguments for ${IMAGE_GENERATE_TOOL_NAME}: ${parsed.error.message}`,
      );
    }
    ...
    if (!model) {
      return this.buildErrorResult(
        ...
        "No available image generation model is configured for this JiaorongAI Agent.",
        parsed.data,
      );
    }
    ...
    try {
      ...
      const imagePreviews = result.images.map<ToolCallImagePreview>(
        (image, index) => ({
      ...
      const metadataImages = imagePreviews.map(
        ({ id, mimeType, title, source }) => ({
          id,
          mimeType,
          title,
          source,
      ...
      return {
        ...
        rawData: {
          ...
          isError: false,
          imagePreviews,
          toolResult: createAgentToolSuccessResult(
            IMAGE_GENERATE_TOOL_NAME,
            content,
            {
  ...
  private async resolveImageGenerationModel(
    ...
    try {
      const session =
        await this.options.runtimePort.resolveConversationSessionInfo(
          conversationId,
        );
      ...
      const config =
        await this.options.configPresenter.resolveDeepChatAgentConfig(
          session.agentId,
        );
      ...
      if (!this.isSupportedImageGenerationModel(providerId, modelId)) {
        logger.warn(
          "[AgentImageGenerationTool] Configured model is not an image model",
          {
            providerId,
            modelId,
            conversationId,
      ...
      logger.warn(
        "[AgentImageGenerationTool] Failed to resolve image generation model",
        {
          conversationId,
          error,
        },
  ...
  private isSupportedImageGenerationModel(
    ...
    try {
      const modelConfig = this.options.configPresenter.getModelConfig(
        modelId,
        providerId,
      );
      return (
      ...
      logger.warn(
        "[AgentImageGenerationTool] Failed to inspect image generation model config",
        {
          providerId,
          modelId,
          error,
  ...
  private toImageGenerationOptions(
