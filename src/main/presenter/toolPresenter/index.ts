...
export class ToolPresenter implements IToolPresenter {
  ...
  /**
   * UI-only tool display catalog. Reads displayName/description from tool definitions
   * without session gating so historical tool calls can still localize labels.
   */
  async getToolDisplayCatalog(): Promise<ToolDisplayMetadata[]> {
    const mcpDefs = (await this.options.mcpPresenter.getAllToolDefinitions()).map(
      toToolDisplayMetadata
    )
    const agentDefs = this.ensureAgentToolManager(null).getToolDisplayCatalog()
    return mergeToolDisplayMetadata(mcpDefs, agentDefs)
  }
