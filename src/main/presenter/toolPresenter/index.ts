...
export class ToolPresenter implements IToolPresenter {
  ...
  private ensureAgentToolManager(
    agentWorkspacePath: string | null,
  ): AgentToolManager {
  ...
  async getAllToolDefinitions(context: {
    ...
    try {
      ...
      const disabledAgentToolSet = new Set(
        normalizeToolNames(context.disabledAgentTools),
      );
      ...
      console.warn(
        "[ToolPresenter] Failed to load Agent tool definitions",
        error,
      );
    }
  ...
  async callTool(
    ...
    if (source === "agent") {
      if (!this.agentToolManager) {
        throw new Error(
          `Agent tool manager not initialized for tool ${toolName}`,
        );
      }
      ...
      if (argsString.trim().length > 0) {
        try {
          ...
          console.warn(
            "[ToolPresenter] Failed to parse tool arguments, trying jsonrepair:",
            error,
          );
          try {
            args = JSON.parse(jsonrepair(argsString)) as Record<
              string,
              unknown
            >;
          } catch (error) {
  ...
  async preCheckToolPermission(
    ...
    if (!source) {
      console.warn(
        `[ToolPresenter] Tool ${toolName} not found for permission check`,
      );
      return null;
    }
  ...
  private resolveAgentToolResponse(
    response: AgentToolCallResult | string,
  ): AgentToolCallResult {
  ...
  private resolveMapper(conversationId?: string): ToolMapper {
    ...
    const existingMapper = this.conversationMappers.get(
  ...
  private buildFilesystemPrompt(
    toolNames: Set<string>,
    offloadPath: string,
  ): string {
    const filesystemTools = FILESYSTEM_TOOL_ORDER.filter((toolName) =>
    ...
    if (hasGlob || hasGrep) {
      if (hasGlob && hasGrep) {
        ...
        lines.push(
          "Use `glob` for file discovery; it returns structured JSON.",
        );
      } else {
        lines.push(
    ...
    if (hasOffloadTools) {
      ...
      lines.push(
        `When you see an offload stub, the full output is stored at: ${offloadPath}`,
      );
      if (toolNames.has("read")) {
        lines.push(
          "Use `read` to inspect that path when you need the full output.",
        );
      }
    }
  ...
  private buildSkillsPrompt(toolNames: Set<string>): string {
    ...
    if (toolNames.has("skill_list")) {
      lines.push(
        "- Use `skill_list` to inspect installed skills and pinned status.",
      );
      hasContent = true;
    }
    ...
    if (toolNames.has("skill_run")) {
      lines.push(
        "- Use `skill_run` to execute bundled scripts from pinned skills.",
      );
      hasContent = true;
    }
  ...
  private buildTapePrompt(tools: MCPToolDefinition[]): string {
    ...
    if (toolNames.has(TAPE_TOOL_NAMES.info)) {
      lines.push(
        "`tape_info` inspects this JiaorongAI-scoped tape subset inspired by bub tape.info.",
      );
    }
  ...
  private buildYoBrowserPrompt(tools: MCPToolDefinition[]): string {
    ...
    if (toolNames.has("get_browser_status")) {
      lines.push(
        "- Use `get_browser_status` to inspect the current session browser state.",
      );
    }
