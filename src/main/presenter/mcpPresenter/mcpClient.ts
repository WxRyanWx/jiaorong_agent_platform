...
function isSessionError(error: unknown): error is SessionError {
  if (error instanceof Error) {
    ...
    const hasSessionPattern = sessionErrorPatterns.some((pattern) =>
    ...
    const hasHttpPattern = httpErrorPatterns.some((pattern) =>
      message.includes(pattern),
    );
    if (
      hasHttpPattern &&
      (message.includes("posting") || message.includes("endpoint"))
    ) {
      return true;
    }
  }
...
export class McpClient {
  ...
  async connect(): Promise<void> {
    ...
    try {
      console.info(
        `Starting MCP server ${this.serverName}...`,
        this.serverConfig,
      );
      ...
      if (this.serverConfig.type === "inmemory") {
        ...
        const _args = Array.isArray(this.serverConfig.args)
          ? this.serverConfig.args
          : [];
        const _env = this.serverConfig.env
          ? (this.serverConfig.env as Record<string, unknown>)
          : {};
        ...
        const processedCommand = this.runtimeHelper.processCommandWithArgs(
          command,
          args,
        );
        ...
        const isNodeCommand = ["node", "npm", "npx", "uv", "uvx"].some(
          (cmd) =>
            command.includes(cmd) || args.some((arg) => arg.includes(cmd)),
        );
        ...
        if (isNodeCommand) {
          ...
          if (process.env) {
            ...
            // 规范化并设置PATH
            const { key, value } =
              this.runtimeHelper.normalizePathEnv(allPaths);
            env[key] = value;
          }
        } else {
        ...
        (this.transport as StdioClientTransport).stderr?.on("data", (data) => {
          console.info(
            "mcp StdioClientTransport error",
            this.serverName,
            data.toString(),
          );
        });
        ...
        throw new Error(
          `Unsupported transport type: ${this.serverConfig.type}`,
        );
      }
      ...
      const timeoutPromise = new Promise<void>((_, reject) => {
        this.connectionTimeout = setTimeout(
          () => {
            console.error(
              `Connection to MCP server ${this.serverName} timed out`,
            );
            reject(
              new Error(
                `Connection to MCP server ${this.serverName} timed out`,
  ...
  private registerNotificationHandlers(): void {
    ...
    this.client.setNotificationHandler(
      ...
      async () => {
        ...
        try {
          ...
          console.warn(
            `[MCP] Failed to refresh tools after notification:`,
            error,
          );
        }
      },
    ...
    this.client.setNotificationHandler(
      ...
      async () => {
        console.info(
          `[MCP] Prompts list changed for server: ${this.serverName}`,
        );
        this.cachedPrompts = null;
        ...
        try {
          ...
          console.warn(
            `[MCP] Failed to refresh prompts after notification:`,
            error,
          );
        }
      },
    ...
    this.client.setNotificationHandler(
      ...
      async () => {
        console.info(
          `[MCP] Resources list changed for server: ${this.serverName}`,
        );
        this.cachedResources = null;
        ...
        try {
          ...
          console.warn(
            `[MCP] Failed to refresh resources after notification:`,
            error,
          );
        }
      },
    ...
    this.client.setNotificationHandler(
      ...
      async (params) => {
        console.info(
          `[MCP] Resource updated for server: ${this.serverName}`,
          params,
        );
        this.cachedResources = null;
        // Actively refresh resource list
        try {
          ...
          console.warn(
            `[MCP] Failed to refresh resources after update notification:`,
            error,
          );
        }
      },
    ...
    this.client.setNotificationHandler(
      ...
      async (params) => {
        console.info(
          `[MCP] Log message from server ${this.serverName}:`,
          params,
        );
      },
    );
  ...
  private async handleSamplingCreateMessage(
    ...
    const { payload, chatMessages } = this.prepareSamplingContext(
      requestId,
      params,
    );

    const decisionPromise =
      presenter.mcpPresenter.handleSamplingRequest(payload);
    ...
    try {
      assistantText =
        await presenter.llmproviderPresenter.generateCompletionStandalone(
          decision.providerId,
          chatMessages,
          decision.modelId,
          undefined,
    ...
    const modelName =
      this.resolveModelDisplayName(decision.providerId, decision.modelId) ??
      decision.modelId;
  ...
  private prepareSamplingContext(
    ...
    const payload: McpSamplingRequestPayload = {
      ...
      modelPreferences: this.normalizeModelPreferences(
    ...
    for (const message of messageList) {
      ...
      if (content.type === "text") {
        ...
        if (
          normalizedMimeType &&
          !ALLOWED_SAMPLING_IMAGE_MIME_TYPES.has(normalizedMimeType)
        ) {
          throw new McpError(
            ErrorCode.InvalidParams,
  ...
  private getServerLabel(): string | undefined {
    ...
    const candidates: Array<string | undefined> = [
      typeof config["descriptions"] === "string"
        ? (config["descriptions"] as string)
        : undefined,
  ...
  private resolveModelDisplayName(
    ...
    try {
      const models =
        presenter.configPresenter.getProviderModels(providerId) || [];
      ...
      const customModels =
        presenter.configPresenter.getCustomModels?.(providerId) || [];
  ...
  private async checkAndHandleSessionError(error: unknown): Promise<void> {
    if (isSessionError(error) && !this.isRecovering) {
      ...
      try {
        ...
        console.info(
          `Service ${this.serverName} restarted due to session error`,
        );
      } catch (restartError) {
        console.error(
          `Failed to restart service ${this.serverName}:`,
          restartError,
        );
      } finally {
          ...
          const validPrompts = promptsArray.map((p) => ({
            ...
            description:
              typeof p === "object" && p !== null && "description" in p
                ...
                : undefined,
            arguments:
              typeof p === "object" && p !== null && "arguments" in p
                ? p.arguments
                : undefined,
            files:
              typeof p === "object" && p !== null && "files" in p
                ? p.files
                : undefined,
          })) as PromptListEntry[];
          // 缓存结果
  ...
  async listResources(): Promise<ResourceListEntry[]> {
    ...
    try {
      ...
      if (response && typeof response === "object" && "resources" in response) {
        ...
        if (Array.isArray(resourcesArray)) {
          ...
          const validResources = resourcesArray.map((r) => ({
            uri:
              typeof r === "object" && r !== null && "uri" in r
                ? String(r.uri)
                : "unknown",
            name:
      ...
      if (
        ...
        console.warn(
          `Server ${this.serverName} does not support listResources`,
        );
