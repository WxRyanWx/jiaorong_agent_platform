...
export class ShortcutPresenter implements IShortcutPresenter {
  ...
  private registerSystemShortcuts(): void {
    ...
    if (this.shortcutKeys.ShowHideWindow) {
      const ok = globalShortcut.register(this.shortcutKeys.ShowHideWindow, () => {
        eventBus.sendToMain(TRAY_EVENTS.SHOW_HIDDEN_WINDOW)
      })
      if (!ok) {
        logger.warn(`failed to register show/hide shortcut: ${this.shortcutKeys.ShowHideWindow}`)
      }
    }

    if (this.shortcutKeys.Screenshot) {
      // 截图快捷键必须跟随统一注册流程；这里会 unregisterAll，独立注册会被后续刷新清掉。
      const screenshotOk = globalShortcut.register(this.shortcutKeys.Screenshot, () => {
        openScreenShotWindow('hotkey')
      })
      if (!screenshotOk) {
        logger.warn(`failed to register screenshot shortcut: ${this.shortcutKeys.Screenshot}`)
      }
    }
  }
