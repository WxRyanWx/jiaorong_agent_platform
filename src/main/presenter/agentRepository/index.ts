...
const mergeDeepChatConfig = (
  ...
  normalizeDeepChatSubagentConfig({
    defaultModelPreset:
      overrideConfig.defaultModelPreset ??
      baseConfig.defaultModelPreset ??
      null,
    assistantModel:
      overrideConfig.assistantModel ?? baseConfig.assistantModel ?? null,
    visionModel: overrideConfig.visionModel ?? baseConfig.visionModel ?? null,
    imageGenerationModel:
      overrideConfig.imageGenerationModel ??
      baseConfig.imageGenerationModel ??
      null,
    defaultProjectPath:
      overrideConfig.defaultProjectPath ??
      baseConfig.defaultProjectPath ??
      null,
    systemPrompt: overrideConfig.systemPrompt ?? baseConfig.systemPrompt ?? "",
    permissionMode:
      overrideConfig.permissionMode ??
      baseConfig.permissionMode ??
      "full_access",
    disabledAgentTools:
      overrideConfig.disabledAgentTools ?? baseConfig.disabledAgentTools ?? [],
    subagentEnabled:
      overrideConfig.subagentEnabled ?? baseConfig.subagentEnabled ?? true,
    subagents:
      overrideConfig.subagents ??
      baseConfig.subagents ??
      createDefaultDeepChatSubagentSlots(),
    autoCompactionEnabled:
      overrideConfig.autoCompactionEnabled ??
      baseConfig.autoCompactionEnabled ??
      true,
...
export class AgentRepository {
  ...
  ensureBuiltinDeepChatAgent(defaults?: {
    ...
    const existing = this.sqlitePresenter.agentsTable.get(
  ...
  updateDeepChatAgent(
    ...
    this.sqlitePresenter.agentsTable.update(agentId, {
      ...
      icon:
        updates.icon === undefined ? row.icon : sanitizeString(updates.icon),
      avatarJson:
        updates.avatar === undefined
          ? row.avatar_json
          : stringifyJson(updates.avatar ?? null),
      configJson:
        updates.config === undefined
          ? row.config_json
  ...
  resolveDeepChatAgentConfig(agentId: string): DeepChatAgentConfig {
    const builtin =
      this.getDeepChatAgentConfig(BUILTIN_DEEPCHAT_AGENT_ID) ?? {};
  ...
  syncRegistryAgents(
    ...
    for (const agent of agents) {
      ...
      const currentState =
        parseJson<StoredAgentState>(currentRow?.state_json) ?? {};
  ...
  clearRegistryAcpAgentInstallation(
    agentId: string,
    installState: AcpAgentInstallState,
  ): boolean {
    const row = this.sqlitePresenter.agentsTable.get(agentId);
