  ...
  getConversionWarnings(
    ...
    if (
      ...
      warnings.push({
        ...
        field: "allowedTools",
        suggestion:
          "Consider adding tool restrictions as a note in the instructions",
      });
    }
    ...
    if (skill.model && !capabilities.supportsModel) {
      warnings.push({
        ...
        field: "model",
        suggestion:
          "Consider adding model preference as a note in the instructions",
      });
    }
    ...
    if (
      capabilities.supportsSubfolders === false &&
      (skill.references || skill.scripts)
    ) {
      if (
        !warnings.some((w) => w.field === "references" || w.field === "scripts")
      ) {
        warnings.push({
          type: "format_change",
      ...
      if (adapter) {
        ...
        if (!adapterCaps.hasFrontmatter) {
          warnings.push({
            type: "feature_loss",
            message:
              "Tags will be lost (target format does not support metadata)",
            field: "tags",
          });
        }
      }
  ...
  private async loadSubfolderFiles(
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const sizeResult = await validateFileSize(
          filePath,
          MAX_SUBFOLDER_FILE_SIZE,
        );
        if (!sizeResult.valid) {
          console.warn(
            `Skipping oversized file ${filePath}: ${sizeResult.error}`,
          );
          continue;
        }
