...
export class DevicePresenter implements IDevicePresenter {
      ...
      switch (resetType) {
          ...
          auxiliaryFiles.forEach((fileName) => {
            ...
            if (fs.existsSync(filePath)) {
              try {
                ...
                console.warn(
                  "Failed to clean auxiliary file:",
                  fileName,
                  error,
                );
              }
        ...
        case "knowledge": {
          ...
          try {
            ...
            console.warn(
              "Error closing knowledge database connections:",
              closeError,
            );
          }
  ...
  async sanitizeSvgContent(svgContent: string): Promise<string | null> {
    try {
      ...
      if (sanitizedContent) {
        logger.info(
          "SVG content sanitized successfully, output length:",
        ...
        // Debug: 检查具体是哪一步失败了
        logger.info(
          "Debug: SVG starts with <svg:",
          svgContent.trim().startsWith("<svg"),
        );
        logger.info(
          "Debug: SVG contains dangerous content:",
