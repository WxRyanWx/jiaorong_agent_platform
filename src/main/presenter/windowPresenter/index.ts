...
export class WindowPresenter implements IWindowPresenter {
  ...
  constructor(
    ...
    eventBus.on(SHORTCUT_EVENTS.GO_SETTINGS, async () => {
      try {
        ...
        console.error(
          "Failed to open/focus settings window via eventBus:",
          err,
        );
      }
    });
    ...
    // 监听更新进程设置应用退出状态的事件
    eventBus.on(
      WINDOW_EVENTS.SET_APPLICATION_QUITTING,
      (data: { isQuitting: boolean }) => {
        logger.info(
          `WindowPresenter: Setting application quitting state to ${data.isQuitting}`,
        );
  ...
  public async openOrFocusSettingsTab(_windowId: number): Promise<void> {
    console.warn(
      "openOrFocusSettingsTab is deprecated. Use openOrFocusSettingsWindow() instead.",
    );
  ...
  previewFile(filePath: string): void {
    ...
    if (
      !targetWindow &&
      this.floatingChatWindow &&
      this.floatingChatWindow.isShowing()
    ) {
  ...
  async sendToAllWindows(channel: string, ...args: unknown[]): Promise<void> {
    ...
    if (this.settingsWindow && !this.settingsWindow.isDestroyed()) {
      try {
        ...
        console.error(
          `Error sending message "${channel}" to settings window:`,
          error,
        );
      }
    }
    ...
    if (this.floatingChatWindow && this.floatingChatWindow.isShowing()) {
      ...
      if (floatingWindow && !floatingWindow.isDestroyed()) {
        try {
          ...
          console.error(
            `Error sending message "${channel}" to floating chat window:`,
            error,
          );
        }
      }
  ...
  sendToWindow(windowId: number, channel: string, ...args: unknown[]): boolean {
    ...
    if (
      ...
      try {
        ...
        console.error(
          `Error sending message "${channel}" to settings window ${windowId}:`,
          error,
        );
        return false;
      }
  ...
  private async createManagedWindow(options?: {
    ...
    const appWindow = new BrowserWindow({
      ...
      trafficLightPosition:
        process.platform === "darwin" ? { x: 12, y: 10 } : undefined, // macOS 红绿灯按钮位置
      webPreferences: {
  ...
  async sendToActiveTab(
    ...
    if (activeTabId) {
      ...
      if (
        ...
        targetWindow.webContents.send(channel, ...args);
        logger.info(
          `  - No active tab, sent event directly to window ${windowId} webContents.`,
        );
        return true;
      }
      console.warn(
  ...
  async sendToDefaultTab(
    ...
    try {
      ...
      if (tabsData.length === 0) {
        ...
        console.warn(
          `Window ${windowId} has no tabs and window is unavailable.`,
        );
        return false;
      }
    ...
    if (this.settingsWindow && !this.settingsWindow.isDestroyed()) {
      ...
      if (navigation) {
        if (this.settingsWindowReady) {
          ...
          logger.info(
            `Settings window is not ready, reloading to target URL: ${targetUrl}`,
          );
  ...
  public setPendingSettingsProviderInstall(
    preview: ProviderInstallPreview,
  ): void {
    this.pendingSettingsProviderInstalls.push(
      this.clonePendingSettingsProviderInstall(preview),
    );
  }
  ...
  private tryNavigateSettingsWindowByUrl(
    channel: string,
    args: unknown[],
  ): boolean {
    if (
    ...
    void this.settingsWindow.webContents
      ...
      .catch((error) => {
        console.error(
          `Failed to reload settings window for navigation: ${targetUrl}`,
          error,
        );
      });
    return true;
  ...
  private toSettingsNavigationPayload(
    raw: unknown,
    ...
    return {
      ...
      section:
        typeof candidate.section === "string" ? candidate.section : undefined,
    };
  }

  private getSettingsWindowTargetUrl(
    navigation?: SettingsNavigationPayload,
  ): string {
  ...
  private validateWindowPosition(
    ...
    const isYValid =
      y >= workArea.y && y + height <= workArea.y + workArea.height;
