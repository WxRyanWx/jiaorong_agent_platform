...
const PLATFORM_SPECIFIC_SERVERS: Record<
  string,
...
const DEFAULT_INMEMORY_SERVERS: Record<
  string,
  Omit<MCPServerConfig, "enabled">
> = {
  ...
  "deepchat-inmemory/conversation-search-server": {
    args: [],
    descriptions: "JiaorongAI built-in conversation history search service",
    icons: "🔍",
    autoApprove: ["all"],
    type: "inmemory" as MCPServerType,
...
const DEFAULT_ENABLED_SERVER_NAMES = [
  "Artifacts",
...
const BUILT_IN_SERVER_NAMES = new Set<string>(
  Object.keys(DEFAULT_MCP_SERVERS.mcpServers),
);
...
export class McpConfHelper {
  ...
  private resolveLegacyEnabledServers(): Set<string> {
    ...
    const oldDefaultServersValue = this.mcpStore.get<string[]>(
      "defaultServers",
      [],
    );
    const oldDefaultServers = Array.isArray(oldDefaultServersValue)
      ? oldDefaultServersValue
      : [];
  ...
  private unmarkBuiltInServerRemoved(name: string): void {
    if (!this.isBuiltInServer(name)) return;
    const removed = this.getRemovedBuiltInServers().filter(
      (server) => server !== name,
    );
  ...
  migrateBuiltinKnowledgeConfigsFromEnv(
    ...
    const mcpServers = this.mcpStore.get<Record<string, MCPServerConfig>>(
      "mcpServers",
      {},
    );
    ...
    if (typeof rawEnv === "string") {
      try {
        ...
        console.warn(
          "Failed to parse builtinKnowledge env for migration:",
          error,
        );
        return existingConfigs;
      }
  ...
  async getMcpServers(): Promise<Record<string, MCPServerConfig>> {
    ...
    if (
      ...
      Object.entries(updatedServers).some(
        ([serverName, config]) =>
          storedServers[serverName]?.enabled !== config.enabled,
      )
    ) {
  ...
  public onUpgrade(oldVersion: string | undefined): void {
    ...
    if (oldVersion && compare(oldVersion, "0.6.0", "<")) {
      try {
        ...
        if (mcpServers.filesystem) {
          logger.info(
            "Removing old filesystem MCP server (now provided via Agent tools)",
          );
        ...
        if (mcpServers.buildInFileSystem) {
          logger.info(
            "Removing buildInFileSystem MCP server (now provided via Agent tools)",
          );
        ...
        if (hasChanges) {
          this.mcpStore.set("mcpServers", mcpServers);
          logger.info(
            "Migration: filesystem MCP servers removed (now available via Agent tools)",
          );
        }
      } catch (error) {
        console.error(
          "Error occurred while migrating filesystem server:",
          error,
        );
      }
    }
    ...
    if (oldVersion && compare(oldVersion, "0.3.5", "<")) {
      try {
        ...
        if (mcpServers[customPromptsServerName]) {
          logger.info(
            "Detected old version custom-prompts-server, starting removal",
          );
        ...
        console.error(
          "Error occurred while removing custom-prompts-server:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "Error occurred while removing deprecated built-in MCP servers:",
        error,
      );
    }
    ...
    try {
      ...
      if (
        ...
        logger.info(
          "Detected macOS platform, adding Apple system integration service",
        );
        mcpServers["deepchat/apple-server"] = {
          ...(PLATFORM_SPECIFIC_SERVERS[
            "deepchat/apple-server"
      ...
      console.error(
        "Error occurred while upgrading platform-specific services:",
        error,
      );
    }
