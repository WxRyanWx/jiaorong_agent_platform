...
import {
  ...
  getDefaultSystemPromptContentHash
...
export class ConfigPresenter implements IConfigPresenter {
  ...
  setSQLitePresenter(sqlitePresenter: SQLitePresenter): void {
    try {
      ...
      void this.syncDefaultSystemPromptFromCode()
    } catch (error) {
  ...
  private async syncDefaultSystemPromptFromCode(): Promise<void> {
    if (!this.dbBackedSettingsStore) {
      return
    }

    const hashKey = 'defaultSystemPromptContentHash'
    const currentHash = getDefaultSystemPromptContentHash()
    if (this.getSetting<string>(hashKey) === currentHash) {
      return
    }

    const prompts = (await this.getSystemPrompts()) ?? []
    const now = Date.now()
    const defaultIndex = prompts.findIndex((prompt) => prompt.id === 'default')
    const nextPrompts =
      defaultIndex >= 0
    ...
    this.setSetting(hashKey, currentHash)
  }
