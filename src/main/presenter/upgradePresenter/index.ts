...
export class UpgradePresenter implements IUpgradePresenter {
  ...
  constructor(configPresenter: IConfigPresenter) {
    ...
    autoUpdater.on('update-available', (info) => {
      ...
      // Dev/unpackaged: allow status checks only — skip auto download (install path is unreliable)
      if (this._lastCheckType === 'autoCheck' && app.isPackaged) {
  ...
  async checkUpdate(type?: string): Promise<void> {
    ...
    try {
      ...
      // When updater is inactive (or returns null without emitting events), leave checking.
      if (!result && this._status === 'checking') {
        this._status = 'not-available'
        this._error = null
        this._progress = null
        this._versionInfo = null
