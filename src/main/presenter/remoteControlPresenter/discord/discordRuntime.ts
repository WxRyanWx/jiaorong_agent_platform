...
import {
  ...
  type DiscordGatewayBotUser,
} from "./discordGatewaySession";
...
const DISCORD_INTERNAL_ERROR_REPLY =
...
const chunkDiscordText = (
  text: string,
  limit: number = DISCORD_OUTBOUND_TEXT_LIMIT,
): string[] => {
  ...
  while (remaining.length > limit) {
    ...
    const nextIndex =
      splitIndex > Math.floor(limit * 0.55) ? splitIndex : limit;
...
export class DiscordRuntime {
  ...
  private readonly processedInboundByMessage = new Map<
    string,
    DiscordProcessedInboundEntry
  >();
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (!this.applicationId) {
        throw new Error(
          "Discord application ID is missing from the READY payload.",
        );
      }

      await this.deps.client.registerCommands(
        this.applicationId,
  ...
  private async acceptDispatch(
    ...
    const parsed = this.deps.parser.parseDispatch(
      payload,
      this.statusSnapshot.botUser?.id,
    );
    if (!parsed) {
      return;
    }
  ...
  private pruneProcessedInbound(now: number): void {
    for (const [
      messageKey,
      entry,
    ] of this.processedInboundByMessage.entries()) {
  ...
  private enqueueEndpointOperation(
    ...
    const previous =
      this.endpointOperations.get(endpointKey) ?? Promise.resolve();
    const next = previous
      .catch(() => undefined)
      .then(async () => {
        if (!this.isCurrentRun(runId)) {
  ...
  private async processInboundMessage(
    ...
    try {
      ...
      if (this.deps.logger?.error) {
        ...
        console.error(
          "[DiscordRuntime] Failed to handle dispatch:",
          error,
          diagnostics,
        );
      }
      ...
      if (parsed.kind === "interaction") {
        await this.safeRespondToInteraction(parsed, [
          DISCORD_INTERNAL_ERROR_REPLY,
        ]);
        return;
      }
  ...
  private async dispatchInteractionRouteResult(
    ...
    await this.deps.client.deferInteractionResponse(
      message.interactionId,
      message.interactionToken,
    );
    if (!this.isCurrentRun(runId)) {
  ...
  private async deliverConversation(
    ...
    const endpointKey = buildDiscordEndpointKey(
      target.chatType,
      target.channelId,
    );
    ...
    while (this.isCurrentRun(runId)) {
      ...
      let deliverySegments = this.getSnapshotDeliverySegments(
        snapshot,
        sourceMessageId,
      );
      ...
      if (sourceMessageId) {
        deliveryState =
          deliveryState ?? this.createDeliveryState(sourceMessageId);
      }
      ...
      if (Date.now() - startedAt >= FEISHU_CONVERSATION_POLL_TIMEOUT_MS) {
        ...
        if (deliveryState) {
          deliveryState = await this.syncDeliverySegments(
            ...
            endpointKey,
            deliveryState,
            this.appendTerminalDeliverySegment(
              deliverySegments,
              sourceMessageId,
              timeoutText,
            ),
  ...
  private getStoredDeliveryState(
    endpointKey: string,
  ): DiscordRemoteDeliveryState | null {
  ...
  private getSnapshotDeliverySegments(
    ...
    if (snapshot.deliverySegments !== undefined) {
      return snapshot.deliverySegments.filter(
  ...
  private appendTerminalDeliverySegment(
    ...
    const lastAnswerSegment = [...segments]
      .reverse()
      .find((segment) => segment.kind === "answer");
  ...
  private isDeliveryStateCompatible(
    ...
    return state.segments.every(
      (segment, index) => segments[index]?.key === segment.key,
    );
  }
  ...
  private async syncDeliverySegment(
    ...
    if (!existing) {
      ...
      for (const chunk of nextChunks) {
        const messageId = await this.deps.client.sendMessage(
          target.channelId,
          chunk,
        );
        if (messageId) {
    ...
    const previousChunks = existing.lastText
      ? chunkDiscordText(existing.lastText)
      : [];
    if (
      ...
      for (const messageId of existing.messageIds) {
        await this.deps.client
          .deleteMessage(target.channelId, messageId)
          .catch(() => undefined);
      }
    ...
    for (let index = editableIndex; index < retainedCount; index += 1) {
      ...
      await this.deps.client.updateMessage(
        target.channelId,
        messageId,
        nextChunks[index],
      );
  ...
  private setBotUser(
    botUser: DiscordBotIdentity | DiscordGatewayBotUser,
  ): void {
    this.setStatus({
      botUser: {
        id: botUser.id,
  ...
  private handleGatewayStatusChange(
    snapshot: DiscordRuntimeStatusSnapshot,
  ): void {
  ...
  private setStatus(
    patch: Partial<DiscordRuntimeStatusSnapshot> & {
      state?: DiscordRuntimeStatusSnapshot["state"];
    },
  ): void {
    this.statusSnapshot = {
      ...this.statusSnapshot,
