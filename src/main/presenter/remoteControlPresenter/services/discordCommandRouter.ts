...
const DISCORD_PENDING_ALLOWED_COMMANDS = new Set([
  "start",
  "help",
  "status",
  "open",
...
export class DiscordCommandRouter {
  ...
  async handleMessage(
    ...
    try {
      ...
      if (message.kind === "interaction") {
        return {
          replies: [
            "Slash commands only support remote control commands. Send a regular message to chat with JiaorongAI.",
          ],
        };
      }
      ...
      return {
        ...
        conversation:
          message.attachments.length > 0
            ? await this.deps.runner.sendInput(
                endpointKey,
                {
                  ...
                  attachments: message.attachments.map((attachment) => ({
                    ...
                    url: attachment.url,
                    resourceType: attachment.contentType?.startsWith("image/")
                      ? "image"
                      : "file",
                  })),
            ...
            : await this.deps.runner.sendText(
                endpointKey,
                message.text,
                bindingMeta,
              ),
  ...
  private formatModelOverview(
    ...
    return [
      ...
      ...providers.flatMap((provider) => [
        ...
        ...provider.models.map(
          (model) =>
            `- ${model.modelName} (${provider.providerId} ${model.modelId})`,
        ),
      ]),
    ].join("\n");
  }

  private formatAgentOverview(
    session: SessionWithState,
  ...
  private formatStartMessage(
    ...
    if (authorized) {
      return [
        "JiaorongAI Discord Remote is ready.",
    ...
    if (chatType === "channel") {
      return [
        "JiaorongAI Discord Remote is online.",
  ...
  private formatHelpMessage(): string {
    return [
      "JiaorongAI Discord Remote commands:",
      ...DISCORD_REMOTE_COMMANDS.map(
  ...
  private formatSessionLabel(
    session: Pick<SessionWithState, "id" | "title">,
  ): string {
