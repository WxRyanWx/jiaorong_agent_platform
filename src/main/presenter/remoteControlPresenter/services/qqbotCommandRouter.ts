...
import {
  QQBOT_REMOTE_COMMANDS,
  buildQQBotBindingMeta,
...
const QQBOT_PENDING_ALLOWED_COMMANDS = new Set([
  "start",
  "help",
  "status",
  "open",
...
export class QQBotCommandRouter {
  ...
  private formatStartMessage(
    ...
    if (auth.ok) {
      return [
        "JiaorongAI QQBot Remote is ready.",
    ...
    if (chatType === "group") {
      return [
        "JiaorongAI QQBot Remote is online.",
  ...
  private formatHelpMessage(): string {
    return [
      "JiaorongAI QQBot Remote commands:",
      ...QQBOT_REMOTE_COMMANDS.map(
