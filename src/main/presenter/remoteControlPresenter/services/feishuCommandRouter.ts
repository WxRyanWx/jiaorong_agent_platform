...
const FEISHU_PENDING_ALLOWED_COMMANDS = new Set([
  "start",
  "help",
  "status",
  "open",
...
export class FeishuCommandRouter {
  ...
  async handleMessage(
    ...
    const endpointKey = buildFeishuEndpointKey(
      message.chatId,
      message.threadId,
    );
            ...
            ? await this.deps.runner.sendInput(
                endpointKey,
                {
                  ...
                  attachments: attachments.filter(
                    (attachment) => !attachment.failedDownload,
                  ),
  ...
  private formatStartMessage(isAuthorized: boolean): string {
    if (isAuthorized) {
      return [
        "JiaorongAI Feishu Remote is ready.",
        "Send any message to continue the bound session, or /help for commands.",
      ].join("\n");
    }

    return [
      "JiaorongAI Feishu Remote is online.",
  ...
  private formatHelpMessage(): string {
    return [
      "JiaorongAI Feishu Remote commands:",
      ...FEISHU_REMOTE_COMMANDS.map(
