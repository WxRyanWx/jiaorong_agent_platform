...
export type SessionNotificationHandler = (
...
export const parseLoadSessionCapability = (
...
export class AcpProcessManager implements AgentProcessManager<
  AcpProcessHandle,
  AcpAgentConfig
> {
  ...
  private readonly getAgentState?: (
    agentId: string,
  ) => Promise<AcpAgentState | null>;
  ...
  private readonly pendingHandles = new Map<
    string,
    Promise<AcpProcessHandle>
  >();
  private readonly sessionListeners = new Map<string, SessionListenerEntry>();
  private readonly bufferedSessionUpdates = new Map<
    string,
    BufferedSessionUpdate[]
  >();
  private readonly permissionResolvers = new Map<
    string,
    PermissionResolverEntry
  >();
  ...
  private readonly protocolRequestsToAgent = new Map<
    string,
    Map<JsonRpcId, string>
  >();
  private readonly protocolRequestsFromAgent = new Map<
    string,
    Map<JsonRpcId, string>
  >();
  ...
  registerSessionWorkdir(
    ...
    this.fsHandlers.set(
      sessionId,
      new AcpFsHandler({ workspaceRoot: workdir }),
    );
  }
  ...
  private getFsHandler(sessionId: string): AcpFsHandler {
    ...
    if (!handler) {
      ...
      const fallbackHandler = new AcpFsHandler({
        workspaceRoot: fallbackWorkdir,
      });
  ...
  private resolveTerminalCwd(
    ...
    if (sessionWorkdir) {
      ...
      if (explicitCwd) {
        const safeCwd = this.resolveCwdInsideWorkdir(
          sessionWorkdir,
          explicitCwd,
        );
        if (safeCwd) {
  ...
  private getFallbackWorkdir(): string {
    ...
    try {
      ...
      console.warn(
        "[ACP] Failed to create fallback workdir, defaulting to system temp:",
        error,
      );
  ...
  private resolveWorkdir(workdir?: string): string {
    ...
    try {
      ...
      console.warn(
        `[ACP] workdir "${trimmed}" is not accessible; using fallback workdir.`,
        error,
      );
      return this.getFallbackWorkdir();
    }
    ...
    console.warn(
      `[ACP] workdir "${trimmed}" does not exist; using fallback workdir.`,
    );
    return this.getFallbackWorkdir();
  }
  ...
  async setPreferredMode(
    ...
    if (existingWarmup && this.isHandleAlive(existingWarmup)) {
      ...
      const modeOption = getAcpConfigOptionByCategory(
  ...
  updateBoundProcessMode(conversationId: string, modeId: string): boolean {
    ...
    if (modeOption?.type === "select") {
      handle.configState =
        updateAcpConfigStateValue(handle.configState, modeOption.id, modeId) ??
        handle.configState;
  ...
  updateBoundProcessConfigState(
    ...
    handle.currentModeId =
      legacyModeState?.currentModeId ?? handle.currentModeId;
    this.syncAgentCache(handle);
    this.notifyConfigOptionsReady(handle, conversationId);
    return true;
  }
  ...
  async release(agentId: string): Promise<void> {
    ...
    try {
      await Promise.allSettled(
        targets.map((handle) => this.disposeHandle(handle)),
      );
  ...
  async shutdown(): Promise<void> {
    ...
    const releases = Array.from(allAgents.values()).map((agentId) =>
  ...
  bindProcess(agentId: string, conversationId: string, workdir?: string): void {
    ...
    const handle =
      warmupHandles.find(
        ([, candidate]) => candidate.workdir === resolvedWorkdir,
      )?.[1] ?? warmupHandles[0]?.[1];
    if (!handle) {
  ...
  getProcessConfigState(
    agentId: string,
    workdir?: string,
  ): AcpConfigState | undefined {
    const handle = this.getScopedHandle(agentId, workdir);
    if (handle) {
  ...
  private async spawnProcess(
    ...
    try {
      return await this.spawnProcessOnce(
        agent,
        workdir,
        launchSpec,
        launchSignature,
      ...
      const repairResult = this.repairNpxCacheIfNeeded(
        agent.id,
        launchSpec,
        error,
      );
      ...
      try {
        ...
        throw this.createNpxRepairRetryError(
          agent.id,
          error,
          retryError,
          repairResult,
  ...
  private async spawnProcessOnce(
    ...
    child.on("error", (error) => {
      console.error(
        `[ACP] Agent process ${agent.id} encountered error:`,
        error,
      );
      this.debugLog.append(agent.id, {
        kind: "error",
    ...
    console.info(
      `[ACP] Process monitoring set up for agent ${agent.id} (PID: ${child.pid})`,
    );
    this.debugLog.append(agent.id, {
      kind: "lifecycle",
    ...
    console.info(
      `[ACP] Starting connection initialization for agent ${agent.id}`,
    );
    ...
    try {
      ...
      console.info(
        `[ACP] Connection initialization completed successfully for agent ${agent.id}`,
      );
      ...
      handleSeed.supportsSessionResume =
        capabilitySnapshot.supports.sessionResume;
      handleSeed.supportsSessionClose =
        capabilitySnapshot.supports.sessionClose;
      ...
      if (capabilitySnapshot.mcpCapabilities) {
        ...
        console.info(
          "[ACP] MCP capabilities:",
          capabilitySnapshot.mcpCapabilities,
        );
      }
      ...
      if (resultData.models) {
        console.info(
          `[ACP] Available models: ${resultData.models.availableModels?.length ?? 0}`,
        );
        console.info(
          `[ACP] Current model: ${resultData.models.currentModelId}`,
        );
      }
      ...
      console.error(
        `[ACP] Connection initialization failed for agent ${agent.id}:`,
        error,
      );
      this.debugLog.append(agent.id, {
        kind: "error",
      ...
      if (!child.killed) {
        try {
          child.kill();
          console.info(
            `[ACP] Killed process for failed agent ${agent.id} (PID: ${child.pid})`,
          );
        } catch (killError) {
          console.warn(
            `[ACP] Failed to kill process for agent ${agent.id}:`,
            killError,
          );
        }
      }
    ...
    const handle: AcpProcessHandle = {
      ...
      workdir,
      configState:
        handleSeed.configState ?? createEmptyAcpConfigState("legacy"),
  ...
  private repairNpxCacheIfNeeded(
    ...
    if (!target) {
      return {
        repaired: false,
        message: "error is not a _npx package.json ENOENT",
      };
    }
  ...
  private findNpxCacheRepairTarget(
    error: unknown,
  ): NpxCacheRepairTarget | null {
  ...
  private repairNpxCacheDirectory(
    target: NpxCacheRepairTarget,
  ): NpxCacheRepairResult {
  ...
  private validateSpawnCwd(
    agentId: string,
    cwd: string,
    source: "configured cwd" | "workdir",
  ) {
    if (!fs.existsSync(cwd)) {
      throw new Error(
        `[ACP] ${source} "${cwd}" does not exist for agent ${agentId}`,
      );
    }
    ...
    if (!stat.isDirectory()) {
      throw new Error(
        `[ACP] ${source} "${cwd}" is not a directory for agent ${agentId}`,
      );
    }
  }
  ...
  private async spawnAgentProcess(
    ...
    if (!launchSpec.command || launchSpec.command.trim().length === 0) {
      throw new Error(
        `[ACP] Invalid command for agent ${agent.id}: command is empty`,
      );
    }

    // Handle path expansion (including ~ and environment variables)
    const useBundledRuntime =
      launchSpec.distributionType === "npx" ||
      launchSpec.distributionType === "uvx";
    ...
    try {
      ...
      console.info(
        `[ACP] Retrieved shell environment variables for agent ${agent.id}`,
      );
    ...
    if (shellPath) {
      console.info(
        `[ACP] Using shell PATH for agent ${agent.id} (length: ${shellPath.length})`,
      );
    }
    ...
    if (useBundledRuntime) {
      const withRuntimePaths =
        this.runtimeHelper.prependBundledRuntimeToEnv(env);
    ...
    this.validateSpawnCwd(
      agent.id,
      cwd,
      configuredCwd ? "configured cwd" : "workdir",
    );
    ...
    console.info(
      `[ACP] Process spawned successfully for agent ${agent.id}, PID: ${child.pid}`,
    );
      ...
      start: async (controller) => {
        ...
        const emitLine = (line: string, action: string) => {
          ...
          try {
            ...
            console.error(
              `[ACP] ${agentId} protocol parse error from stdout:`,
              {
                error: errorMessage,
        ...
        try {
          ...
          didError = true;
          console.error(
            `[ACP] ${agentId} protocol stdout stream failed:`,
            error,
          );
          this.debugLog.append(agentId, {
            kind: "error",
  ...
  private summarizeProtocolMessage(
    ...
    const paramsKeys = isRecord(record.params)
      ? Object.keys(record.params)
      : undefined;
    const resultKeys = isRecord(record.result)
      ? Object.keys(record.result)
      : undefined;
    ...
    if (directMethod && id !== undefined) {
      ...
      const store =
        direction === "out"
          ? this.protocolRequestsToAgent
          : this.protocolRequestsFromAgent;
  ...
  private deliverSessionUpdate(
    ...
    entry.handlers.forEach((handler) => {
      try {
        ...
        console.warn(
          `[ACP] Session handler threw for session ${notification.sessionId}:`,
          error,
        );
      }
    });
  ...
  private notifyConfigOptionsReady(
    handle: AcpProcessHandle,
    conversationId?: string,
  ): void {
    const configState =
      handle.configState ?? createEmptyAcpConfigState("legacy");
    eventBus.sendToRenderer(
  ...
  private getScopedHandle(
    agentId: string,
    workdir?: string,
  ): AcpProcessHandle | undefined {
  ...
  private syncAgentCache(
    handle: Pick<
      AcpProcessHandle,
      "agentId" | "configState" | "availableModes" | "currentModeId"
    >,
  ): void {
    ...
    const availableModes =
      legacyModeState?.availableModes ?? handle.availableModes;
    const currentModeId =
      legacyModeState?.currentModeId ?? handle.currentModeId;
    ...
    if (pid) {
      if (process.platform === "win32") {
        try {
          ...
          console.warn(
            `[ACP] Failed to taskkill process ${pid} (${reason ?? "unknown"}):`,
            error,
          );
        }
      } else {
        try {
          ...
          console.warn(
            `[ACP] Failed to pkill children for process ${pid}:`,
            error,
          );
        }
        try {
