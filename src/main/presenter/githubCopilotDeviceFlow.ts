...
export class GitHubCopilotDeviceFlow {
  ...
  public async getCopilotToken(): Promise<string> {
    ...
    try {
      ...
      console.error(
        "[GitHub Copilot][DeviceFlow] Failed to get Copilot token:",
        error,
      );
      throw error;
    }
  ...
  public async checkExistingAuth(
    ...
    try {
      ...
      console.warn(
        "[GitHub Copilot][DeviceFlow] Error checking existing auth:",
        error,
      );
      return null;
    }
   ...
   * Step 2: Show user code and open browser
   */
  private async showUserCodeAndOpenBrowser(
    ...
    return new Promise((resolve) => {
      ...
      // Handle messages
      instructionWindow.webContents.on(
        "ipc-message",
        (_event, channel, ...args) => {
...
export function createGitHubCopilotDeviceFlow(
  clientIdOverride?: string,
): GitHubCopilotDeviceFlow {
  const clientId =
    clientIdOverride?.trim() || import.meta.env.VITE_GITHUB_CLIENT_ID;
  ...
  logger.info(
    "[GitHub Copilot][DeviceFlow] Creating device flow with config:",
    {
...
export function getGlobalGitHubCopilotDeviceFlow(
  ...
  if (
    !globalDeviceFlowInstance ||
    globalDeviceFlowClientId !== effectiveClientId
  ) {
