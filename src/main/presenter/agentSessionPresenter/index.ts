...
const DISABLED_SEARCH_TOOL_CLEANUP_KEY =
...
const scoreMessageHit = (
  message: SearchableMessageRow,
  normalizedQuery: string,
): number => {
...
export class AgentSessionPresenter {
  ...
  private skillPresenter?: Pick<
    ISkillPresenter,
    "setActiveSkills" | "clearNewAgentSessionSkills"
  >;
  ...
  constructor(
    ...
    this.sessionPermissionPort =
      runtimePorts?.sessionPermissionPort ?? sessionRuntimePort;
    this.sessionUiPort = runtimePorts?.sessionUiPort ?? sessionRuntimePort;

    // Register the built-in deepchat agent
    this.agentRegistry.register(
      { id: "deepchat", name: "JiaorongAI", type: "deepchat", enabled: true },
      agentRuntimeAgent,
    );
  ...
  async createSession(
    ...
    const deepChatAgentConfig =
      agentType === "deepchat"
        ? await this.resolveDeepChatAgentConfigCompat(agentId)
        : null;
    const projectDir =
    ...
    logger.info(
      `[AgentSessionPresenter] resolved provider=${providerId} model=${modelId}`,
    );
    ...
    logger.info(
      `[AgentSessionPresenter] session created id=${sessionId} title="${title}"`,
    );
    ...
    try {
      ...
      await this.cleanupFailedSessionInitialization(
        agent,
        sessionId,
        providerId,
      );
    ...
    if (
      ...
      logger.info(
        `[AgentSessionPresenter] firing queuePendingInput (non-blocking)`,
      );
      if (agent.queuePendingInput) {
        agent
          ...
          .catch((err) => {
            console.error(
              "[AgentSessionPresenter] queuePendingInput failed:",
              err,
            );
          });
      } else {
        agent
          .processMessage(sessionId, normalizedInput, { projectDir })
          .catch((err) => {
            console.error(
              "[AgentSessionPresenter] processMessage failed:",
              err,
            );
          });
      }
  ...
  async createSubagentSession(input: {
    ...
    for (
      let attempt = 1;
      attempt <= SUBAGENT_SESSION_INIT_MAX_ATTEMPTS;
      attempt += 1
    ) {
      ...
      try {
        ...
        if (runtimeConfig.activeSkills.length > 0 && this.skillPresenter) {
          await this.skillPresenter.setActiveSkills(
            sessionId,
            runtimeConfig.activeSkills,
          );
        }
        ...
        if (!record) {
          throw new Error(
            `Subagent session not found after creation: ${sessionId}`,
          );
        }

        const session = (await this.buildSessionWithState(
          record,
        )) as SessionWithState;
        this.emitSessionListUpdated({
  ...
  async ensureAcpDraftSession(input: {
    ...
    let record = await this.findReusableDraftSession(
      agentId,
      projectDir,
      agent,
    );
    ...
    if (!record) {
      ...
      if (!record) {
        throw new Error(
          `Failed to read created ACP draft session: ${sessionId}`,
        );
      }
    ...
    await (this.providerSessionPort?.prepareAcpSession?.(
      record.id,
      agentId,
      projectDir,
    ) ??
      this.llmProviderPresenter.prepareAcpSession(
        record.id,
        agentId,
        projectDir,
      ));
  ...
  async queuePendingInput(
    ...
    if (!agent.queuePendingInput) {
      throw new Error(
        `Agent ${currentSession.agentId} does not support pending inputs.`,
      );
    }
    ...
    this.assertAcpSessionHasWorkdir(
      providerId,
  ...
  async updateQueuedInput(
    ...
    if (!agent.updateQueuedInput) {
      throw new Error(
        `Agent ${session.agentId} does not support pending input edits.`,
      );
    }
    return await agent.updateQueuedInput(
      sessionId,
      itemId,
      this.normalizeSendMessageInput(content),
    );
  }
  ...
  async moveQueuedInput(sessionId: string, itemId: string, toIndex: number) {
    ...
    if (!agent.moveQueuedInput) {
      throw new Error(
        `Agent ${session.agentId} does not support pending input sorting.`,
      );
    }
  ...
  async convertPendingInputToSteer(sessionId: string, itemId: string) {
    ...
    if (!agent.convertPendingInputToSteer) {
      throw new Error(
        `Agent ${session.agentId} does not support steer conversion.`,
      );
    }
  ...
  async deletePendingInput(sessionId: string, itemId: string): Promise<void> {
    ...
    if (!agent.deletePendingInput) {
      throw new Error(
        `Agent ${session.agentId} does not support pending input deletion.`,
      );
    }
  ...
  async resumePendingQueue(sessionId: string): Promise<void> {
    ...
    if (!agent.resumePendingQueue) {
      throw new Error(
        `Agent ${session.agentId} does not support pending queue resume.`,
      );
    }
  ...
  async retryMessage(sessionId: string, messageId: string): Promise<void> {
    ...
    if (!agent.retryMessage) {
      throw new Error(
        `Agent ${session.agentId} does not support message retry.`,
      );
    }
  ...
  async deleteMessage(sessionId: string, messageId: string): Promise<void> {
    ...
    if (!agent.deleteMessage) {
      throw new Error(
        `Agent ${session.agentId} does not support message deletion.`,
      );
    }
  ...
  async editUserMessage(
    ...
    if (!agent.editUserMessage) {
      throw new Error(
        `Agent ${session.agentId} does not support user message editing.`,
      );
    }
  ...
  async forkSession(
    ...
    if (!agent.forkSessionFromMessage) {
      throw new Error(
        `Agent ${sourceSession.agentId} does not support session fork.`,
      );
    }
    ...
    try {
      ...
      await agent.forkSessionFromMessage(
        sourceSessionId,
        targetSessionId,
        targetMessageId,
      );
  ...
  async getLightweightSessionList(options?: {
    ...
    const items = page.records.map((record) =>
      this.mapSessionRecordToListItem(record),
    );
    ...
    if (prioritizeSessionId) {
      ...
      if (
        prioritizedRecord &&
        this.matchesLightweightFilter(prioritizedRecord, options)
      ) {
  ...
  async getLightweightSessionsByIds(
    sessionIds: string[],
  ...
  async searchHistory(
    ...
    const searchDocumentRows =
      this.sqlitePresenter.deepchatSearchDocumentsTable.searchFts(
        normalizedQuery,
        searchDocumentLimit,
      );
    ...
    if (candidateSearchRows.length > 0) {
      const hits = candidateSearchRows
        ...
        .filter(
          (item): item is HistorySearchHit & { rank: number } => item !== null,
        );
    ...
    const sessionRows = db
      ...
      .all(
        likeQuery,
        limit * SESSION_SEARCH_OVERQUERY_FACTOR,
      ) as SearchableSessionRow[];
    ...
    const messageRows = db
      ...
      .all(
        likeQuery,
        limit * MESSAGE_SEARCH_OVERQUERY_FACTOR,
      ) as SearchableMessageRow[];

    const sessionHits: Array<HistorySearchSessionHit & { score: number }> =
      sessionRows
        .map((session) => ({
          kind: "session" as const,
          sessionId: session.id,
    ...
    const messageHits: Array<HistorySearchMessageHit & { score: number }> =
      messageRows
        .map((message) => {
  ...
  async compactSession(
    ...
    if (!agent.compactSession) {
      throw new Error(
        `Agent ${session.agentId} does not support manual compaction.`,
      );
    }
  ...
  async listTapeAnchors(
    ...
    if (!agent.listTapeAnchors) {
      throw new Error(
        `Agent ${session.agentId} does not support tape anchors.`,
      );
    }
  ...
  async handoffTape(
    ...
    if (!agent.handoffTape) {
      throw new Error(
        `Agent ${session.agentId} does not support tape handoff.`,
      );
    }
  ...
  async mergeSubagentTape(
    ...
    if (!agent.mergeSubagentTape) {
      throw new Error(
        `Agent ${parentSession.agentId} does not support subagent tape merge.`,
      );
    }
  ...
  async discardSubagentTape(
    ...
    if (!agent.discardSubagentTape) {
      throw new Error(
        `Agent ${parentSession.agentId} does not support subagent tape discard.`,
      );
    }
  ...
  async getSearchResults(
    ...
    const rows =
      this.sqlitePresenter.deepchatMessageSearchResultsTable.listByMessageId(
        normalizedMessageId,
      );
  ...
  async startUsageStatsBackfill(): Promise<void> {
    ...
    if (
      ...
      !isUsageBackfillRunningStale(currentStatus)
    ) {
      return;
    }
  ...
  async startMainlineNormalizationBackfill(): Promise<void> {
    ...
    this.mainlineNormalizationPromise =
      this.runMainlineNormalizationBackfill().finally(() => {
  ...
  async startDisabledSearchToolCleanupBackfill(): Promise<void> {
    ...
    this.disabledSearchToolCleanupPromise =
      this.runDisabledSearchToolCleanupBackfill().finally(() => {
  ...
  async getUsageDashboard(): Promise<UsageDashboardData> {
    ...
    const cacheHitRate =
      summaryRow.inputTokens > 0
        ? summaryRow.cachedInputTokens / summaryRow.inputTokens
        : 0;
    ...
    const calendar = buildUsageDashboardCalendar(
      usageStatsTable.getDailyCalendarRows(
        this.toLocalDateKey(dateFrom.getTime()),
      ),
    );
  ...
  async repairImportedLegacySessionSkills(
    sessionId: string,
  ): Promise<string[]> {
    return await this.legacyImportService.repairImportedLegacySessionSkills(
      sessionId,
    );
  }
  ...
  async getMessageTraceCount(messageId: string): Promise<number> {
    ...
    return this.sqlitePresenter.deepchatMessageTracesTable.countByMessageId(
      normalizedMessageId,
    );
  }
  ...
  async clearSessionMessages(sessionId: string): Promise<void> {
    ...
    if (!agent.clearMessages) {
      throw new Error(
        `Agent ${session.agentId} does not support clearing messages.`,
      );
    }
  ...
  async exportSession(
    ...
    const exportMessages = records
      ...
      .map((record) =>
        this.mapRecordToExportMessage(record, providerId, modelId),
      );

    const filename = generateExportFilename(format, conversation);
    const content = buildConversationExportContent(
      conversation,
      exportMessages,
      format,
    );
  ...
  async getAgentTransferImpact(agentId: string): Promise<AgentTransferImpact> {
    ...
    for (const session of sessions) {
      ...
      if (
        samples.length < 6 &&
        (!assessment.isEmptyDraft || assessment.blockReason)
      ) {
        samples.push({
          id: session.id,
          title: session.title,
    ...
    return {
      ...
      regularSessions: sessions.filter(
        (session) => session.sessionKind === "regular",
      ).length,
      subagentSessions: sessions.filter(
  ...
  async moveAgentSessions(
    ...
    for (const session of sessions) {
      ...
      if (assessment.blockReason) {
        throw new Error(
          `Session ${session.id} cannot be moved: ${assessment.blockReason}`,
        );
      }
      if (assessment.isEmptyDraft) {
      ...
      await this.resolveTransferTargetContext(
        targetAgentId,
        session.projectDir,
      );
    ...
    try {
      ...
      for (const sessionId of emptyDraftSessionIds) {
        ...
        deleted.forEach((deletedSessionId) =>
      ...
      const partialCounts = [
        ...
        deletedSessionIds.length > 0
          ? `${deletedSessionIds.length} deleted`
          : "",
      ].filter(Boolean);
      ...
      if (partialCounts.length > 0) {
        ...
        throw new Error(
          `${message} Partial transfer completed: ${partialCounts.join(", ")}.`,
        );
      }
      throw error;
    }
  ...
  async deleteAgentSessions(agentId: string): Promise<string[]> {
    ...
    for (const session of sessions) {
      ...
      if (assessment.blockReason) {
        throw new Error(
          `Session ${session.id} cannot be deleted: ${assessment.blockReason}`,
        );
      }
    }
    ...
    for (const session of sessions) {
      if (
        deletedSessionIdSet.has(session.id) ||
        !this.sessionManager.get(session.id)
      ) {
        continue;
      }
  ...
  async respondToolInteraction(
    ...
    if (!agent.respondToolInteraction) {
      throw new Error(
        `Agent ${session.agentId} does not support tool interaction response.`,
      );
    }
    return await agent.respondToolInteraction(
      sessionId,
      messageId,
      toolCallId,
      response,
  ...
  > {
    ...
    return await (this.providerSessionPort?.getAcpSessionCommands?.(
      sessionId,
    ) ?? this.llmProviderPresenter.getAcpSessionCommands(sessionId));
  }

  async getAcpSessionConfigOptions(
    sessionId: string,
    ...
    return await (this.providerSessionPort?.getAcpSessionConfigOptions?.(
      sessionId,
  ...
  async setAcpSessionConfigOption(
    ...
    if (!(await this.isAcpBackedSession(sessionId, session.agentId))) {
      throw new Error(
        "ACP session config options are only available for ACP sessions.",
      );
    }
    return await (this.providerSessionPort?.setAcpSessionConfigOption?.(
      ...
      this.llmProviderPresenter.setAcpSessionConfigOption(
        sessionId,
        configId,
        value,
      ));
  ...
  async setSessionSubagentEnabled(
    sessionId: string,
    enabled: boolean,
  ): Promise<SessionWithState> {
    const session = this.sessionManager.get(sessionId);
    ...
    if ((await this.getAgentType(session.agentId)) !== "deepchat") {
      throw new Error("Only JiaorongAI sessions can change subagent state.");
    }
    ...
    if (!sessionWithState) {
      throw new Error(
        `Failed to build session state for sessionId: ${sessionId}`,
      );
    }
  ...
  async setSessionModel(
    ...
    if (!agent.setSessionModel) {
      throw new Error(
        `Agent ${session.agentId} does not support session model switching.`,
      );
    }
  ...
  async setSessionProjectDir(
    ...
    if (!sessionWithState) {
      throw new Error(
        `Failed to build session state after project update: ${sessionId}`,
      );
    }
    return sessionWithState;
  }

  async getSessionGenerationSettings(
    sessionId: string,
  ...
  async updateSessionGenerationSettings(
    ...
    if (!agent.updateGenerationSettings) {
      throw new Error(
        `Agent ${session.agentId} does not support generation settings updates.`,
      );
    }
    ...
    try {
      ...
      try {
        ...
        const shouldFallback =
          preferredProviderId !== fallbackProviderId ||
  ...
  private emitSessionListUpdated(
    ...
    const reason =
      options.reason ?? (sessionIds.length > 0 ? "updated" : "list-refreshed");

    eventBus.sendToRenderer(
      SESSION_EVENTS.LIST_UPDATED,
      SendTarget.ALL_WINDOWS,
    );
    publishDeepchatEvent("sessions.updated", {
      sessionIds,
  ...
  private dedupeAndSortSessionListItems(
  ...
  private async resolveAgentImplementation(
    agentId: string,
  ): Promise<IAgentImplementation> {
  ...
  private async getAgentType(
    agentId: string,
  ): Promise<"deepchat" | "acp" | null> {
    if (typeof this.configPresenter.getAgentType !== "function") {
      ...
      const fallbackAgent =
        await this.configPresenter.getAgent?.(resolvedAgentId);
      ...
      if (
        acpAgents?.some(
          (agent) => resolveAcpAgentAlias(agent.id) === resolvedAgentId,
        )
      ) {
        return "acp";
      }
    ...
    return await this.configPresenter.getAgentType(
  ...
  private async resolveDeepChatAgentConfigCompat(
    ...
    ReturnType<IConfigPresenter["resolveDeepChatAgentConfig"]>
  > | null> {
  ...
  private async resolveSubagentSessionRuntimeConfig(input: {
    ...
    const normalizedTargetAgentId = input.targetAgentId?.trim()
      ? resolvedAgentId
      : null;
    const agentType = await this.getAgentType(resolvedAgentId);
    if (agentType !== "deepchat" && agentType !== "acp") {
    ...
    return {
      ...
      disabledAgentTools: this.normalizeDisabledAgentTools(
  ...
  private async assessTransferSession(session: SessionRecord): Promise<{
    ...
    const isEmptyDraft =
      Boolean(session.isDraft) && !hasMessages && !hasSubagentChildren;
    let hasPendingInput = false;

    if (agent.listPendingInputs) {
      try {
        hasPendingInput =
          (await agent.listPendingInputs(session.id)).length > 0;
      } catch (error) {
        console.warn(
  ...
  private async moveSessionToAgentInternal(
    ...
    if (!allowSubagent && session.sessionKind !== "regular") {
      throw new Error(
        "Only regular conversations can be moved from the conversation menu.",
      );
    }
    ...
    if (session.agentId === targetAgentId) {
      throw new Error(
        "Conversation is already assigned to the selected agent.",
      );
    }
    ...
    if (assessment.blockReason) {
      throw new Error(
        `Session ${sessionId} cannot be moved: ${assessment.blockReason}`,
      );
    }

    const previousAgentId = session.agentId;
    const targetContext = await this.resolveTransferTargetContext(
      targetAgentId,
      session.projectDir,
    );
    const previousAcpBacked = await this.isAcpBackedSession(
      sessionId,
      previousAgentId,
    );
    ...
    if (!agent.setSessionAgentContext) {
      throw new Error(
        `Agent ${targetContext.agentId} does not support session transfer.`,
      );
    }
    ...
    this.sessionManager.update(sessionId, {
      ...
      subagentEnabled:
        session.sessionKind === "regular"
          ? targetContext.subagentEnabled
          : false,
    });
    this.sessionManager.updateDisabledAgentTools(
      sessionId,
      targetContext.disabledAgentTools,
    );
    ...
    if (!sessionWithState) {
      throw new Error(
        `Failed to build session state after transfer: ${sessionId}`,
      );
    }
  ...
  private async resolveTransferTargetContext(
    ...
    const defaultModel = this.configPresenter.getDefaultModel();
    const providerId =
      config?.defaultModelPreset?.providerId?.trim() ||
      defaultModel?.providerId?.trim() ||
      "";
    const modelId =
      config?.defaultModelPreset?.modelId?.trim() ||
  ...
  private async isAcpBackedSession(
    sessionId: string,
    agentId: string,
  ): Promise<boolean> {
    const resolvedAgentId = resolveAcpAgentAlias(agentId);
  ...
  private async syncAcpSessionWorkdir(
    ...
    try {
      ...
      console.warn(
        "[AgentSessionPresenter] Failed to sync ACP workdir for session:",
        {
          conversationId,
          agentId,
  ...
  private async buildExportConversation(
    ...
    const modelConfig =
      resolvedProviderId && resolvedModelId
        ? this.configPresenter.getModelConfig(
            resolvedModelId,
            resolvedProviderId,
          )
        : undefined;
    ...
    return {
      ...
      settings: {
        ...
        temperature:
          generationSettings?.temperature ?? modelConfig?.temperature ?? 0.7,
        contextLength:
          generationSettings?.contextLength ??
          modelConfig?.contextLength ??
          32000,
        maxTokens:
          generationSettings?.maxTokens ?? modelConfig?.maxTokens ?? 8000,
  ...
  private mapRecordToExportMessage(
    ...
    return {
      ...base,
      role: "assistant",
      content: this.parseAssistantExportBlocks(
        record.content,
        record.createdAt,
      ),
    };
  ...
  private parseUserExportContent(content: string): Message["content"] {
    ...
    try {
      ...
      const files = Array.isArray(parsedRecord.files)
        ? (parsedRecord.files as Array<Record<string, unknown>>).map(
            (file) => ({
  ...
  private parseAssistantExportBlocks(
    content: string,
    timestamp: number,
  ): Message["content"] {
    try {
      ...
      return {
        ...
        cacheWriteInputTokens:
          typeof parsed.cacheWriteInputTokens === "number"
            ...
            : undefined,
        generationTime:
          typeof parsed.generationTime === "number"
            ? parsed.generationTime
            : undefined,
        firstTokenTime:
          typeof parsed.firstTokenTime === "number"
            ? parsed.firstTokenTime
            : undefined,
        tokensPerSecond:
          typeof parsed.tokensPerSecond === "number"
  ...
  private async runMainlineNormalizationBackfill(): Promise<void> {
    const startedAt = Date.now();
    this.sqlitePresenter.configTables.setAgentSetting(
    ...
    try {
      ...
      for (const sessionRow of sessionRows) {
        const activeSkills =
          this.sqlitePresenter.newSessionsTable.getActiveSkills(sessionRow.id);
        const disabledAgentTools =
          this.sqlitePresenter.newSessionsTable.getDisabledAgentTools(
            sessionRow.id,
          );
  ...
  private async runDisabledSearchToolCleanupBackfill(): Promise<void> {
    ...
    try {
      ...
      for (const sessionRow of sessionRows ?? []) {
        ...
        const normalized = this.normalizeDisabledAgentTools(
          disabledAgentTools,
          {
      ...
      const configUpdatedCount =
        await this.cleanupDeepChatAgentConfigDisabledTools();
  ...
  private async cleanupDeepChatAgentConfigDisabledTools(): Promise<number> {
    ...
    for (const agent of agents) {
      ...
      const config = await this.configPresenter.getDeepChatAgentConfig(
        agent.id,
      );
  ...
  private backfillNormalizedMessageRow(row: DeepChatMessageRow): void {
    if (row.role === "user") {
      ...
      if (content) {
        ...
        this.sqlitePresenter.deepchatUserMessageLinksTable.replaceForMessage(
          row.id,
          content.links,
        );
      }
    ...
    if (row.status === "sent" || row.status === "error") {
      const title =
        this.sqlitePresenter.newSessionsTable.get(row.session_id)?.title ?? "";
      this.sqlitePresenter.deepchatSearchDocumentsTable.upsert({
  ...
  private parseBackfillUserMessageContent(
    rawContent: string,
    ...
    try {
      ...
      return {
        ...
        files: Array.isArray(parsed.files)
          ? (parsed.files.filter(Boolean) as MessageFile[])
          : [],
  ...
  private parseBackfillAssistantBlocks(
    rawContent: string,
  ): AssistantMessageBlock[] {
    try {
  ...
  private async runUsageStatsBackfill(): Promise<void> {
    ...
    try {
      const usageStatsTable = this.sqlitePresenter.deepchatUsageStatsTable;
      const candidates =
        this.sqlitePresenter.deepchatMessagesTable.listAssistantUsageCandidates();
  ...
  private getUsageStatsBackfillStatus(): UsageStatsBackfillStatus {
    const normalized = this.normalizeUsageStatsBackfillStatus(
      this.configPresenter.getSetting<UsageStatsBackfillStatus>(
        DASHBOARD_STATS_BACKFILL_KEY,
      ),
    );
    if (
      normalized.status === "failed" &&
      normalized.error === "Usage stats backfill timed out"
    ) {
  ...
  private normalizeUsageStatsBackfillStatus(
    status: unknown,
  ): UsageStatsBackfillStatus {
  ...
  private sortUsageBreakdown(
    items: UsageDashboardBreakdownItem[],
  ): UsageDashboardBreakdownItem[] {
  ...
  private assertAcpSessionHasWorkdir(
    providerId: string,
    projectDir: string | null,
  ): void {
    if (providerId !== "acp") {
    ...
    throw new Error(
      "ACP agent requires selecting a workdir before sending messages.",
    );
  }
