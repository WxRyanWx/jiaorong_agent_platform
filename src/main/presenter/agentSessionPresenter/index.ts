...
export class AgentSessionPresenter {
  ...
  async compactSession(
    ...
    if (agentType === 'acp' || providerId === 'acp') {
      throw new Error('Manual compaction is only available for JiaorongAI agent sessions.')
    }
  ...
  private isTransferChatModelType(type: string | undefined): boolean {
    return (
  ...
  private async resolveDeepChatTransferModelSelection(
    config: Awaited<ReturnType<IConfigPresenter['resolveDeepChatAgentConfig']>> | null
  ): Promise<{ providerId: string; modelId: string } | null> {
    const defaultModel = this.configPresenter.getDefaultModel()
    const candidates = [
      {
        providerId: config?.defaultModelPreset?.providerId?.trim() ?? '',
        modelId: config?.defaultModelPreset?.modelId?.trim() ?? ''
      },
      {
        providerId: defaultModel?.providerId?.trim() ?? '',
        modelId: defaultModel?.modelId?.trim() ?? ''
      }
    ]
    ...
    for (const candidate of candidates) {
      if (
        ...
        candidate.modelId &&
        candidate.providerId.toLowerCase() !== 'acp'
      ) {
        return candidate
      }
    }

    if (typeof this.configPresenter.getAllEnabledModels !== 'function') {
      return null
    }

    const enabledGroups = await this.configPresenter.getAllEnabledModels()
    for (const group of enabledGroups) {
      if (group.providerId.toLowerCase() === 'acp') {
        continue
      }

      for (const model of group.models) {
        if (!model.id || !this.isTransferChatModelType(model.type)) {
          continue
        }
  ...
  private async resolveTransferTargetContext(
    ...
    const explicitProvider = config?.defaultModelPreset?.providerId?.trim() ?? ''
    const explicitModel = config?.defaultModelPreset?.modelId?.trim() ?? ''
    if (explicitProvider && explicitModel) {
      if (explicitProvider.toLowerCase() === 'acp') {
        throw new Error('Conversation history cannot be moved to ACP agents.')
      }
    }
    const modelSelection = await this.resolveDeepChatTransferModelSelection(config)
