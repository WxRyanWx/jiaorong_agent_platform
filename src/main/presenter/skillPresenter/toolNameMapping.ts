...
export function normalizeSkillToolName(
  toolName: string,
): NormalizeSkillToolNameResult {
...
export function normalizeSkillAllowedTools(
  tools: string[],
): NormalizeSkillAllowedToolsResult {
  ...
  for (const originalToolName of tools) {
    ...
    if (!isCanonical && !mapped) {
      warnings.push(
        `[SkillTools] Unknown allowedTools entry kept as-is: ${originalToolName}`,
      );
    }
    if (mapped && canonical !== originalToolName) {
      warnings.push(
        `[SkillTools] Mapped allowedTools entry: ${originalToolName} -> ${canonical}`,
      );
    }
