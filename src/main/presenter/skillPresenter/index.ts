...
import {
  discoverSkillMetadataInWorker,
  logSkillDiscoveryWorkerWarnings,
...
const DRAFT_ALLOWED_TOP_LEVEL_DIRS = new Set([
  "references",
  "templates",
  "scripts",
  "assets",
...
function sanitizeSkillExtensionConfig(input: unknown): SkillExtensionConfig {
  ...
  const runtimePolicy = (candidate.runtimePolicy ??
    {}) as Partial<SkillRuntimePolicy>;
  const python =
  ...
  const scriptOverrides = Object.fromEntries(
    Object.entries(candidate.scriptOverrides ?? {})
      ...
      .map(([key, value]) => {
        ...
        if (
          typeof (override as { description?: unknown }).description ===
          "string"
        ) {
          const description = (
            override as { description: string }
          ).description.trim();
          if (description) {
...
export class SkillPresenter implements ISkillPresenter {
  ...
  private repairPortableDefaultSkillsPath(
    ...
    const match =
      ...
      slashPath.match(
        /^[A-Za-z]:\/Users\/[^/]+\/\.deepchat\/skills(?:\/(.*))?$/i,
      );
  ...
  private async discoverSkillsOnMainThread(): Promise<SkillMetadata[]> {
    ...
    const skillManifestPaths = [
      ...this.collectSkillManifestPaths(this.skillsDir),
    ...
    for (const skillPath of skillManifestPaths) {
      ...
      try {
        ...
        console.error(
          `[SkillPresenter] Failed to parse skill at ${skillPath}:`,
          error,
        );
      }
    }
  ...
  private async discoverPluginSkillsOnMainThread(): Promise<SkillMetadata[]> {
    ...
    for (const contribution of this.pluginSkillContributions.values()) {
      ...
      if (!(await this.pathExists(skillPath))) {
        logger.warn(
          "[SkillPresenter] Plugin skill contribution is missing SKILL.md.",
          {
  ...
  private async parseSkillMetadata(
    ...
    try {
      ...
      if (!data.name || !data.description) {
        console.warn(
          `[SkillPresenter] Skill ${dirName} missing required frontmatter fields`,
        );
        return null;
      }
      ...
      console.error(
        `[SkillPresenter] Error parsing skill metadata at ${skillPath}:`,
        error,
      );
      return null;
    }
  ...
  private getVisibleMetadataFromCache(): SkillMetadata[] {
    return this.sortSkillMetadata(
      Array.from(this.metadataCache.values()).filter((skill) =>
        this.isSkillVisible(skill),
      ),
    );
  }
  ...
  async loadSkillContent(name: string): Promise<SkillContent | null> {
    ...
    try {
      ...
      const skillContent: SkillContent = {
        name,
        content: [renderedContent.trim(), runtimeInstructions]
          .filter(Boolean)
          .join("\n\n"),
      };
      ...
      console.error(
        `[SkillPresenter] Error loading skill content for ${name}:`,
        error,
      );
      return null;
    }
  ...
  async viewSkill(
    ...
    if (options?.filePath?.trim()) {
      try {
        ...
        console.error(
          "[SkillPresenter] Failed to load requested skill file for skill_view:",
          {
            name: metadata.name,
    ...
    try {
      ...
      if (options?.conversationId && !isPinned) {
        const updatedSkills = await this.setActiveSkills(
          options.conversationId,
          [...pinnedSkills, metadata.name],
        );
      ...
      switch (action) {
        ...
        case "write_file": {
          ...
          if (!request.filePath?.trim()) {
            return {
              success: false,
              action,
              error: "filePath is required for write_file",
            };
          }
          if (typeof request.fileContent !== "string") {
            return {
              success: false,
              action,
              error: "fileContent is required for write_file",
            };
          }
          const resolvedFilePath = this.resolveDraftFilePath(
            draftPath,
            request.filePath,
          );
          if (!resolvedFilePath) {
          ...
          const blockedPattern = this.findDraftInjectionPattern(
            request.fileContent,
          );
          if (blockedPattern) {
            return {
        ...
        case "remove_file": {
          ...
          if (!request.filePath?.trim()) {
            return {
              success: false,
              action,
              error: "filePath is required for remove_file",
            };
          }
          const resolvedFilePath = this.resolveDraftFilePath(
            draftPath,
        ...
        default:
          return {
            success: false,
            action,
            error: `Unsupported draft action: ${action}`,
          };
      }
    } catch (error) {
      return {
  ...
  async viewDraftSkill(
    ...
    try {
      ...
      return {
        ...
        draftId: normalizedDraftId,
        ...(parsed.success
          ? { skillName: parsed.skillName, content }
          : { error: parsed.error }),
      };
    } catch (error) {
      return {
  ...
  private replacePathVariables(
    ...
    const pluginContribution = this.getPluginContributionForSkillRoot(
      metadata.skillRoot,
    );
    return content
  ...
  private async buildRuntimeInstructions(
    ...
    const scripts = (await this.listSkillScripts(metadata.name)).filter(
  ...
  async installBuiltinSkills(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (!result.success) {
        console.warn(
          "[SkillPresenter] Failed to install builtin skill:",
          result.error,
        );
      }
    }
  ...
  private supportsCurrentPlatform(platforms?: string[]): boolean {
    ...
    return platforms.some((platform) =>
      aliases.has(platform.trim().toLowerCase()),
    );
  }
  ...
  async installFromZip(
    ...
    const tempDir = fs.mkdtempSync(
      path.join(app.getPath("temp"), "deepchat-skill-"),
    );
    try {
  ...
  private backupExistingSkill(skillName: string): string {
    ...
    while (fs.existsSync(backupDir)) {
      ...
      backupDir = path.join(
        this.skillsDir,
        `${skillName}.backup-${timestamp}-${counter}`,
      );
    }
  ...
  private extractZipToDirectory(zipPath: string, targetDir: string): void {
    ...
    if (stats.size > SKILL_CONFIG.ZIP_MAX_SIZE) {
      throw new Error(
        `ZIP file too large: ${stats.size} bytes (max: ${SKILL_CONFIG.ZIP_MAX_SIZE})`,
      );
    }
  ...
  private async downloadSkillZip(url: string, destPath: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to download skill zip: ${response.status} ${response.statusText}`,
        );
      }
      ...
      if (
        contentLength &&
        parseInt(contentLength) > SKILL_CONFIG.ZIP_MAX_SIZE
      ) {
        throw new Error(
  ...
  async saveSkillWithExtension(
    ...
    const previousSidecarContent = hadSidecar
  ...
  async getSkillExtension(name: string): Promise<SkillExtensionConfig> {
    ...
    try {
      ...
      logger.warn(
        "[SkillPresenter] Failed to read skill sidecar, using defaults",
        {
          name,
          error,
        },
  ...
  async listSkillScripts(name: string): Promise<SkillScriptDescriptor[]> {
    ...
    const descriptors = (
      await this.collectScriptDescriptors(scriptsDir, metadata.skillRoot)
    ).map((script) => {
  ...
  private async loadNewSessionSkills(
    conversationId: string,
  ): Promise<string[]> {
    const persistedSkills = this.getPersistedNewSessionSkills(conversationId);
    if (
      persistedSkills.length > 0 ||
      !this.isImportedLegacySessionId(conversationId)
    ) {
      return persistedSkills;
    }

    try {
      return await this.sessionStatePort.repairImportedLegacySessionSkills(
        conversationId,
      );
    } catch (error) {
      console.warn(
  ...
  private warnLegacySkillRetired(conversationId: string): void {
    ...
    logger.warn(
      "[SkillPresenter] Ignoring skill state update for retired legacy conversation.",
      {
        conversationId,
      },
    );
  ...
  async setActiveSkills(
    ...
    try {
      ...
      const deactivated = previousSkills.filter(
        (skill) => !validSet.has(skill),
      );
      ...
      console.error(
        `[SkillPresenter] Error setting active skills for ${conversationId}:`,
        error,
      );
      throw error;
    }
  ...
  watchSkillFiles(): void {
    ...
    this.watcher = watch(this.skillsDir, {
      ...
      ignored: (watchPath) =>
        watchPath.includes(
          `${path.sep}${SKILL_CONFIG.SIDECAR_DIR}${path.sep}`,
    ...
    this.watcher.on("change", async (filePath: string) => {
      if (path.basename(filePath) === "SKILL.md") {
        const previousName =
          this.findSkillNameByPath(filePath) ??
        ...
        if (metadata) {
          ...
          if (existingMetadata && existingMetadata.path !== metadata.path) {
            ...
            if (
              ...
              previousMetadata?.path === metadata.path
            ) {
  ...
  private async collectScriptDescriptors(
    ...
    for (const entry of entries) {
      ...
      const runtime =
        SUPPORTED_SCRIPT_EXTENSIONS[path.extname(entry.name).toLowerCase()];
      if (!runtime) {
        continue;
      }
  ...
  private deriveSkillCategory(skillRoot: string): string | null {
    const pluginContribution =
      this.getPluginContributionForSkillRoot(skillRoot);
  ...
  private getPluginContributionForSkillRoot(
    ...
    return Array.from(this.pluginSkillContributions.values()).find(
      (contribution) =>
        path.resolve(contribution.skillRoot) === path.resolve(skillRoot),
    );
  }

  private async listSkillLinkedFiles(
    skillRoot: string,
  ...
  private resolveSkillRelativePath(
    skillRoot: string,
    filePath: string,
  ): string | null {
  ...
  private validateDraftSkillDocument(
    ...
    if (!content.trimStart().startsWith("---")) {
      return {
        success: false,
        error: "Draft skill content must include YAML frontmatter",
      };
    }
    ...
    if (!SKILL_NAME_PATTERN.test(skillName) || skillName.length > 64) {
      return {
        success: false,
        error:
          "Skill name must match ^[a-z0-9][a-z0-9._-]*$ and be <= 64 characters",
      };
    }
  ...
  private findDraftInjectionPattern(content: string): string | null {
    const matched = DRAFT_INJECTION_PATTERNS.find((pattern) =>
  ...
  private createDraftHandle(conversationId: string): {
    draftId: string;
    draftPath: string;
  } {
  ...
  private resolveDraftFilePath(
    draftPath: string,
    relativeFilePath: string,
  ): string | null {
    const normalizedFilePath = relativeFilePath
      .trim()
      .replace(/\\/g, "/")
      .replace(/^\/+/, "");
  ...
  private touchDraftActivity(draftPath: string): void {
    fs.writeFileSync(
      this.getDraftActivityMarkerPath(draftPath),
      `${Date.now()}`,
      "utf-8",
    );
  }
  ...
  private cleanupExpiredDrafts(): void {
    ...
    const now = Date.now();
    const conversationEntries = fs.readdirSync(this.draftsRoot, {
      withFileTypes: true,
    });
    for (const conversationEntry of conversationEntries) {
      ...
      const draftEntries = fs.readdirSync(conversationDir, {
        withFileTypes: true,
      });
      ...
      if (
        fs.existsSync(conversationDir) &&
        fs.readdirSync(conversationDir).length === 0
      ) {
  ...
  private setPersistedNewSessionSkills(
    conversationId: string,
    skills: string[],
  ): void {
