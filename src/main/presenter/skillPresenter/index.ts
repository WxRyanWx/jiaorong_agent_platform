...
export class SkillPresenter implements ISkillPresenter {
  ...
  async discoverSkills(): Promise<SkillMetadata[]> {
    ...
    for (const metadata of [
      ...
      const dirName = path.basename(metadata.skillRoot)
      const enriched = this.overlayBuiltinSkillFrontmatter(
        metadata,
        dirName,
        metadata.ownerPluginId
      )
      if (this.metadataCache.has(enriched.name)) {
      ...
      this.metadataCache.set(enriched.name, enriched)
    }
  ...
  private async parseSkillMetadata(
    ...
    try {
      ...
      return this.overlayBuiltinSkillFrontmatter(
        {
  ...
  /** Prefer bundled builtin frontmatter for display fields on built-in skills. */
  private overlayBuiltinSkillFrontmatter(
    ...
    const builtinMdPath = path.join(builtinDir, dirName, 'SKILL.md')
    if (!fs.existsSync(builtinMdPath)) {
      return metadata
    }

    try {
      const { data } = matter(fs.readFileSync(builtinMdPath, 'utf-8'))
      ...
      console.warn(`[SkillPresenter] Failed to overlay builtin frontmatter for ${dirName}:`, error)
      return metadata
    }
  }
