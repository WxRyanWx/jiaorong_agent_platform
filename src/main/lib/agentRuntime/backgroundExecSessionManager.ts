...
export class BackgroundExecSessionManager {
  ...
  async start(
    ...
    logger.info(
      `[BackgroundExec] Started session ${sessionId} for conversation ${conversationId}`,
    );
  ...
  async poll(conversationId: string, sessionId: string): Promise<PollResult> {
    ...
    if (isOffloaded && session.outputFilePath) {
      const output = this.getRecentOutputFromSession(
        session,
  ...
  async remove(conversationId: string, sessionId: string): Promise<void> {
    ...
    await session.outputWriteQueue.catch((error) => {
      logger.warn(
        "[BackgroundExec] Failed while draining output write queue:",
        error,
      );
    });
  ...
  async cleanupConversation(conversationId: string): Promise<void> {
    ...
    await Promise.all(
      sessionIds.map((id) => this.remove(conversationId, id).catch(() => {})),
    );
  }
  ...
  private async killInternal(
    ...
    logger.info(
      `[BackgroundExec] Killing session ${session.sessionId} (reason: ${reason})`,
    );
  ...
  private getRecentOutputFromSession(
    session: BackgroundSession,
    maxChars: number,
  ): string {
    if (!session.outputFilePath) {
    ...
    const filePreview = this.readLastCharsFromFile(
  ...
  private readOutputFromSession(
    ...
    const persistedOutput = this.readFromFile(
      session.outputFilePath,
      offset,
      fileLimit,
    );
  ...
  private queueOutputWrite(
    ...
    session.outputWriteQueue = session.outputWriteQueue
      ...
      .catch((error) => {
        logger.warn(
          `[BackgroundExec] Failed to write output file (${mode}):`,
          error,
        );
  ...
  private async finalizeSession(
    ...
    try {
      ...
      await session.outputWriteQueue.catch((error) => {
        logger.warn(
          "[BackgroundExec] Failed while draining output queue:",
          error,
        );
      });
    } finally {
  ...
  private runCleanup(): void {
    ...
    for (const [conversationId, sessions] of this.sessions) {
      for (const [sessionId, session] of sessions) {
        if (now - session.lastAccessedAt > config.cleanupMs) {
          ...
          now - session.lastAccessedAt > 5 * 60 * 1000
        ) {
          expiredSessions.push({ conversationId, sessionId });
        }
      }
    ...
    for (const { conversationId, sessionId } of expiredSessions) {
      logger.info(
        `[BackgroundExec] Auto-removing expired session ${sessionId}`,
      );
      void this.remove(conversationId, sessionId).catch((error) => {
        logger.warn(
          "[BackgroundExec] Failed to remove expired session:",
          error,
        );
      });
    }
...
class BackgroundExecUtilityProxy {
  ...
  async list(conversationId: string): Promise<SessionMeta[]> {
    ...
    const hostSessions = this.host
      ? await this.request<SessionMeta[]>("list", [conversationId]).catch(
          (error) => {
            logger.warn(
              "[BackgroundExecProxy] Failed to list utility sessions:",
              error,
            );
            return active;
          },
  ...
  async poll(conversationId: string, sessionId: string): Promise<PollResult> {
    ...
    const result = await this.request<PollResult>("poll", [
      conversationId,
      sessionId,
    ]);
  ...
  async log(
    ...
    const result = await this.request<LogResult>("log", [
      conversationId,
      sessionId,
      offset,
      limit,
  ...
  async waitForCompletionOrYield(
    ...
    const result = await this.request<WaitForCompletionOrYieldResult>(
      "waitForCompletionOrYield",
      [conversationId, sessionId, yieldMs],
    );
  ...
  async getCompletionResult(
    ...
    const result = await this.request<SessionCompletionResult>(
      "getCompletionResult",
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      this.rejectPendingRequests(
        new Error("Background exec utility process shut down."),
      );
  ...
  private async startHost(): Promise<UtilityProcess> {
    ...
    host.on("error", (type, location) => {
      logger.error("[BackgroundExecProxy] Utility process error:", {
        type,
        location,
      });
    });
    ...
    return await new Promise<UtilityProcess>((resolve, reject) => {
      ...
      const onExit = (code: number) => {
        settle(() => {
          reject(
            new Error(
              `Background exec utility process exited before spawn: ${code}`,
            ),
          );
        });
      };
  ...
  private handleHostExit(code: number): void {
    const error = new Error(
      `Background exec utility process exited with code ${code}.`,
    );
  ...
  private toCrashedCompletionResult(
    session: TrackedSessionMeta,
  ): SessionCompletionResult {
    return {
      status: "error",
      output: this.crashMessage(session),
