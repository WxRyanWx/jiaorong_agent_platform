<template>
  <div data-testid="settings-deepchat-agents-page" class="flex h-full w-full">
      ...
      <div class="flex items-center justify-between gap-3 px-4 py-4">
        <div>
          <div class="text-lg font-semibold">
            {{ t("settings.deepchatAgents.title") }}
          </div>
    ...
    <main class="min-w-0 flex-1 overflow-y-auto">
      ...
      >
        <div
          class="mx-auto flex w-full max-w-5xl items-start justify-between gap-4 px-6 py-4"
        >
      ...
      <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-6">
        <section
          class="grid gap-4 rounded-2xl border border-border p-5 md:grid-cols-2"
        >
          <label class="space-y-2">
            <div class="text-sm font-medium">
              {{ t("settings.deepchatAgents.name") }}
            </div>
            <Input
              v-model="form.name"
          ...
          <label class="space-y-2">
            <div class="text-sm font-medium">
              {{ t("settings.deepchatAgents.enabledLabel") }}
            </div>
            <div
        ...
        <section class="space-y-4 rounded-2xl border border-border p-5">
          <div class="text-sm font-semibold">
            {{ t("settings.deepchatAgents.avatarTitle") }}
          </div>
          ...
          <div
            v-if="form.avatarKind === 'lucide'"
            class="grid gap-4 md:grid-cols-2"
          >
            <label class="space-y-2 md:col-span-2">
              <div class="text-sm font-medium">
                {{ t("settings.deepchatAgents.lucideIcon") }}
              </div>
            ...
            <label class="space-y-2">
              <div class="text-sm font-medium">
                {{ t("settings.deepchatAgents.lightColor") }}
              </div>
              <div
            ...
            <label class="space-y-2">
              <div class="text-sm font-medium">
                {{ t("settings.deepchatAgents.darkColor") }}
              </div>
              <div
                class="flex items-center gap-3 rounded-lg border border-border px-3 py-2"
              >
          ...
          <div
            v-else-if="form.avatarKind === 'monogram'"
            class="grid gap-4 md:grid-cols-2"
          >
            <label class="space-y-2">
              <div class="text-sm font-medium">
                {{ t("settings.deepchatAgents.monogramText") }}
              </div>
              <Input
        ...
        <section class="space-y-4 rounded-2xl border border-border p-5">
          <div class="text-sm font-semibold">
            {{ t("settings.deepchatAgents.modelsTitle") }}
          </div>
              ...
              <Popover v-model:open="field.open.value">
                ...
                <PopoverContent class="w-[320px] p-0" align="start">
                  ...
                  <ModelSelect
                    ...
                    @update:model="
                      (model, providerId) =>
                        selectModel(field.key, model, providerId)
                    "
                  />
                </PopoverContent>
              </Popover>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" class="w-[20rem]">
                  ...
                  >
                    ...
                    <Icon
                      v-if="
                        normalizePath(form.defaultProjectPath) === project.path
                      "
                      icon="lucide:check"
        ...
        <section class="space-y-4 rounded-2xl border border-border p-5">
          <div class="space-y-2">
            <div class="flex items-center justify-between gap-3">
              <div class="text-sm font-medium">
                {{ t("settings.deepchatAgents.systemPrompt") }}
              </div>
              <Button
                variant="outline"
                size="sm"
                class="gap-2"
                @click="openSystemPromptPicker"
              >
            ...
            <Textarea
              ...
              :placeholder="
                t('settings.deepchatAgents.systemPromptPlaceholder')
              "
            />
          </div>
        </section>
        ...
        <section class="space-y-4 rounded-2xl border border-border p-5">
          <div class="text-sm font-semibold">
            {{ t("settings.deepchatAgents.toolsTitle") }}
          </div>
          <div
        ...
        <section class="space-y-4 rounded-2xl border border-border p-5">
          ...
          <div
            v-if="form.autoCompactionEnabled"
            class="grid gap-4 md:grid-cols-2"
          >
            <label class="space-y-2">
              <div class="text-sm font-medium">
...
const modelFields = computed(() => [
  {
    ...
    label: t("settings.deepchatAgents.chatModel"),
    open: chatOpen,
  },
  {
...
const isAvailableSubagentTargetAgent = (agent: Agent) => {
  ...
  return (
    agent.source !== "registry" || agent.installState?.status === "installed"
  );
};
...
const normalizeAutoCompactionTriggerThreshold = (
  value: EditableNumberValue | null | undefined,
) =>
...
const normalizeAutoCompactionRetainRecentPairs = (
  value: EditableNumberValue | null | undefined,
) =>
  normalizeNumericInput(value, {
...
const fromAgent = (agent?: Agent | null): FormState => {
  ...
  return {
    ...
    assistantModel: config.assistantModel
      ? {
          providerId: config.assistantModel.providerId,
          modelId: config.assistantModel.modelId,
        }
      : null,
    visionModel: config.visionModel
      ? {
          providerId: config.visionModel.providerId,
          modelId: config.visionModel.modelId,
        }
      : null,
...
const modelText = (selection: EditableModel | undefined) => {
  ...
  const matchedModel = providerModels?.models.find(
...
const handleSubagentTargetChange = (
...
const pickDefaultProjectPath = async () => {
  try {
    ...
    console.warn(
      "[DeepChatAgentsSettings] Failed to select default project path:",
      error,
    );
  }
};
...
const isToolEnabled = (toolName: string) =>
  !form.disabledAgentTools.includes(toolName);
...
const getGroupToolNames = (group: ToolGroup) =>
  group.tools.map((tool) => tool.function.name);
...
const loadTools = async () => {
  try {
    const definitions = await toolPresenter.getAllToolDefinitions({
      chatMode: "agent",
    });
...
const selectAgent = (agentId: string) => {
  ...
  assignForm(
    fromAgent(
      deepchatAgents.value.find((agent) => agent.id === agentId) ?? null,
    ),
  );
};
const startCreate = () => {
...
const saveAgent = async () => {
  ...
  try {
    ...
    if (form.id) {
      const updated = await configPresenter.updateDeepChatAgent(
        form.id,
        payload,
      );
...
const handleDeleteAgentWithMove = async (payload: {
