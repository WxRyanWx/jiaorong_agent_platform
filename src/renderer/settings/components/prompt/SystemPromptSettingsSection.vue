        ...
        <AlertDialog v-else>
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                @click="deleteSystemPrompt(currentSystemPrompt.id)"
              >
                {{ t("common.confirm") }}
              </AlertDialogAction>
            </AlertDialogFooter>
...
const updateCurrentSystemPrompt = () => {
  ...
  currentSystemPrompt.value =
    systemPrompts.value.find(
      (prompt) => prompt.id === selectedSystemPromptId.value,
    ) || null;
};
...
const resetDefaultSystemPrompt = async () => {
  try {
    const originalContent = `You are JiaorongAI, a highly capable AI assistant. Your goal is to fully complete the user's requested task before handing the conversation back to them. Keep working autonomously until the task is fully resolved.
