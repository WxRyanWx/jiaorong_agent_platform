    ...
    <div class="flex w-full items-center justify-between px-1 py-2">
        ...
        <template v-if="isAcpAgent">
          ...
          >
            <PopoverTrigger as-child>
              ...
              >
                <span class="truncate">{{
                  getAcpOptionDisplayValue(option)
                }}</span>
        ...
        <Popover v-else-if="showModelPopover" v-model:open="isModelPanelOpen">
          ...
          >
            <div class="flex max-h-[28rem]">
                      ...
                      <div class="space-y-1">
                        ...
                        >
                          <button
                            ...
                            @click="
                              handleModelQuickSelect(group.providerId, model.id)
                            "
                          >
                            <ModelIcon
                              :model-id="
                                resolveModelIconId(group.providerId, model.id)
                              "
                              custom-class="w-3.5 h-3.5 shrink-0"
                              :is-dark="themeStore.isDark"
                            />
                          ...
                          <Button
                            ...
                            @click.stop="
                              openModelSettings(group.providerId, model.id)
                            "
                          >
                            <Icon
              ...
              <div
                v-if="isModelSettingsExpanded"
                class="flex w-[21rem] min-w-0 flex-col"
              >
                ...
                <div class="max-h-[24rem] overflow-y-auto px-3 py-3">
                  ...
                  <div v-else-if="localSettings" class="space-y-4">
                    <TooltipProvider :delay-duration="200">
                      ...
                      <div v-if="showTopPControl" class="space-y-1.5">
                        ...
                        <div class="flex items-center gap-2">
                          <Button
                            ...
                            "
                            :disabled="
                              hasNumericInputError('topP') ||
                              topPDecreaseDisabled
                            "
                            @click="stepTopP(-1)"
                          >
                            <Icon icon="lucide:minus" class="h-3 w-3" />
                      ...
                      <div
                        v-if="!showOpenAIMediaGenerationSettings"
                        class="space-y-1.5"
                      >
                        <label class="text-xs font-medium">{{
                        ...
                        <div class="flex items-center gap-2">
                          <Button
                            ...
                            :aria-label="
                              t('chat.advancedSettings.decreaseValue', {
                                label: t(
                                  'settings.model.modelConfig.timeout.label',
                                ),
                              })
                            "
                            :disabled="
                      ...
                      >
                        ...
                        <Select
                          ...
                          "
                          @update:model-value="
                            onReasoningEffortSelect($event as string)
                          "
                        >
                          <SelectTrigger class="h-8 text-xs">
                            <SelectValue
                              :placeholder="
                                t(
                                  'settings.model.modelConfig.reasoningEffort.placeholder',
                                )
                              "
                            />
                          </SelectTrigger>
                      ...
                      >
                        ...
                        <Select
                          ...
                          "
                          @update:model-value="
                            onReasoningVisibilitySelect($event as string)
                          "
                        >
                          <SelectTrigger class="h-8 text-xs">
                            <SelectValue
                              :placeholder="
                                t(
                                  'settings.model.modelConfig.reasoningVisibility.placeholder',
                                )
                              "
                            />
                          </SelectTrigger>
                      ...
                      >
                        ...
                        <Select
                          :model-value="
                            localSettings.verbosity ??
                            verbosityOptions[0]?.value
                          "
                          @update:model-value="
                            onVerbositySelect($event as string)
                          "
                        >
                          <SelectTrigger class="h-8 text-xs">
                            <SelectValue
                              :placeholder="
                                t(
                                  'settings.model.modelConfig.verbosity.placeholder',
                                )
                              "
                            />
                          </SelectTrigger>
                            ...
                            <Switch
                              ...
                              :aria-label="
                                t('chat.advancedSettings.toggleValue', {
                                  label: t(
                                    'chat.advancedSettings.thinkingBudget',
                                  ),
                                })
                              "
                              @update:model-value="
                                onThinkingBudgetToggle(Boolean($event))
                              "
                            />
                          </div>
                        </div>
                        <div
                          v-if="isThinkingBudgetEnabled"
                        ...
                        <div class="flex items-start justify-between gap-3">
                          <div class="min-w-0">
                            <label class="text-xs font-medium">
                              {{
                                t(
                                  "chat.advancedSettings.forceInterleavedThinkingCompat",
                                )
                              }}
                            </label>
                            <p
                            ...
                            >
                              {{
                                t(
                                  "chat.advancedSettings.forceInterleavedThinkingCompatDescription",
                                )
                              }}
                            </p>
                          </div>
                          <Switch
                            ...
                            "
                            @update:model-value="
                              onInterleavedThinkingToggle(Boolean($event))
                            "
                          />
                        </div>
                      </div>
      ...
      <div class="flex items-center gap-1">
            ...
            <div class="max-h-[24rem] space-y-3 overflow-y-auto px-3 py-3">
              ...
              >
                ...
                <Select
                  ...
                  @update:model-value="
                    onAcpSelectOption(option.id, $event as string)
                  "
                >
                  <SelectTrigger
                    :disabled="
                      acpConfigReadOnly || isAcpOptionSaving(option.id)
                    "
        ...
        <McpIndicator
          ...
          :show-custom-system-prompt-badge="
            selectedSystemPromptId === '__custom__'
          "
        ...
        <DropdownMenu v-if="!isAcpAgent">
          ...
          <DropdownMenuContent align="end" class="min-w-48">
            ...
            >
              <Icon
                :icon="option.icon"
                :class="['h-3.5 w-3.5 shrink-0', option.iconClass]"
              />
...
const inferAgentType = (
  ...
  const matchedAgent = availableAgents.value.find(
  ...
  const selectedAgent =
    agentStore.selectedAgent && agentStore.selectedAgent.id === agentId
      ...
      : null;
  const explicitType =
    matchedAgent?.agentType ?? matchedAgent?.type ?? selectedAgent?.type;
...
const resolveDeepChatAgentConfig = async (
  agentId: string,
  ...
  const defaultSystemPrompt =
    (await configClient.getDefaultSystemPrompt()) ?? "";

  return normalizeDeepChatSubagentConfig({
    defaultModelPreset: undefined,
    systemPrompt:
      typeof defaultSystemPrompt === "string" ? defaultSystemPrompt : "",
    permissionMode: "full_access",
    disabledAgentTools: [],
  });
};
...
const isModelSelectionLocked = computed(
  () => isAcpAgent.value && Boolean(lockedAcpModelId.value),
);
const showModelPopover = computed(
  () =>
    !isAcpAgent.value ||
    Boolean(activeAcpSessionId.value || acpWorkspacePath.value),
);
...
const isModelOptionsReady = computed(
  () => isAcpAgent.value || modelStore.initialized,
);
const hasModelOptionsError = computed(
  () =>
    !isAcpAgent.value &&
    !modelStore.initialized &&
    Boolean(modelStore.initializationError),
);
...
const resolveProviderApiType = (providerId: string): string | undefined =>
  providerStore.sortedProviders.find((provider) => provider.id === providerId)
    ?.apiType;
...
const modelSettingsTargetResolvedConfig = computed(() =>
  isSameModelSelection(
    modelSettingsTarget.value,
    modelSettingsTargetConfigSelection.value,
  )
...
const showOpenAIMediaGenerationSettings = computed(
  () =>
    ...
    showOpenAIVideoGenerationSettings.value,
);
...
watch(
  ...
  async (target) => {
    ...
    try {
      const config = await modelClient.getModelConfig(
        target.modelId,
        target.providerId,
      );
      ...
      console.warn(
        "[ChatStatusBar] Failed to load model settings target config:",
        error,
      );
    }
  },
...
const findEnabledModelMeta = (
  providerId: string,
  modelId: string,
): RENDERER_MODEL_META | null => {
...
const topPCommittedValue = computed(
  () => localSettings.value?.topP ?? TOP_P_MAX,
);
const topPDecreaseDisabled = computed(
  () =>
    localSettings.value?.topP === undefined ||
    topPCommittedValue.value <= TOP_P_MIN,
);
const topPIncreaseDisabled = computed(
  () =>
    localSettings.value?.topP !== undefined &&
    topPCommittedValue.value >= TOP_P_MAX,
);
const contextLengthInputValue = computed(() =>
...
const thinkingBudgetInputValue = computed(() =>
...
const showTemperatureControl = computed(
  () =>
    (capabilitySupportsTemperature.value !== false ||
      isMoonshotKimiTemperatureLocked.value) &&
    Boolean(localSettings.value),
);
const supportsTopPControl = computed(
  () =>
    capabilityProviderId.value !== "anthropic" ||
    capabilitySupportsTemperature.value !== false,
);
...
const isAnthropicReasoningEnabled = computed(() => {
  ...
  return getReasoningEffectiveEnabledForProvider(
    ...
    {
      reasoning:
        localSettings.value.reasoningEffort !== undefined ? true : undefined,
...
const effortOptions = computed(() => {
  return getReasoningEffortOptions(capabilityReasoningPortrait.value).map(
    (value) => ({
      value,
...
const verbosityOptions = computed(() => {
  return getVerbosityOptions(capabilityReasoningPortrait.value).map(
    (value) => ({
      value,
...
const systemPromptOptions = computed<SystemPromptOption[]>(() => {
  ...
  const matched = presetOptions.find(
    (option) => option.content === currentPrompt,
  );
  if (matched) {
    return presetOptions;
  }
...
const selectedSystemPromptId = computed(() => {
  if (
    !hasLoadedGenerationSettingsForCurrentSelection.value ||
    !localSettings.value
  ) {
    return "empty";
  }
  const currentPrompt = localSettings.value.systemPrompt;
  const matched = systemPromptOptions.value.find(
    (option) => option.content === currentPrompt,
  );
...
const modelSettingsProviderText = computed(() => {
  ...
  const providerName =
    providerNameMap.value.get(selection.providerId) ?? selection.providerId;
...
const displayIconId = computed(() => {
  if (hasActiveSession.value) {
    return resolveModelIconId(
      ...
      activeSessionSelection.value?.modelId ||
...
const displayModelText = computed(() => {
  if (!isModelOptionsReady.value) {
    return hasModelOptionsError.value
      ? t("model.error.loadFailed")
      : t("common.loading");
  }
  if (isAcpAgent.value) {
...
const syncDraftModelSelection = async () => {
  ...
  if (isAcpAgent.value) {
    const agentId = agentStore.selectedAgentId;
    applyDraftSelection(
      selectedAgentType.value === "acp" && agentId
        ? { providerId: "acp", modelId: agentId }
        : null,
    );
    return;
    ...
    const resolvedModel = resolvePreferredChatModel({
      ...
      selections: [
        ...
        isModelSelection(agentConfig.defaultModelPreset)
          ...
          : null,
        isModelSelection(preferredModel)
          ? (preferredModel as ChatModelSelection)
          : null,
        isModelSelection(defaultModel)
          ? (defaultModel as ChatModelSelection)
          : null,
      ],
    });
    applyDraftSelection(
...
const resolveDefaultGenerationSettings = async (
  ...
  const maxTokensDefault =
    toValidNonNegativeInteger(modelConfig.maxTokens) ??
  ...
  if (supportsVerbosity(portrait)) {
    const verbosity = normalizeVerbosity(
      portrait,
      modelConfig.verbosity ?? portrait?.verbosity,
    );
    if (verbosity) {
...
const flushGenerationPatch = async () => {
  ...
  try {
    const updated = await sessionClient.updateSessionGenerationSettings(
      sessionId,
      patch,
    );
    ...
    console.warn(
      "[ChatStatusBar] Failed to update generation settings:",
      error,
    );
  }
};

const scheduleGenerationPersist = (
...
const updateLocalGenerationSettings = (
  patch: Partial<SessionGenerationSettings>,
) => {
  if (!localSettings.value) {
    return;
  ...
  if (
    ...
    normalizedPatch.forceInterleavedThinkingCompat =
      next.forceInterleavedThinkingCompat;
  }
  if (Object.prototype.hasOwnProperty.call(nextPatch, "imageGeneration")) {
    normalizedPatch.imageGeneration = normalizeImageGenerationOptions(
  ...
  if (Object.prototype.hasOwnProperty.call(nextPatch, "videoGeneration")) {
    normalizedPatch.videoGeneration = normalizeVideoGenerationOptions(
...
const syncGenerationSettings = async () => {
  ...
  if (sessionId) {
    try {
      const settings =
        await sessionClient.getSessionGenerationSettings(sessionId);
      if (token !== generationSyncToken) {
        return;
      }
      if (settings) {
      ...
      console.warn(
        "[ChatStatusBar] Failed to load session generation settings:",
        error,
      );
    }
  }
...
const reloadSystemPrompts = async () => {
  try {
    ...
    console.warn(
      "[ChatStatusBar] Failed to load system prompt options:",
      error,
    );
  ...
  async ([sessionId, canSelect, draftPermissionMode]) => {
    ...
    if (!sessionId) {
      permissionMode.value =
        draftPermissionMode === "default" ? "default" : "full_access";
      return;
    }
...
async function completeSwitchModelOnboardingIfNeeded(
  ...
  try {
    ...
    console.warn(
      "[ChatStatusBar] Failed to complete switch-model onboarding step:",
      error,
    );
  }
}
...
async function changeModelSelection(
  ...
  if (hasActiveSession.value) {
    ...
    try {
      ...
      console.warn(
        "[ChatStatusBar] Failed to switch active session model:",
        error,
      );
      return { applied: false, selectionChanged: false, previousSelection };
    }
  ...
  const previousDraftSelection = draftModelSelection.value
...
const commitNumericField = (
  ...
  const error = validateGenerationNumericField(
    field,
    rawValue,
    getNumericValidationContext(field),
  );
  if (error) {
    stopNumericInputEdit(field);
  ...
  if (numeric === undefined) {
    ...
    setNumericInputError(
      field,
      field === "temperature" ? "finite_number" : "non_negative_integer",
    );
    return undefined;
  }
...
const roundTemperatureStepValue = (value: number): number =>
...
function stepTopP(direction: -1 | 1) {
  ...
  const next = Math.min(
    TOP_P_MAX,
    Math.max(TOP_P_MIN, current + direction * TOP_P_STEP),
  );
...
function commitTemperatureInput() {
  ...
  const next = commitNumericField(
    "temperature",
    numericInputDrafts.value.temperature,
  );
  if (next === undefined) {
    return;
  }
...
function stepContextLength(direction: -1 | 1) {
  ...
  const next = Math.max(
    0,
    localSettings.value.contextLength + direction * CONTEXT_LENGTH_STEP,
  );
...
function commitContextLengthInput() {
  const next = commitNumericField(
    "contextLength",
    numericInputDrafts.value.contextLength,
  );
  if (next === undefined) {
    return;
  }
...
function stepMaxTokens(direction: -1 | 1) {
  ...
  const next = Math.max(
    0,
    localSettings.value.maxTokens + direction * MAX_TOKENS_STEP,
  );
...
function commitMaxTokensInput() {
  const next = commitNumericField(
    "maxTokens",
    numericInputDrafts.value.maxTokens,
  );
  if (next === undefined) {
    return;
  }
...
function stepTimeout(direction: -1 | 1) {
  ...
  const next = Math.max(
    ...
    Math.min(
      TIMEOUT_MAX,
      localSettings.value.timeout + direction * TIMEOUT_STEP,
    ),
  );
...
function onThinkingBudgetToggle(enabled: boolean) {
  ...
  const preferred =
    normalizeLegacyThinkingBudgetValue(localSettings.value.thinkingBudget) ?? 0;
...
function commitThinkingBudgetInput() {
  const next = commitNumericField(
    "thinkingBudget",
    numericInputDrafts.value.thinkingBudget,
  );
  if (next === undefined) {
    return;
  }
...
function onVerbositySelect(value: string) {
  ...
  const normalized = normalizeVerbosity(
    capabilityReasoningPortrait.value,
    value,
  );
  if (!normalized) {
...
async function onSubagentToggle(enabled: boolean) {
  ...
  try {
    ...
    subagentEnabled.value =
      sessionStore.activeSession?.subagentEnabled === true;
  } finally {
