...
JiaorongAI should import CC Switch providers from the active CC Switch Desktop data directory when CC Switch is configured to use a custom config path.
...
- As a user with Codex-only CC Switch rows, JiaorongAI continues to hide Codex provider rows from provider import.
