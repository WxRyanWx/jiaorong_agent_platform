...
Restore Windows ARM64 desktop startup and smoke E2E coverage by upgrading JiaorongAI's DuckDB Node bindings to a release that ships `win32-arm64` native binaries and preserves built-in knowledge base behavior.
...
The failure is caused by JiaorongAI loading `@duckdb/node-api@1.3.2-alpha.25` during boot. That package version only ships native bindings for:
...
- Keep JiaorongAI's built-in knowledge base DuckDB/VSS flow unchanged if the newer release remains compatible
