...
JiaorongAI Agent applies a chat-oriented provider-call context preflight before sending model
...
- Only apply JiaorongAI's chat context budget to chat model requests.
...
- Image generation models or image endpoints reach the provider even when chat-budget estimation
  would fail.
- Non-chat requests do not trigger the JiaorongAI context-pressure compaction path solely because of
