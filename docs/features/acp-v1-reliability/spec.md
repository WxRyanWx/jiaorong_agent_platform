...
JiaorongAI 已经具备 ACP agent 的基本启动、初始化、`session/new`、`session/load`、`session/prompt`、文件系统、终端、模式、模型和部分 session update 映射能力。但按 ACP v1 官方协议逐项核对后，当前实现仍有几个影响可靠性的缺口：认证流程未产品化，session lifecycle 不完整，部分通知会丢，Prompt 会重复发送历史，Terminal 输出截断方向不符合规范，部分状态更新没有进入 JiaorongAI 状态层。

本目标是把 JiaorongAI 的 ACP 能力修到“全功能可靠”的 v1 版本：按 agent 初始化返回的 capability 精准启用功能，能稳定接入 registry agent、本地 DimCode、Claude Code ACP 和 Codex ACP，且所有协议行为都有测试或手工矩阵覆盖。

会话数据事实源以 JiaorongAI records 为准。远端 ACP agent 返回的 session 是外部资源目录，JiaorongAI 只做 workspace 维度的导入、同步和绑定；导入后必须转换成 JiaorongAI 自己的消息格式并持久化。同步不得反复重复导入，也不得把远端 metadata 直接覆盖用户在 JiaorongAI 内手工维护的会话数据。
...
- 作为 JiaorongAI 用户，我可以从 ACP registry 或本地命令启动 agent，并清楚看到该 agent 支持哪些 ACP v1 能力。
- 作为使用 Claude Code ACP 或 Codex ACP 的用户，我可以在 JiaorongAI 内完成 agent 暴露的认证流程，而不是只看到失败或需要手工猜测环境变量。
- 作为使用 DimCode 的用户，我可以按 workspace 看到远端已有 session，把需要的 session 导入或绑定到 JiaorongAI 会话，并持续收到 slash commands、模式、配置项和标题更新。
...
- `initialize` 会声明 JiaorongAI 实际支持的 client capabilities，并保存 agent 返回的完整 capabilities、auth methods、agent info。
...
- `usage_update`、`session_info_update`、plan、mode、config options、slash commands 都能进入 JiaorongAI 状态或 debug log。
- JiaorongAI conversation 是最终持久化事实源；远端 session list/load/resume 只建立或更新 `AcpSessionLink`，不会重复创建同一远端 session 的本地会话。
- 远端历史消息导入会先转换为 JiaorongAI message/block 格式，再按稳定 fingerprint 去重持久化。
...
| Protocol                              | ACP v1 期望                                                                                                                                    | JiaorongAI 当前状态                                                                                          | 修复/对接目标                                                                                                                                           |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Session Setup: `session/new`          | 创建新 session，传 `cwd` 和 MCP servers，返回 `sessionId`，可带初始 modes/models/config options                                                | 已支持；但 listener 通常在返回后注册，早期 update 可能丢                                                     | 新 JiaorongAI 会话首次使用 ACP agent 时才创建远端 session；返回后写入本地 `AcpSessionLink`；缓冲并 flush 早期 update                                    |
| Session Setup: `session/load`         | 仅 `loadSession=true` 时调用；agent 会重放历史 update，再响应 load 完成                                                                        | 已支持并在 load 前注册 listener                                                                              | 用作远端 session 历史导入/重放；进入 staging buffer，转换为 JiaorongAI message/block 后按 fingerprint 幂等落库                                          |
| Session Setup: `session/resume`       | 仅 `sessionCapabilities.resume` 存在时调用；不重放历史，恢复上下文后返回                                                                       | 未接入                                                                                                       | 用于已绑定 JiaorongAI conversation 的继续对话；不把远端 session 当事实源覆盖本地消息                                                                    |
...
| Session List                          | 仅 `sessionCapabilities.list` 存在时调用；支持 `cwd` filter、cursor pagination；`session_info_update` 同步标题/更新时间                        | 未接入 list；`session_info_update` 被忽略                                                                    | 按 workspace 同步远端 session catalog；用 `agentId + canonicalWorkdir + remoteSessionId` 去重；只更新 link metadata，不直接覆盖 JiaorongAI conversation |
...
| Client Permission                     | Client baseline method `session/request_permission` 用于工具权限确认                                                                           | 主进程已有 resolver 分发底座；需要 UI/超时/debug/test 闭环                                                   | 复用现有 JiaorongAI permission overlay；补 timeout/cancel 默认 outcome；debug log 记录 permission request/result                                        |
...
- 不默认扩大文件系统权限；ACP fs/terminal 继续受 session workdir 和 JiaorongAI 安全策略约束。
- 不做远端 session 的主动批量写入或双向同步；远端 session catalog 是可导入资源，JiaorongAI conversation 才是本地事实源。
