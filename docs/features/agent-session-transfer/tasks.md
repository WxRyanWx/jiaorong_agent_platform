...
- [x] Runtime: add a JiaorongAI runtime method to update session agent context without deleting
...
- [x] ACP handling: clear stale source ACP bindings when moving ACP chats to JiaorongAI, and block ACP
      targets so JiaorongAI-to-ACP and ACP-to-ACP moves cannot occur.
- [x] Agent deletion safety: remove silent JiaorongAI fallback reassignment and prevent deleting an
...
- [x] Review fix: clear source ACP provider bindings only after target JiaorongAI context and
...
- [x] i18n: add English and Chinese strings first, then run the repository i18n workflow for other
      locales.
- [ ] Tests: main presenter coverage exists for impact summaries, JiaorongAI moves, ACP-to-JiaorongAI,
